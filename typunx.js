const swBase = self.location.pathname.replace(/typunx\.js$/, '')

importScripts(swBase + 'ouc/zibr.bundle.js')
importScripts(swBase + 'ouc/zibr.config.js')
importScripts(__hgh$config.sw || swBase + 'ouc/zibr.sw.js')

importScripts(swBase + 'hwrm/eepf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kgdyyjur', 1)
			...
			// `openDB('$kgdyyjur', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dqiq/')) {
