const swBase = self.location.pathname.replace(/khuudp\.js$/, '')

importScripts(swBase + 'wus/hhav.bundle.js')
importScripts(swBase + 'wus/hhav.config.js')
importScripts(__zfw$config.sw || swBase + 'wus/hhav.sw.js')

importScripts(swBase + 'qhll/arun.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$snjkjcwo', 1)
			...
			// `openDB('$snjkjcwo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xhkg/')) {
