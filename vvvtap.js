const swBase = self.location.pathname.replace(/vvvtap\.js$/, '')

importScripts(swBase + 'auj/qlin.bundle.js')
importScripts(swBase + 'auj/qlin.config.js')
importScripts(__ynq$config.sw || swBase + 'auj/qlin.sw.js')

importScripts(swBase + 'dmqy/lizl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dlshzpke', 1)
			...
			// `openDB('$dlshzpke', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zbeb/')) {
