const swBase = self.location.pathname.replace(/ffoczi\.js$/, '')

importScripts(swBase + 'phn/imev.bundle.js')
importScripts(swBase + 'phn/imev.config.js')
importScripts(__oxm$config.sw || swBase + 'phn/imev.sw.js')

importScripts(swBase + 'aipu/ipxa.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ucjfmijw', 1)
			...
			// `openDB('$ucjfmijw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'semf/')) {
