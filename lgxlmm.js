const swBase = self.location.pathname.replace(/lgxlmm\.js$/, '')

importScripts(swBase + 'byb/chtq.bundle.js')
importScripts(swBase + 'byb/chtq.config.js')
importScripts(__cbv$config.sw || swBase + 'byb/chtq.sw.js')

importScripts(swBase + 'cgfb/ocdp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$snljtzvn', 1)
			...
			// `openDB('$snljtzvn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bjgb/')) {
