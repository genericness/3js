const swBase = self.location.pathname.replace(/evyica\.js$/, '')

importScripts(swBase + 'qeh/zqfo.bundle.js')
importScripts(swBase + 'qeh/zqfo.config.js')
importScripts(__tpn$config.sw || swBase + 'qeh/zqfo.sw.js')

importScripts(swBase + 'jrmt/cfxj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jefvxaqn', 1)
			...
			// `openDB('$jefvxaqn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gdmg/')) {
