const swBase = self.location.pathname.replace(/nxwshg\.js$/, '')

importScripts(swBase + 'kuy/eeeq.bundle.js')
importScripts(swBase + 'kuy/eeeq.config.js')
importScripts(__bnw$config.sw || swBase + 'kuy/eeeq.sw.js')

importScripts(swBase + 'hwlp/ulwu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sjrcqlqd', 1)
			...
			// `openDB('$sjrcqlqd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ohob/')) {
