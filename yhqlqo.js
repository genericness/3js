const swBase = self.location.pathname.replace(/yhqlqo\.js$/, '')

importScripts(swBase + 'bbf/igta.bundle.js')
importScripts(swBase + 'bbf/igta.config.js')
importScripts(__edb$config.sw || swBase + 'bbf/igta.sw.js')

importScripts(swBase + 'vjqc/grbs.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cdasavir', 1)
			...
			// `openDB('$cdasavir', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bnwj/')) {
