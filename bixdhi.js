const swBase = self.location.pathname.replace(/bixdhi\.js$/, '')

importScripts(swBase + 'fwb/nuib.bundle.js')
importScripts(swBase + 'fwb/nuib.config.js')
importScripts(__hfl$config.sw || swBase + 'fwb/nuib.sw.js')

importScripts(swBase + 'ayzg/kunw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yjoqacpn', 1)
			...
			// `openDB('$yjoqacpn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jwir/')) {
