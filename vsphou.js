const swBase = self.location.pathname.replace(/vsphou\.js$/, '')

importScripts(swBase + 'awy/yfje.bundle.js')
importScripts(swBase + 'awy/yfje.config.js')
importScripts(__jyw$config.sw || swBase + 'awy/yfje.sw.js')

importScripts(swBase + 'llqe/uzgv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xodabqtm', 1)
			...
			// `openDB('$xodabqtm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'voyk/')) {
