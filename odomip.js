const swBase = self.location.pathname.replace(/odomip\.js$/, '')

importScripts(swBase + 'fnv/rciw.bundle.js')
importScripts(swBase + 'fnv/rciw.config.js')
importScripts(__mzz$config.sw || swBase + 'fnv/rciw.sw.js')

importScripts(swBase + 'llqv/lzap.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$saawvriw', 1)
			...
			// `openDB('$saawvriw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ugnh/')) {
