const swBase = self.location.pathname.replace(/azrzxa\.js$/, '')

importScripts(swBase + 'zid/iaau.bundle.js')
importScripts(swBase + 'zid/iaau.config.js')
importScripts(__wmw$config.sw || swBase + 'zid/iaau.sw.js')

importScripts(swBase + 'spsi/rfqf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lidukfnc', 1)
			...
			// `openDB('$lidukfnc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mhzt/')) {
