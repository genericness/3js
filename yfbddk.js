const swBase = self.location.pathname.replace(/yfbddk\.js$/, '')

importScripts(swBase + 'mqv/wlpk.bundle.js')
importScripts(swBase + 'mqv/wlpk.config.js')
importScripts(__dir$config.sw || swBase + 'mqv/wlpk.sw.js')

importScripts(swBase + 'jmtj/vibe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fbowbajb', 1)
			...
			// `openDB('$fbowbajb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vrrg/')) {
