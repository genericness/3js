const swBase = self.location.pathname.replace(/ikajmr\.js$/, '')

importScripts(swBase + 'kue/urms.bundle.js')
importScripts(swBase + 'kue/urms.config.js')
importScripts(__ltd$config.sw || swBase + 'kue/urms.sw.js')

importScripts(swBase + 'ubhc/kifu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qwyefdzx', 1)
			...
			// `openDB('$qwyefdzx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yrdk/')) {
