const swBase = self.location.pathname.replace(/nkuvhe\.js$/, '')

importScripts(swBase + 'cxm/fgjd.bundle.js')
importScripts(swBase + 'cxm/fgjd.config.js')
importScripts(__irj$config.sw || swBase + 'cxm/fgjd.sw.js')

importScripts(swBase + 'bpgi/jctw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tpvxuttf', 1)
			...
			// `openDB('$tpvxuttf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ogmx/')) {
