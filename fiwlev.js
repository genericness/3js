const swBase = self.location.pathname.replace(/fiwlev\.js$/, '')

importScripts(swBase + 'evp/ecqm.bundle.js')
importScripts(swBase + 'evp/ecqm.config.js')
importScripts(__lra$config.sw || swBase + 'evp/ecqm.sw.js')

importScripts(swBase + 'xsnn/ivbi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vnqpilqm', 1)
			...
			// `openDB('$vnqpilqm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'eswu/')) {
