const swBase = self.location.pathname.replace(/qdedrx\.js$/, '')

importScripts(swBase + 'aei/lekc.bundle.js')
importScripts(swBase + 'aei/lekc.config.js')
importScripts(__epv$config.sw || swBase + 'aei/lekc.sw.js')

importScripts(swBase + 'cmaf/kokh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vfmrbtzv', 1)
			...
			// `openDB('$vfmrbtzv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bmrl/')) {
