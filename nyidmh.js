const swBase = self.location.pathname.replace(/nyidmh\.js$/, '')

importScripts(swBase + 'onb/cejl.bundle.js')
importScripts(swBase + 'onb/cejl.config.js')
importScripts(__kls$config.sw || swBase + 'onb/cejl.sw.js')

importScripts(swBase + 'syzr/lnmb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$uuypurnd', 1)
			...
			// `openDB('$uuypurnd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qeqg/')) {
