const swBase = self.location.pathname.replace(/nlkrif\.js$/, '')

importScripts(swBase + 'hvz/ruwy.bundle.js')
importScripts(swBase + 'hvz/ruwy.config.js')
importScripts(__hkj$config.sw || swBase + 'hvz/ruwy.sw.js')

importScripts(swBase + 'hppo/njqb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sshzpvxv', 1)
			...
			// `openDB('$sshzpvxv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'oicm/')) {
