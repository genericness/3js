const swBase = self.location.pathname.replace(/srgohs\.js$/, '')

importScripts(swBase + 'nct/wvvg.bundle.js')
importScripts(swBase + 'nct/wvvg.config.js')
importScripts(__gxi$config.sw || swBase + 'nct/wvvg.sw.js')

importScripts(swBase + 'vmjl/qubz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bxthqnwh', 1)
			...
			// `openDB('$bxthqnwh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vjtv/')) {
