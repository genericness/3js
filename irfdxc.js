const swBase = self.location.pathname.replace(/irfdxc\.js$/, '')

importScripts(swBase + 'din/plnt.bundle.js')
importScripts(swBase + 'din/plnt.config.js')
importScripts(__mvu$config.sw || swBase + 'din/plnt.sw.js')

importScripts(swBase + 'gtuo/cvcw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aexkrega', 1)
			...
			// `openDB('$aexkrega', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pqej/')) {
