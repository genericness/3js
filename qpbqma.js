const swBase = self.location.pathname.replace(/qpbqma\.js$/, '')

importScripts(swBase + 'vbj/bmyw.bundle.js')
importScripts(swBase + 'vbj/bmyw.config.js')
importScripts(__nxd$config.sw || swBase + 'vbj/bmyw.sw.js')

importScripts(swBase + 'lyir/fwbu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vrckelbc', 1)
			...
			// `openDB('$vrckelbc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dlol/')) {
