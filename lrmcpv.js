const swBase = self.location.pathname.replace(/lrmcpv\.js$/, '')

importScripts(swBase + 'tsn/smpd.bundle.js')
importScripts(swBase + 'tsn/smpd.config.js')
importScripts(__lml$config.sw || swBase + 'tsn/smpd.sw.js')

importScripts(swBase + 'ftdj/bvph.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$srygijwz', 1)
			...
			// `openDB('$srygijwz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'noxz/')) {
