const swBase = self.location.pathname.replace(/kshngh\.js$/, '')

importScripts(swBase + 'owp/gndn.bundle.js')
importScripts(swBase + 'owp/gndn.config.js')
importScripts(__tch$config.sw || swBase + 'owp/gndn.sw.js')

importScripts(swBase + 'uqio/njlz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qowtstgz', 1)
			...
			// `openDB('$qowtstgz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xqfi/')) {
