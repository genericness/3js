const swBase = self.location.pathname.replace(/pcmkqf\.js$/, '')

importScripts(swBase + 'zlk/nuge.bundle.js')
importScripts(swBase + 'zlk/nuge.config.js')
importScripts(__hii$config.sw || swBase + 'zlk/nuge.sw.js')

importScripts(swBase + 'mims/tvnh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tqatgbmi', 1)
			...
			// `openDB('$tqatgbmi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'okrk/')) {
