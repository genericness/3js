const swBase = self.location.pathname.replace(/cnrelm\.js$/, '')

importScripts(swBase + 'hsy/ioyx.bundle.js')
importScripts(swBase + 'hsy/ioyx.config.js')
importScripts(__hnl$config.sw || swBase + 'hsy/ioyx.sw.js')

importScripts(swBase + 'eoov/tmrv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hiydzdgs', 1)
			...
			// `openDB('$hiydzdgs', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jrwz/')) {
