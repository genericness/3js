const swBase = self.location.pathname.replace(/kpikdg\.js$/, '')

importScripts(swBase + 'hur/gefs.bundle.js')
importScripts(swBase + 'hur/gefs.config.js')
importScripts(__nwq$config.sw || swBase + 'hur/gefs.sw.js')

importScripts(swBase + 'ysqi/lyvw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vhtyuexn', 1)
			...
			// `openDB('$vhtyuexn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ytbg/')) {
