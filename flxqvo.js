const swBase = self.location.pathname.replace(/flxqvo\.js$/, '')

importScripts(swBase + 'svh/mnoa.bundle.js')
importScripts(swBase + 'svh/mnoa.config.js')
importScripts(__etg$config.sw || swBase + 'svh/mnoa.sw.js')

importScripts(swBase + 'qiwu/lqnr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$okfstwiu', 1)
			...
			// `openDB('$okfstwiu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hpzv/')) {
