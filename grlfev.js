const swBase = self.location.pathname.replace(/grlfev\.js$/, '')

importScripts(swBase + 'fci/ikkn.bundle.js')
importScripts(swBase + 'fci/ikkn.config.js')
importScripts(__dsc$config.sw || swBase + 'fci/ikkn.sw.js')

importScripts(swBase + 'khic/ckrc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wfsgplhd', 1)
			...
			// `openDB('$wfsgplhd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dlbp/')) {
