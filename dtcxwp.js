const swBase = self.location.pathname.replace(/dtcxwp\.js$/, '')

importScripts(swBase + 'kws/ucqf.bundle.js')
importScripts(swBase + 'kws/ucqf.config.js')
importScripts(__emm$config.sw || swBase + 'kws/ucqf.sw.js')

importScripts(swBase + 'mzrt/errw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mqgusvle', 1)
			...
			// `openDB('$mqgusvle', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gtmi/')) {
