const swBase = self.location.pathname.replace(/zqlepo\.js$/, '')

importScripts(swBase + 'bgp/jeuw.bundle.js')
importScripts(swBase + 'bgp/jeuw.config.js')
importScripts(__jln$config.sw || swBase + 'bgp/jeuw.sw.js')

importScripts(swBase + 'opin/kiww.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xsunotfs', 1)
			...
			// `openDB('$xsunotfs', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nhez/')) {
