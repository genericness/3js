const swBase = self.location.pathname.replace(/nvvenx\.js$/, '')

importScripts(swBase + 'lnw/vfys.bundle.js')
importScripts(swBase + 'lnw/vfys.config.js')
importScripts(__wth$config.sw || swBase + 'lnw/vfys.sw.js')

importScripts(swBase + 'hudx/cmor.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pmpkwivc', 1)
			...
			// `openDB('$pmpkwivc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fnls/')) {
