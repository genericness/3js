const swBase = self.location.pathname.replace(/cqrerj\.js$/, '')

importScripts(swBase + 'kvs/mukp.bundle.js')
importScripts(swBase + 'kvs/mukp.config.js')
importScripts(__mtq$config.sw || swBase + 'kvs/mukp.sw.js')

importScripts(swBase + 'dnzd/xipz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vgsveels', 1)
			...
			// `openDB('$vgsveels', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gyzr/')) {
