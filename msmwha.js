const swBase = self.location.pathname.replace(/msmwha\.js$/, '')

importScripts(swBase + 'qsg/qhgy.bundle.js')
importScripts(swBase + 'qsg/qhgy.config.js')
importScripts(__ezc$config.sw || swBase + 'qsg/qhgy.sw.js')

importScripts(swBase + 'uqvn/baoa.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ppuzuxix', 1)
			...
			// `openDB('$ppuzuxix', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lkjw/')) {
