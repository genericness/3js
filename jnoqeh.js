const swBase = self.location.pathname.replace(/jnoqeh\.js$/, '')

importScripts(swBase + 'qjv/lwvz.bundle.js')
importScripts(swBase + 'qjv/lwvz.config.js')
importScripts(__dde$config.sw || swBase + 'qjv/lwvz.sw.js')

importScripts(swBase + 'zerb/zjzd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$najzxqlu', 1)
			...
			// `openDB('$najzxqlu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cfto/')) {
