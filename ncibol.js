const swBase = self.location.pathname.replace(/ncibol\.js$/, '')

importScripts(swBase + 'ize/merk.bundle.js')
importScripts(swBase + 'ize/merk.config.js')
importScripts(__ijf$config.sw || swBase + 'ize/merk.sw.js')

importScripts(swBase + 'pdme/feoq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ufylgvhl', 1)
			...
			// `openDB('$ufylgvhl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vnpv/')) {
