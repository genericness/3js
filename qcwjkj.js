const swBase = self.location.pathname.replace(/qcwjkj\.js$/, '')

importScripts(swBase + 'bvc/cceb.bundle.js')
importScripts(swBase + 'bvc/cceb.config.js')
importScripts(__nzn$config.sw || swBase + 'bvc/cceb.sw.js')

importScripts(swBase + 'axtd/yssw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$twwfjguf', 1)
			...
			// `openDB('$twwfjguf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qgsn/')) {
