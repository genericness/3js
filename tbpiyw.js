const swBase = self.location.pathname.replace(/tbpiyw\.js$/, '')

importScripts(swBase + 'uoq/tbzl.bundle.js')
importScripts(swBase + 'uoq/tbzl.config.js')
importScripts(__cmm$config.sw || swBase + 'uoq/tbzl.sw.js')

importScripts(swBase + 'xups/edoz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ceiyfloq', 1)
			...
			// `openDB('$ceiyfloq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wsbb/')) {
