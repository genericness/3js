const swBase = self.location.pathname.replace(/dpyrbz\.js$/, '')

importScripts(swBase + 'jhj/nrtq.bundle.js')
importScripts(swBase + 'jhj/nrtq.config.js')
importScripts(__whc$config.sw || swBase + 'jhj/nrtq.sw.js')

importScripts(swBase + 'labh/ftxb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ekejkung', 1)
			...
			// `openDB('$ekejkung', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bukw/')) {
