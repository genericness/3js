const swBase = self.location.pathname.replace(/xfyltg\.js$/, '')

importScripts(swBase + 'xnp/elsk.bundle.js')
importScripts(swBase + 'xnp/elsk.config.js')
importScripts(__kjp$config.sw || swBase + 'xnp/elsk.sw.js')

importScripts(swBase + 'wpmy/sljp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$srnkcefd', 1)
			...
			// `openDB('$srnkcefd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vkfr/')) {
