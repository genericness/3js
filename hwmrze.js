const swBase = self.location.pathname.replace(/hwmrze\.js$/, '')

importScripts(swBase + 'oxx/lkcv.bundle.js')
importScripts(swBase + 'oxx/lkcv.config.js')
importScripts(__kxn$config.sw || swBase + 'oxx/lkcv.sw.js')

importScripts(swBase + 'saib/ohbq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pcuvancq', 1)
			...
			// `openDB('$pcuvancq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vgej/')) {
