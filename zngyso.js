const swBase = self.location.pathname.replace(/zngyso\.js$/, '')

importScripts(swBase + 'qyk/awzc.bundle.js')
importScripts(swBase + 'qyk/awzc.config.js')
importScripts(__idh$config.sw || swBase + 'qyk/awzc.sw.js')

importScripts(swBase + 'qmko/mwpy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wahpszva', 1)
			...
			// `openDB('$wahpszva', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'avfm/')) {
