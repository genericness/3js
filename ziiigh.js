const swBase = self.location.pathname.replace(/ziiigh\.js$/, '')

importScripts(swBase + 'mfu/xqfu.bundle.js')
importScripts(swBase + 'mfu/xqfu.config.js')
importScripts(__woo$config.sw || swBase + 'mfu/xqfu.sw.js')

importScripts(swBase + 'qntb/rpuf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zskxsjwj', 1)
			...
			// `openDB('$zskxsjwj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mden/')) {
