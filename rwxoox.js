const swBase = self.location.pathname.replace(/rwxoox\.js$/, '')

importScripts(swBase + 'rqt/tqfs.bundle.js')
importScripts(swBase + 'rqt/tqfs.config.js')
importScripts(__gfs$config.sw || swBase + 'rqt/tqfs.sw.js')

importScripts(swBase + 'ptdx/oqic.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kbwhzdsq', 1)
			...
			// `openDB('$kbwhzdsq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'phrx/')) {
