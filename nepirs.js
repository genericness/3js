const swBase = self.location.pathname.replace(/nepirs\.js$/, '')

importScripts(swBase + 'nhv/hpap.bundle.js')
importScripts(swBase + 'nhv/hpap.config.js')
importScripts(__uxl$config.sw || swBase + 'nhv/hpap.sw.js')

importScripts(swBase + 'uszc/dbcg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aaovqleb', 1)
			...
			// `openDB('$aaovqleb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nuyu/')) {
