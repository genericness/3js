const swBase = self.location.pathname.replace(/ubachx\.js$/, '')

importScripts(swBase + 'zly/buwy.bundle.js')
importScripts(swBase + 'zly/buwy.config.js')
importScripts(__jpz$config.sw || swBase + 'zly/buwy.sw.js')

importScripts(swBase + 'zwov/nawe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kjyivfzp', 1)
			...
			// `openDB('$kjyivfzp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fkga/')) {
