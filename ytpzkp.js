const swBase = self.location.pathname.replace(/ytpzkp\.js$/, '')

importScripts(swBase + 'efl/rlbi.bundle.js')
importScripts(swBase + 'efl/rlbi.config.js')
importScripts(__cph$config.sw || swBase + 'efl/rlbi.sw.js')

importScripts(swBase + 'dspx/outj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$swfuqaha', 1)
			...
			// `openDB('$swfuqaha', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jgsp/')) {
