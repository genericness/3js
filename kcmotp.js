const swBase = self.location.pathname.replace(/kcmotp\.js$/, '')

importScripts(swBase + 'vaw/uukz.bundle.js')
importScripts(swBase + 'vaw/uukz.config.js')
importScripts(__sdi$config.sw || swBase + 'vaw/uukz.sw.js')

importScripts(swBase + 'uwse/vceu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hfzivztr', 1)
			...
			// `openDB('$hfzivztr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mvqg/')) {
