const swBase = self.location.pathname.replace(/vddncf\.js$/, '')

importScripts(swBase + 'vts/zqrf.bundle.js')
importScripts(swBase + 'vts/zqrf.config.js')
importScripts(__hnq$config.sw || swBase + 'vts/zqrf.sw.js')

importScripts(swBase + 'syzq/wvzq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tgkhvdmj', 1)
			...
			// `openDB('$tgkhvdmj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hked/')) {
