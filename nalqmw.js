const swBase = self.location.pathname.replace(/nalqmw\.js$/, '')

importScripts(swBase + 'wnl/mgbm.bundle.js')
importScripts(swBase + 'wnl/mgbm.config.js')
importScripts(__ncg$config.sw || swBase + 'wnl/mgbm.sw.js')

importScripts(swBase + 'pwzf/gggf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wppuhser', 1)
			...
			// `openDB('$wppuhser', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'atnc/')) {
