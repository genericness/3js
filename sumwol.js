const swBase = self.location.pathname.replace(/sumwol\.js$/, '')

importScripts(swBase + 'fom/qkhu.bundle.js')
importScripts(swBase + 'fom/qkhu.config.js')
importScripts(__eku$config.sw || swBase + 'fom/qkhu.sw.js')

importScripts(swBase + 'ajvm/aemp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$snvjosbe', 1)
			...
			// `openDB('$snvjosbe', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wnub/')) {
