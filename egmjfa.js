const swBase = self.location.pathname.replace(/egmjfa\.js$/, '')

importScripts(swBase + 'tdo/rxuj.bundle.js')
importScripts(swBase + 'tdo/rxuj.config.js')
importScripts(__blk$config.sw || swBase + 'tdo/rxuj.sw.js')

importScripts(swBase + 'wjvc/wqnv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$uatrdgfd', 1)
			...
			// `openDB('$uatrdgfd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zkft/')) {
