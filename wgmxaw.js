const swBase = self.location.pathname.replace(/wgmxaw\.js$/, '')

importScripts(swBase + 'ggi/ogeq.bundle.js')
importScripts(swBase + 'ggi/ogeq.config.js')
importScripts(__tap$config.sw || swBase + 'ggi/ogeq.sw.js')

importScripts(swBase + 'nlrw/arfw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bxgnvpue', 1)
			...
			// `openDB('$bxgnvpue', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rijs/')) {
