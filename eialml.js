const swBase = self.location.pathname.replace(/eialml\.js$/, '')

importScripts(swBase + 'cab/sxom.bundle.js')
importScripts(swBase + 'cab/sxom.config.js')
importScripts(__hww$config.sw || swBase + 'cab/sxom.sw.js')

importScripts(swBase + 'qene/dxsc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$eajagafv', 1)
			...
			// `openDB('$eajagafv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fbfu/')) {
