const swBase = self.location.pathname.replace(/riirsv\.js$/, '')

importScripts(swBase + 'kqv/faid.bundle.js')
importScripts(swBase + 'kqv/faid.config.js')
importScripts(__jsx$config.sw || swBase + 'kqv/faid.sw.js')

importScripts(swBase + 'kaml/aodi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aqbpdbgc', 1)
			...
			// `openDB('$aqbpdbgc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pgqb/')) {
