const swBase = self.location.pathname.replace(/iyefgu\.js$/, '')

importScripts(swBase + 'kym/hirj.bundle.js')
importScripts(swBase + 'kym/hirj.config.js')
importScripts(__coa$config.sw || swBase + 'kym/hirj.sw.js')

importScripts(swBase + 'ckdg/fprl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$twzsfglb', 1)
			...
			// `openDB('$twzsfglb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'iyvv/')) {
