const swBase = self.location.pathname.replace(/jfwtsn\.js$/, '')

importScripts(swBase + 'pio/bafv.bundle.js')
importScripts(swBase + 'pio/bafv.config.js')
importScripts(__gor$config.sw || swBase + 'pio/bafv.sw.js')

importScripts(swBase + 'ngsf/tukp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dmedjgnw', 1)
			...
			// `openDB('$dmedjgnw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'odrs/')) {
