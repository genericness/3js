const swBase = self.location.pathname.replace(/lnsqkf\.js$/, '')

importScripts(swBase + 'sdl/thpx.bundle.js')
importScripts(swBase + 'sdl/thpx.config.js')
importScripts(__hyb$config.sw || swBase + 'sdl/thpx.sw.js')

importScripts(swBase + 'ustz/qeao.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$amvrsskv', 1)
			...
			// `openDB('$amvrsskv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kldd/')) {
