const swBase = self.location.pathname.replace(/yevtbt\.js$/, '')

importScripts(swBase + 'ira/qcci.bundle.js')
importScripts(swBase + 'ira/qcci.config.js')
importScripts(__ptp$config.sw || swBase + 'ira/qcci.sw.js')

importScripts(swBase + 'wbwt/pdkp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rtgufncz', 1)
			...
			// `openDB('$rtgufncz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'iuox/')) {
