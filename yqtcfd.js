const swBase = self.location.pathname.replace(/yqtcfd\.js$/, '')

importScripts(swBase + 'odr/diul.bundle.js')
importScripts(swBase + 'odr/diul.config.js')
importScripts(__skx$config.sw || swBase + 'odr/diul.sw.js')

importScripts(swBase + 'akzl/uwks.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vkmsjuaf', 1)
			...
			// `openDB('$vkmsjuaf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dihe/')) {
