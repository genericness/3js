const swBase = self.location.pathname.replace(/cylcpm\.js$/, '')

importScripts(swBase + 'oas/ooxm.bundle.js')
importScripts(swBase + 'oas/ooxm.config.js')
importScripts(__szj$config.sw || swBase + 'oas/ooxm.sw.js')

importScripts(swBase + 'icdx/mmha.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hwwuuvij', 1)
			...
			// `openDB('$hwwuuvij', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'juzq/')) {
