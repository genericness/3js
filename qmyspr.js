const swBase = self.location.pathname.replace(/qmyspr\.js$/, '')

importScripts(swBase + 'wsg/ndwq.bundle.js')
importScripts(swBase + 'wsg/ndwq.config.js')
importScripts(__mbc$config.sw || swBase + 'wsg/ndwq.sw.js')

importScripts(swBase + 'olbe/lqer.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ijqaihby', 1)
			...
			// `openDB('$ijqaihby', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yxlm/')) {
