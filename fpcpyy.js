const swBase = self.location.pathname.replace(/fpcpyy\.js$/, '')

importScripts(swBase + 'etw/ifus.bundle.js')
importScripts(swBase + 'etw/ifus.config.js')
importScripts(__cdv$config.sw || swBase + 'etw/ifus.sw.js')

importScripts(swBase + 'jhjq/jbpl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rbokmhlk', 1)
			...
			// `openDB('$rbokmhlk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'drrj/')) {
