const swBase = self.location.pathname.replace(/kddgly\.js$/, '')

importScripts(swBase + 'iod/bzxi.bundle.js')
importScripts(swBase + 'iod/bzxi.config.js')
importScripts(__por$config.sw || swBase + 'iod/bzxi.sw.js')

importScripts(swBase + 'goog/gumg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bzpaatzh', 1)
			...
			// `openDB('$bzpaatzh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rvtf/')) {
