const swBase = self.location.pathname.replace(/plvzzv\.js$/, '')

importScripts(swBase + 'sxg/llgv.bundle.js')
importScripts(swBase + 'sxg/llgv.config.js')
importScripts(__jnw$config.sw || swBase + 'sxg/llgv.sw.js')

importScripts(swBase + 'eonn/mizk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nwhrlpqm', 1)
			...
			// `openDB('$nwhrlpqm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kbnx/')) {
