const swBase = self.location.pathname.replace(/auwwlr\.js$/, '')

importScripts(swBase + 'mos/ybhv.bundle.js')
importScripts(swBase + 'mos/ybhv.config.js')
importScripts(__neb$config.sw || swBase + 'mos/ybhv.sw.js')

importScripts(swBase + 'ecyo/rycf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rnrchqun', 1)
			...
			// `openDB('$rnrchqun', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qftl/')) {
