const swBase = self.location.pathname.replace(/gditax\.js$/, '')

importScripts(swBase + 'duy/oqyk.bundle.js')
importScripts(swBase + 'duy/oqyk.config.js')
importScripts(__bew$config.sw || swBase + 'duy/oqyk.sw.js')

importScripts(swBase + 'skyg/plsa.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ofqryeep', 1)
			...
			// `openDB('$ofqryeep', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jxia/')) {
