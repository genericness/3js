const swBase = self.location.pathname.replace(/gqvbts\.js$/, '')

importScripts(swBase + 'yyq/jevt.bundle.js')
importScripts(swBase + 'yyq/jevt.config.js')
importScripts(__lsb$config.sw || swBase + 'yyq/jevt.sw.js')

importScripts(swBase + 'novh/tmss.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zkhcylwk', 1)
			...
			// `openDB('$zkhcylwk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jzem/')) {
