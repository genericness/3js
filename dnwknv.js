const swBase = self.location.pathname.replace(/dnwknv\.js$/, '')

importScripts(swBase + 'wom/rist.bundle.js')
importScripts(swBase + 'wom/rist.config.js')
importScripts(__ldv$config.sw || swBase + 'wom/rist.sw.js')

importScripts(swBase + 'lslf/rkkw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jyzpodca', 1)
			...
			// `openDB('$jyzpodca', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'klmv/')) {
