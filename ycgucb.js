const swBase = self.location.pathname.replace(/ycgucb\.js$/, '')

importScripts(swBase + 'lng/kcfr.bundle.js')
importScripts(swBase + 'lng/kcfr.config.js')
importScripts(__cvf$config.sw || swBase + 'lng/kcfr.sw.js')

importScripts(swBase + 'wasu/yxbh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$atigsfpq', 1)
			...
			// `openDB('$atigsfpq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kbzs/')) {
