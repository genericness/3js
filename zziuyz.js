const swBase = self.location.pathname.replace(/zziuyz\.js$/, '')

importScripts(swBase + 'wrf/hjfh.bundle.js')
importScripts(swBase + 'wrf/hjfh.config.js')
importScripts(__aeh$config.sw || swBase + 'wrf/hjfh.sw.js')

importScripts(swBase + 'okav/frnf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$unbblvuw', 1)
			...
			// `openDB('$unbblvuw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ogpf/')) {
