const swBase = self.location.pathname.replace(/gqtbyy\.js$/, '')

importScripts(swBase + 'ois/kfzq.bundle.js')
importScripts(swBase + 'ois/kfzq.config.js')
importScripts(__wxo$config.sw || swBase + 'ois/kfzq.sw.js')

importScripts(swBase + 'gofj/xqyn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bkscvlxt', 1)
			...
			// `openDB('$bkscvlxt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'itaa/')) {
