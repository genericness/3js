const swBase = self.location.pathname.replace(/sugiii\.js$/, '')

importScripts(swBase + 'sfs/rvbb.bundle.js')
importScripts(swBase + 'sfs/rvbb.config.js')
importScripts(__ocu$config.sw || swBase + 'sfs/rvbb.sw.js')

importScripts(swBase + 'ulms/fedk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bbihgvks', 1)
			...
			// `openDB('$bbihgvks', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nllu/')) {
