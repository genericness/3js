const swBase = self.location.pathname.replace(/pjjovw\.js$/, '')

importScripts(swBase + 'tpv/sbqv.bundle.js')
importScripts(swBase + 'tpv/sbqv.config.js')
importScripts(__mqn$config.sw || swBase + 'tpv/sbqv.sw.js')

importScripts(swBase + 'bpdy/rjdj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$urjwuuwy', 1)
			...
			// `openDB('$urjwuuwy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zqfd/')) {
