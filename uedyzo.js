const swBase = self.location.pathname.replace(/uedyzo\.js$/, '')

importScripts(swBase + 'xae/iyuk.bundle.js')
importScripts(swBase + 'xae/iyuk.config.js')
importScripts(__ldp$config.sw || swBase + 'xae/iyuk.sw.js')

importScripts(swBase + 'onca/szei.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xgzcdwla', 1)
			...
			// `openDB('$xgzcdwla', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fygz/')) {
