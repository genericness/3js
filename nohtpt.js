const swBase = self.location.pathname.replace(/nohtpt\.js$/, '')

importScripts(swBase + 'jhh/alfr.bundle.js')
importScripts(swBase + 'jhh/alfr.config.js')
importScripts(__ass$config.sw || swBase + 'jhh/alfr.sw.js')

importScripts(swBase + 'hpyx/bttm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hxyuebwj', 1)
			...
			// `openDB('$hxyuebwj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zxsm/')) {
