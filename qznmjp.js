const swBase = self.location.pathname.replace(/qznmjp\.js$/, '')

importScripts(swBase + 'wet/njcv.bundle.js')
importScripts(swBase + 'wet/njcv.config.js')
importScripts(__pcq$config.sw || swBase + 'wet/njcv.sw.js')

importScripts(swBase + 'pnue/njzc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ssolglzs', 1)
			...
			// `openDB('$ssolglzs', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lhsg/')) {
