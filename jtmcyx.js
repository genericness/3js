const swBase = self.location.pathname.replace(/jtmcyx\.js$/, '')

importScripts(swBase + 'qhh/bvsy.bundle.js')
importScripts(swBase + 'qhh/bvsy.config.js')
importScripts(__fty$config.sw || swBase + 'qhh/bvsy.sw.js')

importScripts(swBase + 'anjp/vhni.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kblwdtna', 1)
			...
			// `openDB('$kblwdtna', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rqjf/')) {
