const swBase = self.location.pathname.replace(/zhjrno\.js$/, '')

importScripts(swBase + 'dod/xabv.bundle.js')
importScripts(swBase + 'dod/xabv.config.js')
importScripts(__tjf$config.sw || swBase + 'dod/xabv.sw.js')

importScripts(swBase + 'ywcz/ymwn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$esershtq', 1)
			...
			// `openDB('$esershtq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tusn/')) {
