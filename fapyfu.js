const swBase = self.location.pathname.replace(/fapyfu\.js$/, '')

importScripts(swBase + 'wef/bhju.bundle.js')
importScripts(swBase + 'wef/bhju.config.js')
importScripts(__jjw$config.sw || swBase + 'wef/bhju.sw.js')

importScripts(swBase + 'voio/bzfs.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cmcpedem', 1)
			...
			// `openDB('$cmcpedem', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'upph/')) {
