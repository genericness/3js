const swBase = self.location.pathname.replace(/hyqwyc\.js$/, '')

importScripts(swBase + 'jbs/pgjq.bundle.js')
importScripts(swBase + 'jbs/pgjq.config.js')
importScripts(__dzg$config.sw || swBase + 'jbs/pgjq.sw.js')

importScripts(swBase + 'kjqq/fymy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kzlhxfgg', 1)
			...
			// `openDB('$kzlhxfgg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ylxf/')) {
