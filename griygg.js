const swBase = self.location.pathname.replace(/griygg\.js$/, '')

importScripts(swBase + 'lrm/nhax.bundle.js')
importScripts(swBase + 'lrm/nhax.config.js')
importScripts(__kqs$config.sw || swBase + 'lrm/nhax.sw.js')

importScripts(swBase + 'fyxa/bjti.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pfpkykip', 1)
			...
			// `openDB('$pfpkykip', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pcad/')) {
