const swBase = self.location.pathname.replace(/nyqcus\.js$/, '')

importScripts(swBase + 'xxj/hkfx.bundle.js')
importScripts(swBase + 'xxj/hkfx.config.js')
importScripts(__pph$config.sw || swBase + 'xxj/hkfx.sw.js')

importScripts(swBase + 'hlcl/mcnj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$khsxkqey', 1)
			...
			// `openDB('$khsxkqey', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'npto/')) {
