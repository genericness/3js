const swBase = self.location.pathname.replace(/cgspqi\.js$/, '')

importScripts(swBase + 'fbp/oyva.bundle.js')
importScripts(swBase + 'fbp/oyva.config.js')
importScripts(__wsc$config.sw || swBase + 'fbp/oyva.sw.js')

importScripts(swBase + 'mzfv/fwqg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pdlzcqgh', 1)
			...
			// `openDB('$pdlzcqgh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ktey/')) {
