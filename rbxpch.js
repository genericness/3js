const swBase = self.location.pathname.replace(/rbxpch\.js$/, '')

importScripts(swBase + 'qii/gyqa.bundle.js')
importScripts(swBase + 'qii/gyqa.config.js')
importScripts(__iyq$config.sw || swBase + 'qii/gyqa.sw.js')

importScripts(swBase + 'dszp/znux.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qbzehewq', 1)
			...
			// `openDB('$qbzehewq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mavq/')) {
