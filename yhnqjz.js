const swBase = self.location.pathname.replace(/yhnqjz\.js$/, '')

importScripts(swBase + 'mra/liqi.bundle.js')
importScripts(swBase + 'mra/liqi.config.js')
importScripts(__mme$config.sw || swBase + 'mra/liqi.sw.js')

importScripts(swBase + 'oxiv/xqky.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rxpjtxoj', 1)
			...
			// `openDB('$rxpjtxoj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mqlg/')) {
