const swBase = self.location.pathname.replace(/cmqxvn\.js$/, '')

importScripts(swBase + 'wqb/wxir.bundle.js')
importScripts(swBase + 'wqb/wxir.config.js')
importScripts(__nff$config.sw || swBase + 'wqb/wxir.sw.js')

importScripts(swBase + 'nuty/wzut.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ekllpxxo', 1)
			...
			// `openDB('$ekllpxxo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xetp/')) {
