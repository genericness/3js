const swBase = self.location.pathname.replace(/fimepb\.js$/, '')

importScripts(swBase + 'xnj/cubv.bundle.js')
importScripts(swBase + 'xnj/cubv.config.js')
importScripts(__kjf$config.sw || swBase + 'xnj/cubv.sw.js')

importScripts(swBase + 'msmt/ofgy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nfbxfaxj', 1)
			...
			// `openDB('$nfbxfaxj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ftbu/')) {
