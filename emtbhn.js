const swBase = self.location.pathname.replace(/emtbhn\.js$/, '')

importScripts(swBase + 'rkq/jsdc.bundle.js')
importScripts(swBase + 'rkq/jsdc.config.js')
importScripts(__xbl$config.sw || swBase + 'rkq/jsdc.sw.js')

importScripts(swBase + 'chan/vuxo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cpmwqoeo', 1)
			...
			// `openDB('$cpmwqoeo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'epxt/')) {
