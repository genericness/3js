const swBase = self.location.pathname.replace(/apcqdq\.js$/, '')

importScripts(swBase + 'acr/vbcj.bundle.js')
importScripts(swBase + 'acr/vbcj.config.js')
importScripts(__bnn$config.sw || swBase + 'acr/vbcj.sw.js')

importScripts(swBase + 'wakk/dafj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nxbkzxht', 1)
			...
			// `openDB('$nxbkzxht', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dhad/')) {
