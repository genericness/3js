const swBase = self.location.pathname.replace(/oiefbe\.js$/, '')

importScripts(swBase + 'bnm/czqv.bundle.js')
importScripts(swBase + 'bnm/czqv.config.js')
importScripts(__xdt$config.sw || swBase + 'bnm/czqv.sw.js')

importScripts(swBase + 'xocq/haol.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gmpiaroe', 1)
			...
			// `openDB('$gmpiaroe', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yaxw/')) {
