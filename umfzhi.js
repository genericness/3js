const swBase = self.location.pathname.replace(/umfzhi\.js$/, '')

importScripts(swBase + 'iqh/ajla.bundle.js')
importScripts(swBase + 'iqh/ajla.config.js')
importScripts(__mba$config.sw || swBase + 'iqh/ajla.sw.js')

importScripts(swBase + 'wbdi/imcr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mlhspbpi', 1)
			...
			// `openDB('$mlhspbpi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tzkv/')) {
