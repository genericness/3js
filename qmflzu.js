const swBase = self.location.pathname.replace(/qmflzu\.js$/, '')

importScripts(swBase + 'gsz/gnqc.bundle.js')
importScripts(swBase + 'gsz/gnqc.config.js')
importScripts(__lxf$config.sw || swBase + 'gsz/gnqc.sw.js')

importScripts(swBase + 'igzo/hcqs.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nibmomow', 1)
			...
			// `openDB('$nibmomow', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rjhd/')) {
