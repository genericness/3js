const swBase = self.location.pathname.replace(/uneudn\.js$/, '')

importScripts(swBase + 'wxh/ktcr.bundle.js')
importScripts(swBase + 'wxh/ktcr.config.js')
importScripts(__brp$config.sw || swBase + 'wxh/ktcr.sw.js')

importScripts(swBase + 'zsxx/rkrm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$boivuyzr', 1)
			...
			// `openDB('$boivuyzr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lkhe/')) {
