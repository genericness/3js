const swBase = self.location.pathname.replace(/ajhayr\.js$/, '')

importScripts(swBase + 'yic/bljy.bundle.js')
importScripts(swBase + 'yic/bljy.config.js')
importScripts(__anj$config.sw || swBase + 'yic/bljy.sw.js')

importScripts(swBase + 'myop/gixn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xgbpcrou', 1)
			...
			// `openDB('$xgbpcrou', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'eopb/')) {
