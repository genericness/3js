const swBase = self.location.pathname.replace(/shouhf\.js$/, '')

importScripts(swBase + 'lew/sbij.bundle.js')
importScripts(swBase + 'lew/sbij.config.js')
importScripts(__fwp$config.sw || swBase + 'lew/sbij.sw.js')

importScripts(swBase + 'rjsg/peja.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ruhmhgkv', 1)
			...
			// `openDB('$ruhmhgkv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vqwa/')) {
