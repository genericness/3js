const swBase = self.location.pathname.replace(/jbptxm\.js$/, '')

importScripts(swBase + 'wjf/rqwh.bundle.js')
importScripts(swBase + 'wjf/rqwh.config.js')
importScripts(__eht$config.sw || swBase + 'wjf/rqwh.sw.js')

importScripts(swBase + 'owuq/hrgt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mtkedxox', 1)
			...
			// `openDB('$mtkedxox', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cxjn/')) {
