const swBase = self.location.pathname.replace(/fkpmdi\.js$/, '')

importScripts(swBase + 'vfa/fcjf.bundle.js')
importScripts(swBase + 'vfa/fcjf.config.js')
importScripts(__sub$config.sw || swBase + 'vfa/fcjf.sw.js')

importScripts(swBase + 'fcml/jzeb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mlvxhfqu', 1)
			...
			// `openDB('$mlvxhfqu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tvfg/')) {
