const swBase = self.location.pathname.replace(/gbztkj\.js$/, '')

importScripts(swBase + 'iaf/vgum.bundle.js')
importScripts(swBase + 'iaf/vgum.config.js')
importScripts(__acu$config.sw || swBase + 'iaf/vgum.sw.js')

importScripts(swBase + 'algf/uikm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gxmkedqm', 1)
			...
			// `openDB('$gxmkedqm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ndis/')) {
