const swBase = self.location.pathname.replace(/bxfygc\.js$/, '')

importScripts(swBase + 'mtp/utsh.bundle.js')
importScripts(swBase + 'mtp/utsh.config.js')
importScripts(__vfb$config.sw || swBase + 'mtp/utsh.sw.js')

importScripts(swBase + 'icqp/aoai.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vexgieez', 1)
			...
			// `openDB('$vexgieez', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zzwk/')) {
