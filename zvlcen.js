const swBase = self.location.pathname.replace(/zvlcen\.js$/, '')

importScripts(swBase + 'hkq/utse.bundle.js')
importScripts(swBase + 'hkq/utse.config.js')
importScripts(__xls$config.sw || swBase + 'hkq/utse.sw.js')

importScripts(swBase + 'tojh/bpte.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$maypoeue', 1)
			...
			// `openDB('$maypoeue', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uwtu/')) {
