const swBase = self.location.pathname.replace(/donhfe\.js$/, '')

importScripts(swBase + 'ifq/xzrd.bundle.js')
importScripts(swBase + 'ifq/xzrd.config.js')
importScripts(__gbg$config.sw || swBase + 'ifq/xzrd.sw.js')

importScripts(swBase + 'iiix/cvtf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kobpitnb', 1)
			...
			// `openDB('$kobpitnb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bwnn/')) {
