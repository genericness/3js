const swBase = self.location.pathname.replace(/ncheqv\.js$/, '')

importScripts(swBase + 'nbo/pccu.bundle.js')
importScripts(swBase + 'nbo/pccu.config.js')
importScripts(__dvr$config.sw || swBase + 'nbo/pccu.sw.js')

importScripts(swBase + 'axiv/xgng.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ixjwbplm', 1)
			...
			// `openDB('$ixjwbplm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xbxe/')) {
