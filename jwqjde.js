const swBase = self.location.pathname.replace(/jwqjde\.js$/, '')

importScripts(swBase + 'jik/iiqv.bundle.js')
importScripts(swBase + 'jik/iiqv.config.js')
importScripts(__qkb$config.sw || swBase + 'jik/iiqv.sw.js')

importScripts(swBase + 'ayfm/bvex.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yufjcpqv', 1)
			...
			// `openDB('$yufjcpqv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bbks/')) {
