const swBase = self.location.pathname.replace(/ugulvq\.js$/, '')

importScripts(swBase + 'mcs/erqn.bundle.js')
importScripts(swBase + 'mcs/erqn.config.js')
importScripts(__azy$config.sw || swBase + 'mcs/erqn.sw.js')

importScripts(swBase + 'cknh/lxgi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$degdotcr', 1)
			...
			// `openDB('$degdotcr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tokn/')) {
