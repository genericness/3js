const swBase = self.location.pathname.replace(/chbgnd\.js$/, '')

importScripts(swBase + 'wov/sfup.bundle.js')
importScripts(swBase + 'wov/sfup.config.js')
importScripts(__xvl$config.sw || swBase + 'wov/sfup.sw.js')

importScripts(swBase + 'ebjp/hzby.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ovjarqbc', 1)
			...
			// `openDB('$ovjarqbc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bydz/')) {
