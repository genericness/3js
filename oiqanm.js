const swBase = self.location.pathname.replace(/oiqanm\.js$/, '')

importScripts(swBase + 'rgn/tpdg.bundle.js')
importScripts(swBase + 'rgn/tpdg.config.js')
importScripts(__wyf$config.sw || swBase + 'rgn/tpdg.sw.js')

importScripts(swBase + 'jvvb/lenz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lefcklwn', 1)
			...
			// `openDB('$lefcklwn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uhef/')) {
