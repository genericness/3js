const swBase = self.location.pathname.replace(/gsvgur\.js$/, '')

importScripts(swBase + 'tsb/nenw.bundle.js')
importScripts(swBase + 'tsb/nenw.config.js')
importScripts(__fqu$config.sw || swBase + 'tsb/nenw.sw.js')

importScripts(swBase + 'iarf/ahmp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kbjjlkpx', 1)
			...
			// `openDB('$kbjjlkpx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dqcn/')) {
