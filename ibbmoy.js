const swBase = self.location.pathname.replace(/ibbmoy\.js$/, '')

importScripts(swBase + 'qzj/fhre.bundle.js')
importScripts(swBase + 'qzj/fhre.config.js')
importScripts(__ljr$config.sw || swBase + 'qzj/fhre.sw.js')

importScripts(swBase + 'wnff/mgmw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$betrskuj', 1)
			...
			// `openDB('$betrskuj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pbxm/')) {
