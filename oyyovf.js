const swBase = self.location.pathname.replace(/oyyovf\.js$/, '')

importScripts(swBase + 'tbv/bdvd.bundle.js')
importScripts(swBase + 'tbv/bdvd.config.js')
importScripts(__dha$config.sw || swBase + 'tbv/bdvd.sw.js')

importScripts(swBase + 'hzve/rmqe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mtozqyfg', 1)
			...
			// `openDB('$mtozqyfg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yyxb/')) {
