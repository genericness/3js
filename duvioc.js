const swBase = self.location.pathname.replace(/duvioc\.js$/, '')

importScripts(swBase + 'rux/lnzz.bundle.js')
importScripts(swBase + 'rux/lnzz.config.js')
importScripts(__iuk$config.sw || swBase + 'rux/lnzz.sw.js')

importScripts(swBase + 'skyv/jtwr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wndrcglp', 1)
			...
			// `openDB('$wndrcglp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jync/')) {
