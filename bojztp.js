const swBase = self.location.pathname.replace(/bojztp\.js$/, '')

importScripts(swBase + 'abr/zudz.bundle.js')
importScripts(swBase + 'abr/zudz.config.js')
importScripts(__hut$config.sw || swBase + 'abr/zudz.sw.js')

importScripts(swBase + 'xyyy/knpb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wqqzyabs', 1)
			...
			// `openDB('$wqqzyabs', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'iwbv/')) {
