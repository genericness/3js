const swBase = self.location.pathname.replace(/uoipnf\.js$/, '')

importScripts(swBase + 'jbb/divk.bundle.js')
importScripts(swBase + 'jbb/divk.config.js')
importScripts(__frn$config.sw || swBase + 'jbb/divk.sw.js')

importScripts(swBase + 'irmz/qlxy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kahywild', 1)
			...
			// `openDB('$kahywild', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tsja/')) {
