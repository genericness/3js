const swBase = self.location.pathname.replace(/klshra\.js$/, '')

importScripts(swBase + 'tne/oeep.bundle.js')
importScripts(swBase + 'tne/oeep.config.js')
importScripts(__yux$config.sw || swBase + 'tne/oeep.sw.js')

importScripts(swBase + 'ywsu/pxrr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zxgkhomg', 1)
			...
			// `openDB('$zxgkhomg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'imbc/')) {
