const swBase = self.location.pathname.replace(/ahdznt\.js$/, '')

importScripts(swBase + 'kqa/smvb.bundle.js')
importScripts(swBase + 'kqa/smvb.config.js')
importScripts(__zkn$config.sw || swBase + 'kqa/smvb.sw.js')

importScripts(swBase + 'mfdm/ebtf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ciuzbawa', 1)
			...
			// `openDB('$ciuzbawa', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dovb/')) {
