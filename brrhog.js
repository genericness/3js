const swBase = self.location.pathname.replace(/brrhog\.js$/, '')

importScripts(swBase + 'izv/cacb.bundle.js')
importScripts(swBase + 'izv/cacb.config.js')
importScripts(__jsd$config.sw || swBase + 'izv/cacb.sw.js')

importScripts(swBase + 'ftna/fybg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qedkoxza', 1)
			...
			// `openDB('$qedkoxza', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tyre/')) {
