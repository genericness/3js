const swBase = self.location.pathname.replace(/kifhfh\.js$/, '')

importScripts(swBase + 'nca/iyoi.bundle.js')
importScripts(swBase + 'nca/iyoi.config.js')
importScripts(__oyg$config.sw || swBase + 'nca/iyoi.sw.js')

importScripts(swBase + 'upcw/ediu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$daceuxuf', 1)
			...
			// `openDB('$daceuxuf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'oszw/')) {
