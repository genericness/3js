const swBase = self.location.pathname.replace(/zigypg\.js$/, '')

importScripts(swBase + 'aak/kkyd.bundle.js')
importScripts(swBase + 'aak/kkyd.config.js')
importScripts(__cxf$config.sw || swBase + 'aak/kkyd.sw.js')

importScripts(swBase + 'fbqn/horn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yxltwmwa', 1)
			...
			// `openDB('$yxltwmwa', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kjne/')) {
