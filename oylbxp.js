const swBase = self.location.pathname.replace(/oylbxp\.js$/, '')

importScripts(swBase + 'tlr/fejn.bundle.js')
importScripts(swBase + 'tlr/fejn.config.js')
importScripts(__rxu$config.sw || swBase + 'tlr/fejn.sw.js')

importScripts(swBase + 'othg/sgzr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xworzgfj', 1)
			...
			// `openDB('$xworzgfj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kwtw/')) {
