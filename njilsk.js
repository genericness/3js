const swBase = self.location.pathname.replace(/njilsk\.js$/, '')

importScripts(swBase + 'dak/udlc.bundle.js')
importScripts(swBase + 'dak/udlc.config.js')
importScripts(__del$config.sw || swBase + 'dak/udlc.sw.js')

importScripts(swBase + 'hwvw/okcy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kpdyvsxv', 1)
			...
			// `openDB('$kpdyvsxv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ivnh/')) {
