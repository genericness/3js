const swBase = self.location.pathname.replace(/xvplzy\.js$/, '')

importScripts(swBase + 'lod/wuai.bundle.js')
importScripts(swBase + 'lod/wuai.config.js')
importScripts(__ykq$config.sw || swBase + 'lod/wuai.sw.js')

importScripts(swBase + 'bbum/rvcf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$juxcxvly', 1)
			...
			// `openDB('$juxcxvly', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dydl/')) {
