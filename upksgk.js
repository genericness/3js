const swBase = self.location.pathname.replace(/upksgk\.js$/, '')

importScripts(swBase + 'sqb/xhlc.bundle.js')
importScripts(swBase + 'sqb/xhlc.config.js')
importScripts(__hak$config.sw || swBase + 'sqb/xhlc.sw.js')

importScripts(swBase + 'iqul/xvth.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nwtmldvl', 1)
			...
			// `openDB('$nwtmldvl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ftpv/')) {
