const swBase = self.location.pathname.replace(/cesqub\.js$/, '')

importScripts(swBase + 'tei/azoq.bundle.js')
importScripts(swBase + 'tei/azoq.config.js')
importScripts(__nas$config.sw || swBase + 'tei/azoq.sw.js')

importScripts(swBase + 'qcpt/tdrm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ymxvhxce', 1)
			...
			// `openDB('$ymxvhxce', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dkwp/')) {
