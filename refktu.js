const swBase = self.location.pathname.replace(/refktu\.js$/, '')

importScripts(swBase + 'afp/ohuc.bundle.js')
importScripts(swBase + 'afp/ohuc.config.js')
importScripts(__dbs$config.sw || swBase + 'afp/ohuc.sw.js')

importScripts(swBase + 'onms/eqxz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pzvrmcfj', 1)
			...
			// `openDB('$pzvrmcfj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mrhm/')) {
