const swBase = self.location.pathname.replace(/ygfzyn\.js$/, '')

importScripts(swBase + 'ini/eikb.bundle.js')
importScripts(swBase + 'ini/eikb.config.js')
importScripts(__gda$config.sw || swBase + 'ini/eikb.sw.js')

importScripts(swBase + 'qqqd/aajz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cgesjard', 1)
			...
			// `openDB('$cgesjard', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ooun/')) {
