const swBase = self.location.pathname.replace(/bukinz\.js$/, '')

importScripts(swBase + 'gic/hbze.bundle.js')
importScripts(swBase + 'gic/hbze.config.js')
importScripts(__bhe$config.sw || swBase + 'gic/hbze.sw.js')

importScripts(swBase + 'tvie/txdj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xrxplijd', 1)
			...
			// `openDB('$xrxplijd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vqph/')) {
