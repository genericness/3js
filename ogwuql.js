const swBase = self.location.pathname.replace(/ogwuql\.js$/, '')

importScripts(swBase + 'gvs/frwn.bundle.js')
importScripts(swBase + 'gvs/frwn.config.js')
importScripts(__kux$config.sw || swBase + 'gvs/frwn.sw.js')

importScripts(swBase + 'ybpg/tizd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$coczykhq', 1)
			...
			// `openDB('$coczykhq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cpfg/')) {
