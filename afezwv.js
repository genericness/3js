const swBase = self.location.pathname.replace(/afezwv\.js$/, '')

importScripts(swBase + 'lrb/nudx.bundle.js')
importScripts(swBase + 'lrb/nudx.config.js')
importScripts(__mmf$config.sw || swBase + 'lrb/nudx.sw.js')

importScripts(swBase + 'csnn/tknw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hznkhudd', 1)
			...
			// `openDB('$hznkhudd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ghqc/')) {
