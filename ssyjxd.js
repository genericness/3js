const swBase = self.location.pathname.replace(/ssyjxd\.js$/, '')

importScripts(swBase + 'lpa/olvp.bundle.js')
importScripts(swBase + 'lpa/olvp.config.js')
importScripts(__qkr$config.sw || swBase + 'lpa/olvp.sw.js')

importScripts(swBase + 'msxn/ycse.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ovvawujw', 1)
			...
			// `openDB('$ovvawujw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zyie/')) {
