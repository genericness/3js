const swBase = self.location.pathname.replace(/rbsatg\.js$/, '')

importScripts(swBase + 'qkc/elay.bundle.js')
importScripts(swBase + 'qkc/elay.config.js')
importScripts(__cwm$config.sw || swBase + 'qkc/elay.sw.js')

importScripts(swBase + 'nset/kout.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qofwjoms', 1)
			...
			// `openDB('$qofwjoms', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lbce/')) {
