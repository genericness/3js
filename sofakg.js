const swBase = self.location.pathname.replace(/sofakg\.js$/, '')

importScripts(swBase + 'tpv/mjzh.bundle.js')
importScripts(swBase + 'tpv/mjzh.config.js')
importScripts(__trw$config.sw || swBase + 'tpv/mjzh.sw.js')

importScripts(swBase + 'ktpq/cgft.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lthpzysg', 1)
			...
			// `openDB('$lthpzysg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kegj/')) {
