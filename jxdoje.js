const swBase = self.location.pathname.replace(/jxdoje\.js$/, '')

importScripts(swBase + 'ymb/mejt.bundle.js')
importScripts(swBase + 'ymb/mejt.config.js')
importScripts(__qxp$config.sw || swBase + 'ymb/mejt.sw.js')

importScripts(swBase + 'waes/hkyw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nmirvisb', 1)
			...
			// `openDB('$nmirvisb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nuih/')) {
