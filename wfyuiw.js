const swBase = self.location.pathname.replace(/wfyuiw\.js$/, '')

importScripts(swBase + 'wyk/sgzm.bundle.js')
importScripts(swBase + 'wyk/sgzm.config.js')
importScripts(__gas$config.sw || swBase + 'wyk/sgzm.sw.js')

importScripts(swBase + 'nbay/ogfc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jnljykpu', 1)
			...
			// `openDB('$jnljykpu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'himb/')) {
