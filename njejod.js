const swBase = self.location.pathname.replace(/njejod\.js$/, '')

importScripts(swBase + 'axe/rqnq.bundle.js')
importScripts(swBase + 'axe/rqnq.config.js')
importScripts(__yic$config.sw || swBase + 'axe/rqnq.sw.js')

importScripts(swBase + 'sdfu/gzwj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$oudgpfpm', 1)
			...
			// `openDB('$oudgpfpm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'chei/')) {
