const swBase = self.location.pathname.replace(/ddkxcp\.js$/, '')

importScripts(swBase + 'uit/jpzt.bundle.js')
importScripts(swBase + 'uit/jpzt.config.js')
importScripts(__qaq$config.sw || swBase + 'uit/jpzt.sw.js')

importScripts(swBase + 'eejm/irvx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kgzxidty', 1)
			...
			// `openDB('$kgzxidty', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sogt/')) {
