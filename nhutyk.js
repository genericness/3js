const swBase = self.location.pathname.replace(/nhutyk\.js$/, '')

importScripts(swBase + 'djs/fbwo.bundle.js')
importScripts(swBase + 'djs/fbwo.config.js')
importScripts(__umh$config.sw || swBase + 'djs/fbwo.sw.js')

importScripts(swBase + 'lgoh/zgvy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$twbxonvc', 1)
			...
			// `openDB('$twbxonvc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sgeb/')) {
