const swBase = self.location.pathname.replace(/xuqfji\.js$/, '')

importScripts(swBase + 'ooe/ewbn.bundle.js')
importScripts(swBase + 'ooe/ewbn.config.js')
importScripts(__xnf$config.sw || swBase + 'ooe/ewbn.sw.js')

importScripts(swBase + 'wisb/nrqq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qpblzqqu', 1)
			...
			// `openDB('$qpblzqqu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qkmf/')) {
