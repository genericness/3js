const swBase = self.location.pathname.replace(/spbjqc\.js$/, '')

importScripts(swBase + 'enr/tpkg.bundle.js')
importScripts(swBase + 'enr/tpkg.config.js')
importScripts(__azf$config.sw || swBase + 'enr/tpkg.sw.js')

importScripts(swBase + 'tjmj/cvaj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$muhhziyl', 1)
			...
			// `openDB('$muhhziyl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'htdv/')) {
