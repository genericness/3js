const swBase = self.location.pathname.replace(/aphyqp\.js$/, '')

importScripts(swBase + 'hgh/bccq.bundle.js')
importScripts(swBase + 'hgh/bccq.config.js')
importScripts(__taf$config.sw || swBase + 'hgh/bccq.sw.js')

importScripts(swBase + 'tcjz/ucge.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jflkalso', 1)
			...
			// `openDB('$jflkalso', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vopq/')) {
