const swBase = self.location.pathname.replace(/ggxtfr\.js$/, '')

importScripts(swBase + 'fyu/pbqv.bundle.js')
importScripts(swBase + 'fyu/pbqv.config.js')
importScripts(__ehs$config.sw || swBase + 'fyu/pbqv.sw.js')

importScripts(swBase + 'srfg/jnyf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$emusugdl', 1)
			...
			// `openDB('$emusugdl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ncml/')) {
