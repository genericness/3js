const swBase = self.location.pathname.replace(/txtxjt\.js$/, '')

importScripts(swBase + 'rss/kotv.bundle.js')
importScripts(swBase + 'rss/kotv.config.js')
importScripts(__dxf$config.sw || swBase + 'rss/kotv.sw.js')

importScripts(swBase + 'zyux/qgfc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$evivbvyn', 1)
			...
			// `openDB('$evivbvyn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ryzr/')) {
