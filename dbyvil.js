const swBase = self.location.pathname.replace(/dbyvil\.js$/, '')

importScripts(swBase + 'oqc/hqqt.bundle.js')
importScripts(swBase + 'oqc/hqqt.config.js')
importScripts(__sfm$config.sw || swBase + 'oqc/hqqt.sw.js')

importScripts(swBase + 'dlsq/cctd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zssdgpxw', 1)
			...
			// `openDB('$zssdgpxw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dhhi/')) {
