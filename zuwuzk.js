const swBase = self.location.pathname.replace(/zuwuzk\.js$/, '')

importScripts(swBase + 'xto/xmwn.bundle.js')
importScripts(swBase + 'xto/xmwn.config.js')
importScripts(__sjq$config.sw || swBase + 'xto/xmwn.sw.js')

importScripts(swBase + 'tydj/oyad.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vbmeskun', 1)
			...
			// `openDB('$vbmeskun', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ldrb/')) {
