const swBase = self.location.pathname.replace(/aronfa\.js$/, '')

importScripts(swBase + 'ofo/lxyv.bundle.js')
importScripts(swBase + 'ofo/lxyv.config.js')
importScripts(__ulz$config.sw || swBase + 'ofo/lxyv.sw.js')

importScripts(swBase + 'sxbu/knyi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$shhwdggo', 1)
			...
			// `openDB('$shhwdggo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hyaz/')) {
