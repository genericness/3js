const swBase = self.location.pathname.replace(/bdzgqa\.js$/, '')

importScripts(swBase + 'cim/scry.bundle.js')
importScripts(swBase + 'cim/scry.config.js')
importScripts(__lbm$config.sw || swBase + 'cim/scry.sw.js')

importScripts(swBase + 'cdkr/qjaj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rimyexan', 1)
			...
			// `openDB('$rimyexan', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ntru/')) {
