const swBase = self.location.pathname.replace(/snstnp\.js$/, '')

importScripts(swBase + 'seh/ztza.bundle.js')
importScripts(swBase + 'seh/ztza.config.js')
importScripts(__siw$config.sw || swBase + 'seh/ztza.sw.js')

importScripts(swBase + 'lmka/snqp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pzefqqra', 1)
			...
			// `openDB('$pzefqqra', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'eyqy/')) {
