const swBase = self.location.pathname.replace(/oezgpr\.js$/, '')

importScripts(swBase + 'rht/vjlp.bundle.js')
importScripts(swBase + 'rht/vjlp.config.js')
importScripts(__spe$config.sw || swBase + 'rht/vjlp.sw.js')

importScripts(swBase + 'stsh/zgbo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jnvhfkhn', 1)
			...
			// `openDB('$jnvhfkhn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'itxe/')) {
