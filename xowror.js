const swBase = self.location.pathname.replace(/xowror\.js$/, '')

importScripts(swBase + 'heq/zxgy.bundle.js')
importScripts(swBase + 'heq/zxgy.config.js')
importScripts(__zic$config.sw || swBase + 'heq/zxgy.sw.js')

importScripts(swBase + 'ewkv/bgac.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gccwpuca', 1)
			...
			// `openDB('$gccwpuca', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hbsv/')) {
