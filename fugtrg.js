const swBase = self.location.pathname.replace(/fugtrg\.js$/, '')

importScripts(swBase + 'cyl/aqdu.bundle.js')
importScripts(swBase + 'cyl/aqdu.config.js')
importScripts(__ojw$config.sw || swBase + 'cyl/aqdu.sw.js')

importScripts(swBase + 'exju/lgwu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tnchsdjf', 1)
			...
			// `openDB('$tnchsdjf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hgqf/')) {
