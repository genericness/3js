const swBase = self.location.pathname.replace(/twgfhb\.js$/, '')

importScripts(swBase + 'rjo/nhiu.bundle.js')
importScripts(swBase + 'rjo/nhiu.config.js')
importScripts(__pwy$config.sw || swBase + 'rjo/nhiu.sw.js')

importScripts(swBase + 'ymje/puem.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vaoatfcy', 1)
			...
			// `openDB('$vaoatfcy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'boxv/')) {
