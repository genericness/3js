const swBase = self.location.pathname.replace(/qtoqpo\.js$/, '')

importScripts(swBase + 'ncq/wibh.bundle.js')
importScripts(swBase + 'ncq/wibh.config.js')
importScripts(__ikk$config.sw || swBase + 'ncq/wibh.sw.js')

importScripts(swBase + 'gjbt/wrti.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hdinxhfj', 1)
			...
			// `openDB('$hdinxhfj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'iqim/')) {
