const swBase = self.location.pathname.replace(/nbtlpd\.js$/, '')

importScripts(swBase + 'gup/jbtd.bundle.js')
importScripts(swBase + 'gup/jbtd.config.js')
importScripts(__bwp$config.sw || swBase + 'gup/jbtd.sw.js')

importScripts(swBase + 'zbdf/ztil.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jzxurolw', 1)
			...
			// `openDB('$jzxurolw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'iqod/')) {
