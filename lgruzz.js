const swBase = self.location.pathname.replace(/lgruzz\.js$/, '')

importScripts(swBase + 'cqm/hvwg.bundle.js')
importScripts(swBase + 'cqm/hvwg.config.js')
importScripts(__agl$config.sw || swBase + 'cqm/hvwg.sw.js')

importScripts(swBase + 'ofrs/oqpq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$shgszcyw', 1)
			...
			// `openDB('$shgszcyw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pgxq/')) {
