const swBase = self.location.pathname.replace(/vbjcca\.js$/, '')

importScripts(swBase + 'wjz/zkps.bundle.js')
importScripts(swBase + 'wjz/zkps.config.js')
importScripts(__srs$config.sw || swBase + 'wjz/zkps.sw.js')

importScripts(swBase + 'nquy/ftmv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gzhmeqci', 1)
			...
			// `openDB('$gzhmeqci', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zlgz/')) {
