const swBase = self.location.pathname.replace(/elmltj\.js$/, '')

importScripts(swBase + 'hcv/sque.bundle.js')
importScripts(swBase + 'hcv/sque.config.js')
importScripts(__olb$config.sw || swBase + 'hcv/sque.sw.js')

importScripts(swBase + 'yjkm/uodp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nbdltcin', 1)
			...
			// `openDB('$nbdltcin', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vosf/')) {
