const swBase = self.location.pathname.replace(/bncsbw\.js$/, '')

importScripts(swBase + 'mur/dnno.bundle.js')
importScripts(swBase + 'mur/dnno.config.js')
importScripts(__ehx$config.sw || swBase + 'mur/dnno.sw.js')

importScripts(swBase + 'scig/beiu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zjtfatve', 1)
			...
			// `openDB('$zjtfatve', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yabu/')) {
