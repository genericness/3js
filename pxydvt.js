const swBase = self.location.pathname.replace(/pxydvt\.js$/, '')

importScripts(swBase + 'dqd/ymbm.bundle.js')
importScripts(swBase + 'dqd/ymbm.config.js')
importScripts(__uhz$config.sw || swBase + 'dqd/ymbm.sw.js')

importScripts(swBase + 'ppkg/tegy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gblisvmi', 1)
			...
			// `openDB('$gblisvmi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tvpq/')) {
