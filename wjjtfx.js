const swBase = self.location.pathname.replace(/wjjtfx\.js$/, '')

importScripts(swBase + 'xve/wnsc.bundle.js')
importScripts(swBase + 'xve/wnsc.config.js')
importScripts(__mvi$config.sw || swBase + 'xve/wnsc.sw.js')

importScripts(swBase + 'omxx/rnsd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fsfsfoaj', 1)
			...
			// `openDB('$fsfsfoaj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'grob/')) {
