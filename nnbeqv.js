const swBase = self.location.pathname.replace(/nnbeqv\.js$/, '')

importScripts(swBase + 'pda/dimr.bundle.js')
importScripts(swBase + 'pda/dimr.config.js')
importScripts(__pqk$config.sw || swBase + 'pda/dimr.sw.js')

importScripts(swBase + 'hyso/mwpa.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dmotacou', 1)
			...
			// `openDB('$dmotacou', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zcxx/')) {
