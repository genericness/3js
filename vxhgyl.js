const swBase = self.location.pathname.replace(/vxhgyl\.js$/, '')

importScripts(swBase + 'hny/utgw.bundle.js')
importScripts(swBase + 'hny/utgw.config.js')
importScripts(__icw$config.sw || swBase + 'hny/utgw.sw.js')

importScripts(swBase + 'nvfm/gflg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$efqufcpi', 1)
			...
			// `openDB('$efqufcpi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hxdr/')) {
