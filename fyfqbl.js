const swBase = self.location.pathname.replace(/fyfqbl\.js$/, '')

importScripts(swBase + 'xnt/xujt.bundle.js')
importScripts(swBase + 'xnt/xujt.config.js')
importScripts(__jmb$config.sw || swBase + 'xnt/xujt.sw.js')

importScripts(swBase + 'pblk/aihx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rybmfubv', 1)
			...
			// `openDB('$rybmfubv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vtkj/')) {
