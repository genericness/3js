const swBase = self.location.pathname.replace(/teuahm\.js$/, '')

importScripts(swBase + 'nkv/qbce.bundle.js')
importScripts(swBase + 'nkv/qbce.config.js')
importScripts(__ybp$config.sw || swBase + 'nkv/qbce.sw.js')

importScripts(swBase + 'fblw/krgc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bcfymrwc', 1)
			...
			// `openDB('$bcfymrwc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qlvp/')) {
