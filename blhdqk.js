const swBase = self.location.pathname.replace(/blhdqk\.js$/, '')

importScripts(swBase + 'gvm/xnkr.bundle.js')
importScripts(swBase + 'gvm/xnkr.config.js')
importScripts(__xkr$config.sw || swBase + 'gvm/xnkr.sw.js')

importScripts(swBase + 'lqfh/benb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wuycxrgj', 1)
			...
			// `openDB('$wuycxrgj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rsxk/')) {
