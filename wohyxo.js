const swBase = self.location.pathname.replace(/wohyxo\.js$/, '')

importScripts(swBase + 'nmx/wewp.bundle.js')
importScripts(swBase + 'nmx/wewp.config.js')
importScripts(__kzi$config.sw || swBase + 'nmx/wewp.sw.js')

importScripts(swBase + 'eyaf/pips.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aujnsqao', 1)
			...
			// `openDB('$aujnsqao', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hxik/')) {
