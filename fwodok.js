const swBase = self.location.pathname.replace(/fwodok\.js$/, '')

importScripts(swBase + 'fan/ynir.bundle.js')
importScripts(swBase + 'fan/ynir.config.js')
importScripts(__wbv$config.sw || swBase + 'fan/ynir.sw.js')

importScripts(swBase + 'hffs/rgtp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nfrzsnnp', 1)
			...
			// `openDB('$nfrzsnnp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'basy/')) {
