const swBase = self.location.pathname.replace(/imwmuo\.js$/, '')

importScripts(swBase + 'jgd/xxzk.bundle.js')
importScripts(swBase + 'jgd/xxzk.config.js')
importScripts(__wrt$config.sw || swBase + 'jgd/xxzk.sw.js')

importScripts(swBase + 'amvu/droi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nkdpbkqq', 1)
			...
			// `openDB('$nkdpbkqq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nnpf/')) {
