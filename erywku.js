const swBase = self.location.pathname.replace(/erywku\.js$/, '')

importScripts(swBase + 'qjh/mljh.bundle.js')
importScripts(swBase + 'qjh/mljh.config.js')
importScripts(__cwq$config.sw || swBase + 'qjh/mljh.sw.js')

importScripts(swBase + 'yqjo/zsmt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xlchnxmz', 1)
			...
			// `openDB('$xlchnxmz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bitc/')) {
