const swBase = self.location.pathname.replace(/fooymf\.js$/, '')

importScripts(swBase + 'odj/jkqz.bundle.js')
importScripts(swBase + 'odj/jkqz.config.js')
importScripts(__zhd$config.sw || swBase + 'odj/jkqz.sw.js')

importScripts(swBase + 'kzoz/bwem.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$klakmtty', 1)
			...
			// `openDB('$klakmtty', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vzni/')) {
