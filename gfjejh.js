const swBase = self.location.pathname.replace(/gfjejh\.js$/, '')

importScripts(swBase + 'zdp/isql.bundle.js')
importScripts(swBase + 'zdp/isql.config.js')
importScripts(__hld$config.sw || swBase + 'zdp/isql.sw.js')

importScripts(swBase + 'slmo/rzlw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fdobesqa', 1)
			...
			// `openDB('$fdobesqa', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qsxw/')) {
