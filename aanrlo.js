const swBase = self.location.pathname.replace(/aanrlo\.js$/, '')

importScripts(swBase + 'gsl/dxlo.bundle.js')
importScripts(swBase + 'gsl/dxlo.config.js')
importScripts(__trz$config.sw || swBase + 'gsl/dxlo.sw.js')

importScripts(swBase + 'hkfj/cqpv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jghvbwcz', 1)
			...
			// `openDB('$jghvbwcz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zzfh/')) {
