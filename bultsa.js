const swBase = self.location.pathname.replace(/bultsa\.js$/, '')

importScripts(swBase + 'pqa/fuvq.bundle.js')
importScripts(swBase + 'pqa/fuvq.config.js')
importScripts(__hau$config.sw || swBase + 'pqa/fuvq.sw.js')

importScripts(swBase + 'ignf/krie.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mnsrstil', 1)
			...
			// `openDB('$mnsrstil', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yrmm/')) {
