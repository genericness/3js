const swBase = self.location.pathname.replace(/svlibl\.js$/, '')

importScripts(swBase + 'qtf/vgga.bundle.js')
importScripts(swBase + 'qtf/vgga.config.js')
importScripts(__bxu$config.sw || swBase + 'qtf/vgga.sw.js')

importScripts(swBase + 'ufuq/eshn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cxoqmcps', 1)
			...
			// `openDB('$cxoqmcps', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nnms/')) {
