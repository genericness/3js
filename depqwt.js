const swBase = self.location.pathname.replace(/depqwt\.js$/, '')

importScripts(swBase + 'sdn/yybi.bundle.js')
importScripts(swBase + 'sdn/yybi.config.js')
importScripts(__eid$config.sw || swBase + 'sdn/yybi.sw.js')

importScripts(swBase + 'heov/ubqv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gnliuarx', 1)
			...
			// `openDB('$gnliuarx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kuyt/')) {
