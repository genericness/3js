const swBase = self.location.pathname.replace(/iwaiab\.js$/, '')

importScripts(swBase + 'fcx/rfde.bundle.js')
importScripts(swBase + 'fcx/rfde.config.js')
importScripts(__qjr$config.sw || swBase + 'fcx/rfde.sw.js')

importScripts(swBase + 'jdbn/srxw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$opxkrfwl', 1)
			...
			// `openDB('$opxkrfwl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cqce/')) {
