const swBase = self.location.pathname.replace(/buwawb\.js$/, '')

importScripts(swBase + 'tqd/szez.bundle.js')
importScripts(swBase + 'tqd/szez.config.js')
importScripts(__ivw$config.sw || swBase + 'tqd/szez.sw.js')

importScripts(swBase + 'rcsl/kpuh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fynjghmi', 1)
			...
			// `openDB('$fynjghmi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'semp/')) {
