const swBase = self.location.pathname.replace(/qoaqlu\.js$/, '')

importScripts(swBase + 'xsk/nraq.bundle.js')
importScripts(swBase + 'xsk/nraq.config.js')
importScripts(__agh$config.sw || swBase + 'xsk/nraq.sw.js')

importScripts(swBase + 'yyqy/byxu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gogtkkfp', 1)
			...
			// `openDB('$gogtkkfp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ytsa/')) {
