const swBase = self.location.pathname.replace(/ytvamy\.js$/, '')

importScripts(swBase + 'ype/kdym.bundle.js')
importScripts(swBase + 'ype/kdym.config.js')
importScripts(__gtl$config.sw || swBase + 'ype/kdym.sw.js')

importScripts(swBase + 'ipoh/rriv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fpkhbvks', 1)
			...
			// `openDB('$fpkhbvks', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'putb/')) {
