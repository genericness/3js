const swBase = self.location.pathname.replace(/cuohpr\.js$/, '')

importScripts(swBase + 'pft/vnqy.bundle.js')
importScripts(swBase + 'pft/vnqy.config.js')
importScripts(__abg$config.sw || swBase + 'pft/vnqy.sw.js')

importScripts(swBase + 'sbcb/edjd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xxuydytc', 1)
			...
			// `openDB('$xxuydytc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'quxb/')) {
