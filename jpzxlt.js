const swBase = self.location.pathname.replace(/jpzxlt\.js$/, '')

importScripts(swBase + 'iie/qpry.bundle.js')
importScripts(swBase + 'iie/qpry.config.js')
importScripts(__rja$config.sw || swBase + 'iie/qpry.sw.js')

importScripts(swBase + 'usxt/ikwt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$exjrufui', 1)
			...
			// `openDB('$exjrufui', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'acft/')) {
