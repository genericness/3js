const swBase = self.location.pathname.replace(/bazwpw\.js$/, '')

importScripts(swBase + 'kcf/hxmu.bundle.js')
importScripts(swBase + 'kcf/hxmu.config.js')
importScripts(__lbx$config.sw || swBase + 'kcf/hxmu.sw.js')

importScripts(swBase + 'flru/vitg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cxdjpsxc', 1)
			...
			// `openDB('$cxdjpsxc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qoec/')) {
