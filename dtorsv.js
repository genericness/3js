const swBase = self.location.pathname.replace(/dtorsv\.js$/, '')

importScripts(swBase + 'dvu/wgxt.bundle.js')
importScripts(swBase + 'dvu/wgxt.config.js')
importScripts(__ucr$config.sw || swBase + 'dvu/wgxt.sw.js')

importScripts(swBase + 'lveh/nzdz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$uxaikawp', 1)
			...
			// `openDB('$uxaikawp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'crfo/')) {
