const swBase = self.location.pathname.replace(/jbltxz\.js$/, '')

importScripts(swBase + 'wpu/pmdb.bundle.js')
importScripts(swBase + 'wpu/pmdb.config.js')
importScripts(__yrp$config.sw || swBase + 'wpu/pmdb.sw.js')

importScripts(swBase + 'jlfu/jnjh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mdduqccc', 1)
			...
			// `openDB('$mdduqccc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lren/')) {
