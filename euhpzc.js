const swBase = self.location.pathname.replace(/euhpzc\.js$/, '')

importScripts(swBase + 'rfy/lyme.bundle.js')
importScripts(swBase + 'rfy/lyme.config.js')
importScripts(__lze$config.sw || swBase + 'rfy/lyme.sw.js')

importScripts(swBase + 'czqc/ctyu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$blljhpku', 1)
			...
			// `openDB('$blljhpku', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zkwj/')) {
