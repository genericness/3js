const swBase = self.location.pathname.replace(/xsiygn\.js$/, '')

importScripts(swBase + 'vwe/irsx.bundle.js')
importScripts(swBase + 'vwe/irsx.config.js')
importScripts(__fbq$config.sw || swBase + 'vwe/irsx.sw.js')

importScripts(swBase + 'aakj/venb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pfidwwwk', 1)
			...
			// `openDB('$pfidwwwk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dbxq/')) {
