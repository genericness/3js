const swBase = self.location.pathname.replace(/ujoeyq\.js$/, '')

importScripts(swBase + 'ytf/qius.bundle.js')
importScripts(swBase + 'ytf/qius.config.js')
importScripts(__lrj$config.sw || swBase + 'ytf/qius.sw.js')

importScripts(swBase + 'cewe/afhk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hvybyutr', 1)
			...
			// `openDB('$hvybyutr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'etyz/')) {
