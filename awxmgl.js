const swBase = self.location.pathname.replace(/awxmgl\.js$/, '')

importScripts(swBase + 'jkh/jadd.bundle.js')
importScripts(swBase + 'jkh/jadd.config.js')
importScripts(__pye$config.sw || swBase + 'jkh/jadd.sw.js')

importScripts(swBase + 'lqfw/dzyv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lnnpvimf', 1)
			...
			// `openDB('$lnnpvimf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cfjo/')) {
