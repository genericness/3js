const swBase = self.location.pathname.replace(/asnjsj\.js$/, '')

importScripts(swBase + 'fpt/csgk.bundle.js')
importScripts(swBase + 'fpt/csgk.config.js')
importScripts(__wkm$config.sw || swBase + 'fpt/csgk.sw.js')

importScripts(swBase + 'mhqc/nklc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sxxdlsih', 1)
			...
			// `openDB('$sxxdlsih', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'oerz/')) {
