const swBase = self.location.pathname.replace(/hrstcl\.js$/, '')

importScripts(swBase + 'tmn/kvrz.bundle.js')
importScripts(swBase + 'tmn/kvrz.config.js')
importScripts(__uyb$config.sw || swBase + 'tmn/kvrz.sw.js')

importScripts(swBase + 'ccwj/qjqo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fnrjywwn', 1)
			...
			// `openDB('$fnrjywwn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bpcx/')) {
