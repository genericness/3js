const swBase = self.location.pathname.replace(/durkrn\.js$/, '')

importScripts(swBase + 'sll/hmgc.bundle.js')
importScripts(swBase + 'sll/hmgc.config.js')
importScripts(__qks$config.sw || swBase + 'sll/hmgc.sw.js')

importScripts(swBase + 'zjvv/jtzg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rsnfxwae', 1)
			...
			// `openDB('$rsnfxwae', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xxth/')) {
