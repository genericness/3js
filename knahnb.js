const swBase = self.location.pathname.replace(/knahnb\.js$/, '')

importScripts(swBase + 'tyy/upog.bundle.js')
importScripts(swBase + 'tyy/upog.config.js')
importScripts(__nik$config.sw || swBase + 'tyy/upog.sw.js')

importScripts(swBase + 'uenm/hqht.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mziaxtgy', 1)
			...
			// `openDB('$mziaxtgy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qril/')) {
