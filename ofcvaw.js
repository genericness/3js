const swBase = self.location.pathname.replace(/ofcvaw\.js$/, '')

importScripts(swBase + 'dnz/xelf.bundle.js')
importScripts(swBase + 'dnz/xelf.config.js')
importScripts(__jmo$config.sw || swBase + 'dnz/xelf.sw.js')

importScripts(swBase + 'jvfw/shuu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jejunlng', 1)
			...
			// `openDB('$jejunlng', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pvtp/')) {
