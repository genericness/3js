const swBase = self.location.pathname.replace(/xtdmze\.js$/, '')

importScripts(swBase + 'yxz/wmgj.bundle.js')
importScripts(swBase + 'yxz/wmgj.config.js')
importScripts(__qhi$config.sw || swBase + 'yxz/wmgj.sw.js')

importScripts(swBase + 'dwkq/ieap.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$otwcotoj', 1)
			...
			// `openDB('$otwcotoj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dvat/')) {
