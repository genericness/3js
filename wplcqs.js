const swBase = self.location.pathname.replace(/wplcqs\.js$/, '')

importScripts(swBase + 'oqz/qxuc.bundle.js')
importScripts(swBase + 'oqz/qxuc.config.js')
importScripts(__svv$config.sw || swBase + 'oqz/qxuc.sw.js')

importScripts(swBase + 'nbew/geuf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$audvegbw', 1)
			...
			// `openDB('$audvegbw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zxtm/')) {
