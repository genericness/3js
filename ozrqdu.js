const swBase = self.location.pathname.replace(/ozrqdu\.js$/, '')

importScripts(swBase + 'mbr/ljry.bundle.js')
importScripts(swBase + 'mbr/ljry.config.js')
importScripts(__bhh$config.sw || swBase + 'mbr/ljry.sw.js')

importScripts(swBase + 'zlcf/gwcl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ugyblgod', 1)
			...
			// `openDB('$ugyblgod', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'caro/')) {
