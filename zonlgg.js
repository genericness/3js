const swBase = self.location.pathname.replace(/zonlgg\.js$/, '')

importScripts(swBase + 'chm/neyd.bundle.js')
importScripts(swBase + 'chm/neyd.config.js')
importScripts(__slh$config.sw || swBase + 'chm/neyd.sw.js')

importScripts(swBase + 'guzs/fpel.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$atmtocni', 1)
			...
			// `openDB('$atmtocni', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nokv/')) {
