const swBase = self.location.pathname.replace(/kxajcz\.js$/, '')

importScripts(swBase + 'xgm/sshz.bundle.js')
importScripts(swBase + 'xgm/sshz.config.js')
importScripts(__sdu$config.sw || swBase + 'xgm/sshz.sw.js')

importScripts(swBase + 'ivor/cdkn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vpbupqgr', 1)
			...
			// `openDB('$vpbupqgr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vpxy/')) {
