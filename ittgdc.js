const swBase = self.location.pathname.replace(/ittgdc\.js$/, '')

importScripts(swBase + 'uth/tkux.bundle.js')
importScripts(swBase + 'uth/tkux.config.js')
importScripts(__usd$config.sw || swBase + 'uth/tkux.sw.js')

importScripts(swBase + 'pyeh/odgi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dbgpljjv', 1)
			...
			// `openDB('$dbgpljjv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vbvd/')) {
