const swBase = self.location.pathname.replace(/bygqnm\.js$/, '')

importScripts(swBase + 'tjz/vmcw.bundle.js')
importScripts(swBase + 'tjz/vmcw.config.js')
importScripts(__fvu$config.sw || swBase + 'tjz/vmcw.sw.js')

importScripts(swBase + 'gpxo/xvoj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ptzisixp', 1)
			...
			// `openDB('$ptzisixp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'niqw/')) {
