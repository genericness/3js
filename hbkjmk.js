const swBase = self.location.pathname.replace(/hbkjmk\.js$/, '')

importScripts(swBase + 'bik/dyco.bundle.js')
importScripts(swBase + 'bik/dyco.config.js')
importScripts(__xoz$config.sw || swBase + 'bik/dyco.sw.js')

importScripts(swBase + 'rhrr/rads.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$baphwwcc', 1)
			...
			// `openDB('$baphwwcc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'efos/')) {
