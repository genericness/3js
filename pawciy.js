const swBase = self.location.pathname.replace(/pawciy\.js$/, '')

importScripts(swBase + 'yft/npcl.bundle.js')
importScripts(swBase + 'yft/npcl.config.js')
importScripts(__zoh$config.sw || swBase + 'yft/npcl.sw.js')

importScripts(swBase + 'rkht/hlwh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tjxbdpxq', 1)
			...
			// `openDB('$tjxbdpxq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qdjh/')) {
