const swBase = self.location.pathname.replace(/svomlb\.js$/, '')

importScripts(swBase + 'msi/lqvu.bundle.js')
importScripts(swBase + 'msi/lqvu.config.js')
importScripts(__lqx$config.sw || swBase + 'msi/lqvu.sw.js')

importScripts(swBase + 'wcor/qbro.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ccxwjmkm', 1)
			...
			// `openDB('$ccxwjmkm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fhqb/')) {
