const swBase = self.location.pathname.replace(/nuimeb\.js$/, '')

importScripts(swBase + 'ubp/kcmc.bundle.js')
importScripts(swBase + 'ubp/kcmc.config.js')
importScripts(__wpr$config.sw || swBase + 'ubp/kcmc.sw.js')

importScripts(swBase + 'hyqi/mmav.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ztheabpe', 1)
			...
			// `openDB('$ztheabpe', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cewq/')) {
