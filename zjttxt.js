const swBase = self.location.pathname.replace(/zjttxt\.js$/, '')

importScripts(swBase + 'ndp/toyg.bundle.js')
importScripts(swBase + 'ndp/toyg.config.js')
importScripts(__baf$config.sw || swBase + 'ndp/toyg.sw.js')

importScripts(swBase + 'twlj/hiee.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$slugsbyi', 1)
			...
			// `openDB('$slugsbyi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zcuz/')) {
