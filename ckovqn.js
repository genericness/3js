const swBase = self.location.pathname.replace(/ckovqn\.js$/, '')

importScripts(swBase + 'fhq/rczk.bundle.js')
importScripts(swBase + 'fhq/rczk.config.js')
importScripts(__zbf$config.sw || swBase + 'fhq/rczk.sw.js')

importScripts(swBase + 'zzgw/gbin.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pvoseoco', 1)
			...
			// `openDB('$pvoseoco', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zvkq/')) {
