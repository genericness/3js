const swBase = self.location.pathname.replace(/zmjnty\.js$/, '')

importScripts(swBase + 'fqt/kdmo.bundle.js')
importScripts(swBase + 'fqt/kdmo.config.js')
importScripts(__mxd$config.sw || swBase + 'fqt/kdmo.sw.js')

importScripts(swBase + 'otmg/nxwc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vqbhptzf', 1)
			...
			// `openDB('$vqbhptzf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bweq/')) {
