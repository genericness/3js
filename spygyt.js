const swBase = self.location.pathname.replace(/spygyt\.js$/, '')

importScripts(swBase + 'xkk/gpqx.bundle.js')
importScripts(swBase + 'xkk/gpqx.config.js')
importScripts(__tlu$config.sw || swBase + 'xkk/gpqx.sw.js')

importScripts(swBase + 'hvxb/rtza.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$andhvepz', 1)
			...
			// `openDB('$andhvepz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qyfo/')) {
