const swBase = self.location.pathname.replace(/hxaxfn\.js$/, '')

importScripts(swBase + 'mpn/dblj.bundle.js')
importScripts(swBase + 'mpn/dblj.config.js')
importScripts(__hwo$config.sw || swBase + 'mpn/dblj.sw.js')

importScripts(swBase + 'cvdi/nmnc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$okpgugqc', 1)
			...
			// `openDB('$okpgugqc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'quyf/')) {
