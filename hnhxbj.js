const swBase = self.location.pathname.replace(/hnhxbj\.js$/, '')

importScripts(swBase + 'eba/bfex.bundle.js')
importScripts(swBase + 'eba/bfex.config.js')
importScripts(__xvh$config.sw || swBase + 'eba/bfex.sw.js')

importScripts(swBase + 'hzdf/mkjt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xevsmxuy', 1)
			...
			// `openDB('$xevsmxuy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pmcb/')) {
