const swBase = self.location.pathname.replace(/rhccbw\.js$/, '')

importScripts(swBase + 'myx/fvod.bundle.js')
importScripts(swBase + 'myx/fvod.config.js')
importScripts(__sxf$config.sw || swBase + 'myx/fvod.sw.js')

importScripts(swBase + 'axdm/rzoa.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$okcdfekm', 1)
			...
			// `openDB('$okcdfekm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cakv/')) {
