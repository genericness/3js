const swBase = self.location.pathname.replace(/jttnrq\.js$/, '')

importScripts(swBase + 'vwy/xcnp.bundle.js')
importScripts(swBase + 'vwy/xcnp.config.js')
importScripts(__fhl$config.sw || swBase + 'vwy/xcnp.sw.js')

importScripts(swBase + 'dkxe/rfbf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gnqaywxz', 1)
			...
			// `openDB('$gnqaywxz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jdsw/')) {
