const swBase = self.location.pathname.replace(/pnugbc\.js$/, '')

importScripts(swBase + 'ked/wnsa.bundle.js')
importScripts(swBase + 'ked/wnsa.config.js')
importScripts(__rws$config.sw || swBase + 'ked/wnsa.sw.js')

importScripts(swBase + 'znht/acuk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pjwsiiga', 1)
			...
			// `openDB('$pjwsiiga', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'otxz/')) {
