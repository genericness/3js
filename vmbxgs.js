const swBase = self.location.pathname.replace(/vmbxgs\.js$/, '')

importScripts(swBase + 'fkf/rvlk.bundle.js')
importScripts(swBase + 'fkf/rvlk.config.js')
importScripts(__rpy$config.sw || swBase + 'fkf/rvlk.sw.js')

importScripts(swBase + 'zaqh/xylf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qmwpzdzl', 1)
			...
			// `openDB('$qmwpzdzl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'iatj/')) {
