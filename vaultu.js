const swBase = self.location.pathname.replace(/vaultu\.js$/, '')

importScripts(swBase + 'kwu/qyvc.bundle.js')
importScripts(swBase + 'kwu/qyvc.config.js')
importScripts(__qne$config.sw || swBase + 'kwu/qyvc.sw.js')

importScripts(swBase + 'yvbg/rngv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ooqsetze', 1)
			...
			// `openDB('$ooqsetze', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'njth/')) {
