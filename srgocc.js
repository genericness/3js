const swBase = self.location.pathname.replace(/srgocc\.js$/, '')

importScripts(swBase + 'fgy/snpa.bundle.js')
importScripts(swBase + 'fgy/snpa.config.js')
importScripts(__dfr$config.sw || swBase + 'fgy/snpa.sw.js')

importScripts(swBase + 'cbmq/ckiw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nzrbtrvd', 1)
			...
			// `openDB('$nzrbtrvd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'this/')) {
