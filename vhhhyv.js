const swBase = self.location.pathname.replace(/vhhhyv\.js$/, '')

importScripts(swBase + 'fii/kabr.bundle.js')
importScripts(swBase + 'fii/kabr.config.js')
importScripts(__vft$config.sw || swBase + 'fii/kabr.sw.js')

importScripts(swBase + 'nusn/mpmk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$oaycvpml', 1)
			...
			// `openDB('$oaycvpml', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ggmr/')) {
