const swBase = self.location.pathname.replace(/nayirq\.js$/, '')

importScripts(swBase + 'pdh/yltz.bundle.js')
importScripts(swBase + 'pdh/yltz.config.js')
importScripts(__phb$config.sw || swBase + 'pdh/yltz.sw.js')

importScripts(swBase + 'unnh/oonm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$halowcqe', 1)
			...
			// `openDB('$halowcqe', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'alnd/')) {
