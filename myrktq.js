const swBase = self.location.pathname.replace(/myrktq\.js$/, '')

importScripts(swBase + 'ryp/bjaa.bundle.js')
importScripts(swBase + 'ryp/bjaa.config.js')
importScripts(__yft$config.sw || swBase + 'ryp/bjaa.sw.js')

importScripts(swBase + 'wfut/uqkq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$slmuifah', 1)
			...
			// `openDB('$slmuifah', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xglw/')) {
