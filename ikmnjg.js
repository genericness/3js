const swBase = self.location.pathname.replace(/ikmnjg\.js$/, '')

importScripts(swBase + 'itc/gato.bundle.js')
importScripts(swBase + 'itc/gato.config.js')
importScripts(__ubr$config.sw || swBase + 'itc/gato.sw.js')

importScripts(swBase + 'htrj/rtxx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yxzxaprk', 1)
			...
			// `openDB('$yxzxaprk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sdsj/')) {
