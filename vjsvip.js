const swBase = self.location.pathname.replace(/vjsvip\.js$/, '')

importScripts(swBase + 'lrn/zzwc.bundle.js')
importScripts(swBase + 'lrn/zzwc.config.js')
importScripts(__xus$config.sw || swBase + 'lrn/zzwc.sw.js')

importScripts(swBase + 'vtcz/wrmh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$khfcyawz', 1)
			...
			// `openDB('$khfcyawz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'htwj/')) {
