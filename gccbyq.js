const swBase = self.location.pathname.replace(/gccbyq\.js$/, '')

importScripts(swBase + 'gtc/rrzm.bundle.js')
importScripts(swBase + 'gtc/rrzm.config.js')
importScripts(__ujo$config.sw || swBase + 'gtc/rrzm.sw.js')

importScripts(swBase + 'xqxa/dtpk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$emkmijwk', 1)
			...
			// `openDB('$emkmijwk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'meaq/')) {
