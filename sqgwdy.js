const swBase = self.location.pathname.replace(/sqgwdy\.js$/, '')

importScripts(swBase + 'gps/afkr.bundle.js')
importScripts(swBase + 'gps/afkr.config.js')
importScripts(__lqb$config.sw || swBase + 'gps/afkr.sw.js')

importScripts(swBase + 'zdcy/saxb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hzzfuqcr', 1)
			...
			// `openDB('$hzzfuqcr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xpva/')) {
