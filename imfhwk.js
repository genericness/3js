const swBase = self.location.pathname.replace(/imfhwk\.js$/, '')

importScripts(swBase + 'pkv/gjcr.bundle.js')
importScripts(swBase + 'pkv/gjcr.config.js')
importScripts(__mnh$config.sw || swBase + 'pkv/gjcr.sw.js')

importScripts(swBase + 'bhpl/rhui.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yyuvbsnx', 1)
			...
			// `openDB('$yyuvbsnx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pdml/')) {
