const swBase = self.location.pathname.replace(/rhyize\.js$/, '')

importScripts(swBase + 'lul/okvs.bundle.js')
importScripts(swBase + 'lul/okvs.config.js')
importScripts(__rfu$config.sw || swBase + 'lul/okvs.sw.js')

importScripts(swBase + 'xsox/ecft.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bjlwrfeh', 1)
			...
			// `openDB('$bjlwrfeh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lqqt/')) {
