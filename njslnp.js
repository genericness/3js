const swBase = self.location.pathname.replace(/njslnp\.js$/, '')

importScripts(swBase + 'yfc/hsqa.bundle.js')
importScripts(swBase + 'yfc/hsqa.config.js')
importScripts(__etr$config.sw || swBase + 'yfc/hsqa.sw.js')

importScripts(swBase + 'kgxz/ikfm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$oqgyhewu', 1)
			...
			// `openDB('$oqgyhewu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dowy/')) {
