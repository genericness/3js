const swBase = self.location.pathname.replace(/furdqw\.js$/, '')

importScripts(swBase + 'bnl/dvtk.bundle.js')
importScripts(swBase + 'bnl/dvtk.config.js')
importScripts(__cyb$config.sw || swBase + 'bnl/dvtk.sw.js')

importScripts(swBase + 'wdxn/vwuk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$benfyqhu', 1)
			...
			// `openDB('$benfyqhu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rrpn/')) {
