const swBase = self.location.pathname.replace(/ujxrqn\.js$/, '')

importScripts(swBase + 'oud/tyuz.bundle.js')
importScripts(swBase + 'oud/tyuz.config.js')
importScripts(__bcn$config.sw || swBase + 'oud/tyuz.sw.js')

importScripts(swBase + 'oejo/wavd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$otnubrdr', 1)
			...
			// `openDB('$otnubrdr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lrhu/')) {
