const swBase = self.location.pathname.replace(/jrmjsq\.js$/, '')

importScripts(swBase + 'hyy/aehy.bundle.js')
importScripts(swBase + 'hyy/aehy.config.js')
importScripts(__lsm$config.sw || swBase + 'hyy/aehy.sw.js')

importScripts(swBase + 'kvyl/nxjy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qtokkadb', 1)
			...
			// `openDB('$qtokkadb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qezx/')) {
