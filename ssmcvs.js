const swBase = self.location.pathname.replace(/ssmcvs\.js$/, '')

importScripts(swBase + 'erp/opqj.bundle.js')
importScripts(swBase + 'erp/opqj.config.js')
importScripts(__llh$config.sw || swBase + 'erp/opqj.sw.js')

importScripts(swBase + 'tusj/gzqe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kfhwdynk', 1)
			...
			// `openDB('$kfhwdynk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qoor/')) {
