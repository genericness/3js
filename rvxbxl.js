const swBase = self.location.pathname.replace(/rvxbxl\.js$/, '')

importScripts(swBase + 'jng/ojlh.bundle.js')
importScripts(swBase + 'jng/ojlh.config.js')
importScripts(__yir$config.sw || swBase + 'jng/ojlh.sw.js')

importScripts(swBase + 'zfrq/kupu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qpenbjmh', 1)
			...
			// `openDB('$qpenbjmh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gxvi/')) {
