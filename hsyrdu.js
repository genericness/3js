const swBase = self.location.pathname.replace(/hsyrdu\.js$/, '')

importScripts(swBase + 'dxb/kuwq.bundle.js')
importScripts(swBase + 'dxb/kuwq.config.js')
importScripts(__uhv$config.sw || swBase + 'dxb/kuwq.sw.js')

importScripts(swBase + 'wrtx/dzwy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dmexitbt', 1)
			...
			// `openDB('$dmexitbt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pujm/')) {
