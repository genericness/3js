const swBase = self.location.pathname.replace(/hpwfgh\.js$/, '')

importScripts(swBase + 'dgu/odxk.bundle.js')
importScripts(swBase + 'dgu/odxk.config.js')
importScripts(__hva$config.sw || swBase + 'dgu/odxk.sw.js')

importScripts(swBase + 'rmuw/fomg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$crhgfken', 1)
			...
			// `openDB('$crhgfken', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dcdd/')) {
