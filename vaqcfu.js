const swBase = self.location.pathname.replace(/vaqcfu\.js$/, '')

importScripts(swBase + 'qzq/sbja.bundle.js')
importScripts(swBase + 'qzq/sbja.config.js')
importScripts(__xub$config.sw || swBase + 'qzq/sbja.sw.js')

importScripts(swBase + 'rjya/vuno.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$enofggap', 1)
			...
			// `openDB('$enofggap', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xqmo/')) {
