const swBase = self.location.pathname.replace(/vejthg\.js$/, '')

importScripts(swBase + 'qla/ntsv.bundle.js')
importScripts(swBase + 'qla/ntsv.config.js')
importScripts(__myg$config.sw || swBase + 'qla/ntsv.sw.js')

importScripts(swBase + 'lerw/twsh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xpncvmce', 1)
			...
			// `openDB('$xpncvmce', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ouvt/')) {
