const swBase = self.location.pathname.replace(/ghfyjb\.js$/, '')

importScripts(swBase + 'ucd/nies.bundle.js')
importScripts(swBase + 'ucd/nies.config.js')
importScripts(__ggh$config.sw || swBase + 'ucd/nies.sw.js')

importScripts(swBase + 'krvw/icob.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$eaiwavtd', 1)
			...
			// `openDB('$eaiwavtd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'swpy/')) {
