const swBase = self.location.pathname.replace(/pslgre\.js$/, '')

importScripts(swBase + 'wad/lcye.bundle.js')
importScripts(swBase + 'wad/lcye.config.js')
importScripts(__vbo$config.sw || swBase + 'wad/lcye.sw.js')

importScripts(swBase + 'bbds/gzid.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qtjejjih', 1)
			...
			// `openDB('$qtjejjih', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ilhq/')) {
