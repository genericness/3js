const swBase = self.location.pathname.replace(/zqqqga\.js$/, '')

importScripts(swBase + 'isa/ddup.bundle.js')
importScripts(swBase + 'isa/ddup.config.js')
importScripts(__jve$config.sw || swBase + 'isa/ddup.sw.js')

importScripts(swBase + 'xphz/ikzb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lysnztku', 1)
			...
			// `openDB('$lysnztku', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zgtl/')) {
