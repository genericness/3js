const swBase = self.location.pathname.replace(/gsprdf\.js$/, '')

importScripts(swBase + 'zch/eizn.bundle.js')
importScripts(swBase + 'zch/eizn.config.js')
importScripts(__zlc$config.sw || swBase + 'zch/eizn.sw.js')

importScripts(swBase + 'fefd/eqjb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ejpjbdkf', 1)
			...
			// `openDB('$ejpjbdkf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'esxt/')) {
