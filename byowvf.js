const swBase = self.location.pathname.replace(/byowvf\.js$/, '')

importScripts(swBase + 'efb/zeaj.bundle.js')
importScripts(swBase + 'efb/zeaj.config.js')
importScripts(__zkd$config.sw || swBase + 'efb/zeaj.sw.js')

importScripts(swBase + 'iirg/xvfm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yhgveobt', 1)
			...
			// `openDB('$yhgveobt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fjcd/')) {
