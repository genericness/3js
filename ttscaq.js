const swBase = self.location.pathname.replace(/ttscaq\.js$/, '')

importScripts(swBase + 'obn/riqy.bundle.js')
importScripts(swBase + 'obn/riqy.config.js')
importScripts(__peo$config.sw || swBase + 'obn/riqy.sw.js')

importScripts(swBase + 'fyfh/pmad.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hqprntau', 1)
			...
			// `openDB('$hqprntau', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wucn/')) {
