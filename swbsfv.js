const swBase = self.location.pathname.replace(/swbsfv\.js$/, '')

importScripts(swBase + 'gcx/gyie.bundle.js')
importScripts(swBase + 'gcx/gyie.config.js')
importScripts(__wcg$config.sw || swBase + 'gcx/gyie.sw.js')

importScripts(swBase + 'njgg/ilnn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hogmazmw', 1)
			...
			// `openDB('$hogmazmw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dnjz/')) {
