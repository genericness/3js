const swBase = self.location.pathname.replace(/ttutew\.js$/, '')

importScripts(swBase + 'lzq/mpuk.bundle.js')
importScripts(swBase + 'lzq/mpuk.config.js')
importScripts(__plp$config.sw || swBase + 'lzq/mpuk.sw.js')

importScripts(swBase + 'dsbh/ybmv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lwpcqtzm', 1)
			...
			// `openDB('$lwpcqtzm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'peut/')) {
