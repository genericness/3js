const swBase = self.location.pathname.replace(/iveekq\.js$/, '')

importScripts(swBase + 'bjc/qbkb.bundle.js')
importScripts(swBase + 'bjc/qbkb.config.js')
importScripts(__dza$config.sw || swBase + 'bjc/qbkb.sw.js')

importScripts(swBase + 'xryq/lzuw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qnngtkft', 1)
			...
			// `openDB('$qnngtkft', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jauu/')) {
