const swBase = self.location.pathname.replace(/rioken\.js$/, '')

importScripts(swBase + 'hft/nfib.bundle.js')
importScripts(swBase + 'hft/nfib.config.js')
importScripts(__aoj$config.sw || swBase + 'hft/nfib.sw.js')

importScripts(swBase + 'oqnl/dtor.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$upszpekn', 1)
			...
			// `openDB('$upszpekn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lsry/')) {
