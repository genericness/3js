const swBase = self.location.pathname.replace(/asrgfb\.js$/, '')

importScripts(swBase + 'cjn/uxft.bundle.js')
importScripts(swBase + 'cjn/uxft.config.js')
importScripts(__qgp$config.sw || swBase + 'cjn/uxft.sw.js')

importScripts(swBase + 'nmuj/fjvs.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mwliwivt', 1)
			...
			// `openDB('$mwliwivt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cvzj/')) {
