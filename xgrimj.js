const swBase = self.location.pathname.replace(/xgrimj\.js$/, '')

importScripts(swBase + 'xgw/vshf.bundle.js')
importScripts(swBase + 'xgw/vshf.config.js')
importScripts(__ixt$config.sw || swBase + 'xgw/vshf.sw.js')

importScripts(swBase + 'mkzr/zihn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qjwqeyrr', 1)
			...
			// `openDB('$qjwqeyrr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nhdk/')) {
