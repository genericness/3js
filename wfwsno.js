const swBase = self.location.pathname.replace(/wfwsno\.js$/, '')

importScripts(swBase + 'uvs/ihnv.bundle.js')
importScripts(swBase + 'uvs/ihnv.config.js')
importScripts(__hbe$config.sw || swBase + 'uvs/ihnv.sw.js')

importScripts(swBase + 'yshb/cxjs.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hcsqlvuv', 1)
			...
			// `openDB('$hcsqlvuv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xzlz/')) {
