const swBase = self.location.pathname.replace(/aouvjk\.js$/, '')

importScripts(swBase + 'llb/clrl.bundle.js')
importScripts(swBase + 'llb/clrl.config.js')
importScripts(__ayy$config.sw || swBase + 'llb/clrl.sw.js')

importScripts(swBase + 'muxw/dhqq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lmfbeihv', 1)
			...
			// `openDB('$lmfbeihv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gkah/')) {
