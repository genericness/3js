const swBase = self.location.pathname.replace(/kmmjmd\.js$/, '')

importScripts(swBase + 'ixk/qjfa.bundle.js')
importScripts(swBase + 'ixk/qjfa.config.js')
importScripts(__zzx$config.sw || swBase + 'ixk/qjfa.sw.js')

importScripts(swBase + 'whrd/yhud.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$glxicmkf', 1)
			...
			// `openDB('$glxicmkf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nyen/')) {
