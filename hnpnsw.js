const swBase = self.location.pathname.replace(/hnpnsw\.js$/, '')

importScripts(swBase + 'hyv/wqkn.bundle.js')
importScripts(swBase + 'hyv/wqkn.config.js')
importScripts(__ylm$config.sw || swBase + 'hyv/wqkn.sw.js')

importScripts(swBase + 'dpym/muao.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qhshkqzn', 1)
			...
			// `openDB('$qhshkqzn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'topv/')) {
