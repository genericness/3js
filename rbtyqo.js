const swBase = self.location.pathname.replace(/rbtyqo\.js$/, '')

importScripts(swBase + 'yil/xuea.bundle.js')
importScripts(swBase + 'yil/xuea.config.js')
importScripts(__whw$config.sw || swBase + 'yil/xuea.sw.js')

importScripts(swBase + 'xkfx/ueko.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$oghzabbv', 1)
			...
			// `openDB('$oghzabbv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hjfm/')) {
