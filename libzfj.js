const swBase = self.location.pathname.replace(/libzfj\.js$/, '')

importScripts(swBase + 'fik/xssm.bundle.js')
importScripts(swBase + 'fik/xssm.config.js')
importScripts(__xdv$config.sw || swBase + 'fik/xssm.sw.js')

importScripts(swBase + 'aegt/xfbr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yiitwoqx', 1)
			...
			// `openDB('$yiitwoqx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lxfm/')) {
