const swBase = self.location.pathname.replace(/qlpnro\.js$/, '')

importScripts(swBase + 'lbz/kcfo.bundle.js')
importScripts(swBase + 'lbz/kcfo.config.js')
importScripts(__ofa$config.sw || swBase + 'lbz/kcfo.sw.js')

importScripts(swBase + 'hwvp/gefc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fgitbngh', 1)
			...
			// `openDB('$fgitbngh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hjik/')) {
