const swBase = self.location.pathname.replace(/srabgw\.js$/, '')

importScripts(swBase + 'qvw/merb.bundle.js')
importScripts(swBase + 'qvw/merb.config.js')
importScripts(__tyk$config.sw || swBase + 'qvw/merb.sw.js')

importScripts(swBase + 'sczs/cuot.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jldsgrqc', 1)
			...
			// `openDB('$jldsgrqc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gesv/')) {
