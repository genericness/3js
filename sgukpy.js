const swBase = self.location.pathname.replace(/sgukpy\.js$/, '')

importScripts(swBase + 'vxn/yhxc.bundle.js')
importScripts(swBase + 'vxn/yhxc.config.js')
importScripts(__vdq$config.sw || swBase + 'vxn/yhxc.sw.js')

importScripts(swBase + 'tnvf/alxi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wjxgcidp', 1)
			...
			// `openDB('$wjxgcidp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'atsa/')) {
