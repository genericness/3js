const swBase = self.location.pathname.replace(/wwhmcv\.js$/, '')

importScripts(swBase + 'fxj/rjkv.bundle.js')
importScripts(swBase + 'fxj/rjkv.config.js')
importScripts(__tcq$config.sw || swBase + 'fxj/rjkv.sw.js')

importScripts(swBase + 'nhks/fohz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$prglyrdb', 1)
			...
			// `openDB('$prglyrdb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jzak/')) {
