const swBase = self.location.pathname.replace(/klbutt\.js$/, '')

importScripts(swBase + 'ark/cnwn.bundle.js')
importScripts(swBase + 'ark/cnwn.config.js')
importScripts(__bsg$config.sw || swBase + 'ark/cnwn.sw.js')

importScripts(swBase + 'kdqb/myux.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$evixvlud', 1)
			...
			// `openDB('$evixvlud', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dbow/')) {
