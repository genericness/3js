const swBase = self.location.pathname.replace(/hltaqs\.js$/, '')

importScripts(swBase + 'oay/gcdk.bundle.js')
importScripts(swBase + 'oay/gcdk.config.js')
importScripts(__fqc$config.sw || swBase + 'oay/gcdk.sw.js')

importScripts(swBase + 'mifm/cylv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cgrhxtkc', 1)
			...
			// `openDB('$cgrhxtkc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cyjn/')) {
