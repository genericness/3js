const swBase = self.location.pathname.replace(/sgkvsc\.js$/, '')

importScripts(swBase + 'fgv/rtzl.bundle.js')
importScripts(swBase + 'fgv/rtzl.config.js')
importScripts(__zyi$config.sw || swBase + 'fgv/rtzl.sw.js')

importScripts(swBase + 'sdor/twit.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$iyxpquww', 1)
			...
			// `openDB('$iyxpquww', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ojiu/')) {
