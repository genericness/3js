const swBase = self.location.pathname.replace(/gnglvl\.js$/, '')

importScripts(swBase + 'tfw/wjna.bundle.js')
importScripts(swBase + 'tfw/wjna.config.js')
importScripts(__ynw$config.sw || swBase + 'tfw/wjna.sw.js')

importScripts(swBase + 'mmyv/qhsu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aglfztwq', 1)
			...
			// `openDB('$aglfztwq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ppyy/')) {
