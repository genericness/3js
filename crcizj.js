const swBase = self.location.pathname.replace(/crcizj\.js$/, '')

importScripts(swBase + 'bhi/evxx.bundle.js')
importScripts(swBase + 'bhi/evxx.config.js')
importScripts(__mxf$config.sw || swBase + 'bhi/evxx.sw.js')

importScripts(swBase + 'ouxm/cgnd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fdatdsbq', 1)
			...
			// `openDB('$fdatdsbq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zyze/')) {
