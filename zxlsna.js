const swBase = self.location.pathname.replace(/zxlsna\.js$/, '')

importScripts(swBase + 'ely/ogog.bundle.js')
importScripts(swBase + 'ely/ogog.config.js')
importScripts(__kce$config.sw || swBase + 'ely/ogog.sw.js')

importScripts(swBase + 'egfl/jqre.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ttrsfnkl', 1)
			...
			// `openDB('$ttrsfnkl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vucs/')) {
