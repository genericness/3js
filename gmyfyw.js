const swBase = self.location.pathname.replace(/gmyfyw\.js$/, '')

importScripts(swBase + 'mci/itax.bundle.js')
importScripts(swBase + 'mci/itax.config.js')
importScripts(__pcz$config.sw || swBase + 'mci/itax.sw.js')

importScripts(swBase + 'zhyj/uwfk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ixvgmpgk', 1)
			...
			// `openDB('$ixvgmpgk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ssnu/')) {
