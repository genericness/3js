const swBase = self.location.pathname.replace(/lukrnw\.js$/, '')

importScripts(swBase + 'axn/kyuf.bundle.js')
importScripts(swBase + 'axn/kyuf.config.js')
importScripts(__ffi$config.sw || swBase + 'axn/kyuf.sw.js')

importScripts(swBase + 'qybz/verg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xlkroyim', 1)
			...
			// `openDB('$xlkroyim', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'guel/')) {
