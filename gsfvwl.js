const swBase = self.location.pathname.replace(/gsfvwl\.js$/, '')

importScripts(swBase + 'yff/hvbi.bundle.js')
importScripts(swBase + 'yff/hvbi.config.js')
importScripts(__unt$config.sw || swBase + 'yff/hvbi.sw.js')

importScripts(swBase + 'iemx/uzdc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$uxqlfxzb', 1)
			...
			// `openDB('$uxqlfxzb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lwyy/')) {
