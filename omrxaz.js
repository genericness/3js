const swBase = self.location.pathname.replace(/omrxaz\.js$/, '')

importScripts(swBase + 'bsh/rlce.bundle.js')
importScripts(swBase + 'bsh/rlce.config.js')
importScripts(__ovv$config.sw || swBase + 'bsh/rlce.sw.js')

importScripts(swBase + 'yfmn/aeyz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fknielfp', 1)
			...
			// `openDB('$fknielfp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mpmq/')) {
