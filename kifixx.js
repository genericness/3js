const swBase = self.location.pathname.replace(/kifixx\.js$/, '')

importScripts(swBase + 'cuf/wayb.bundle.js')
importScripts(swBase + 'cuf/wayb.config.js')
importScripts(__yig$config.sw || swBase + 'cuf/wayb.sw.js')

importScripts(swBase + 'rpcn/okoh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hhiwpwpf', 1)
			...
			// `openDB('$hhiwpwpf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gqmt/')) {
