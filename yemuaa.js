const swBase = self.location.pathname.replace(/yemuaa\.js$/, '')

importScripts(swBase + 'jva/jrjr.bundle.js')
importScripts(swBase + 'jva/jrjr.config.js')
importScripts(__gej$config.sw || swBase + 'jva/jrjr.sw.js')

importScripts(swBase + 'vkej/vgrz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vltooxfv', 1)
			...
			// `openDB('$vltooxfv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vyco/')) {
