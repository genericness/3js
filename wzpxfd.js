const swBase = self.location.pathname.replace(/wzpxfd\.js$/, '')

importScripts(swBase + 'cqg/wpfr.bundle.js')
importScripts(swBase + 'cqg/wpfr.config.js')
importScripts(__ysr$config.sw || swBase + 'cqg/wpfr.sw.js')

importScripts(swBase + 'gjbi/qxcx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xkgbtfed', 1)
			...
			// `openDB('$xkgbtfed', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'eyxz/')) {
