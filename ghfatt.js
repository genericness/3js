const swBase = self.location.pathname.replace(/ghfatt\.js$/, '')

importScripts(swBase + 'crn/tohu.bundle.js')
importScripts(swBase + 'crn/tohu.config.js')
importScripts(__mdq$config.sw || swBase + 'crn/tohu.sw.js')

importScripts(swBase + 'aghp/esue.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jlyseoyd', 1)
			...
			// `openDB('$jlyseoyd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'foue/')) {
