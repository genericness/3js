const swBase = self.location.pathname.replace(/xzhmzn\.js$/, '')

importScripts(swBase + 'gep/kvrb.bundle.js')
importScripts(swBase + 'gep/kvrb.config.js')
importScripts(__gsw$config.sw || swBase + 'gep/kvrb.sw.js')

importScripts(swBase + 'edze/hdky.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rotccmfq', 1)
			...
			// `openDB('$rotccmfq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pujx/')) {
