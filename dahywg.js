const swBase = self.location.pathname.replace(/dahywg\.js$/, '')

importScripts(swBase + 'fqg/dqho.bundle.js')
importScripts(swBase + 'fqg/dqho.config.js')
importScripts(__kbo$config.sw || swBase + 'fqg/dqho.sw.js')

importScripts(swBase + 'vjzn/tiej.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gxyohdoi', 1)
			...
			// `openDB('$gxyohdoi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dene/')) {
