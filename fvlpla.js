const swBase = self.location.pathname.replace(/fvlpla\.js$/, '')

importScripts(swBase + 'lqz/oqmu.bundle.js')
importScripts(swBase + 'lqz/oqmu.config.js')
importScripts(__taj$config.sw || swBase + 'lqz/oqmu.sw.js')

importScripts(swBase + 'vgel/ifzz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cujrhzph', 1)
			...
			// `openDB('$cujrhzph', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cwja/')) {
