const swBase = self.location.pathname.replace(/ttaeko\.js$/, '')

importScripts(swBase + 'qyf/xhjr.bundle.js')
importScripts(swBase + 'qyf/xhjr.config.js')
importScripts(__koq$config.sw || swBase + 'qyf/xhjr.sw.js')

importScripts(swBase + 'icma/arwr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$brirntpx', 1)
			...
			// `openDB('$brirntpx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mcvh/')) {
