const swBase = self.location.pathname.replace(/qpbrvm\.js$/, '')

importScripts(swBase + 'pcd/oycj.bundle.js')
importScripts(swBase + 'pcd/oycj.config.js')
importScripts(__zam$config.sw || swBase + 'pcd/oycj.sw.js')

importScripts(swBase + 'janr/jdzh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pinutkzt', 1)
			...
			// `openDB('$pinutkzt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'phvp/')) {
