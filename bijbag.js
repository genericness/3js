const swBase = self.location.pathname.replace(/bijbag\.js$/, '')

importScripts(swBase + 'qgo/qudv.bundle.js')
importScripts(swBase + 'qgo/qudv.config.js')
importScripts(__mdv$config.sw || swBase + 'qgo/qudv.sw.js')

importScripts(swBase + 'uend/wpuq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zmvgyfhl', 1)
			...
			// `openDB('$zmvgyfhl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dfup/')) {
