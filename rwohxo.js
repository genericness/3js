const swBase = self.location.pathname.replace(/rwohxo\.js$/, '')

importScripts(swBase + 'kkn/wucv.bundle.js')
importScripts(swBase + 'kkn/wucv.config.js')
importScripts(__hrk$config.sw || swBase + 'kkn/wucv.sw.js')

importScripts(swBase + 'dgdk/rexq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$prgmlzoo', 1)
			...
			// `openDB('$prgmlzoo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tyzd/')) {
