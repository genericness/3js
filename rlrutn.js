const swBase = self.location.pathname.replace(/rlrutn\.js$/, '')

importScripts(swBase + 'vvu/shkm.bundle.js')
importScripts(swBase + 'vvu/shkm.config.js')
importScripts(__tzs$config.sw || swBase + 'vvu/shkm.sw.js')

importScripts(swBase + 'qohd/achn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kxovtano', 1)
			...
			// `openDB('$kxovtano', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gqjj/')) {
