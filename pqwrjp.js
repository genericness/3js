const swBase = self.location.pathname.replace(/pqwrjp\.js$/, '')

importScripts(swBase + 'szk/hrba.bundle.js')
importScripts(swBase + 'szk/hrba.config.js')
importScripts(__mzz$config.sw || swBase + 'szk/hrba.sw.js')

importScripts(swBase + 'zgfe/hndf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vpkaakhd', 1)
			...
			// `openDB('$vpkaakhd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'puyp/')) {
