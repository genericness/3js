const swBase = self.location.pathname.replace(/eotzdo\.js$/, '')

importScripts(swBase + 'awd/omva.bundle.js')
importScripts(swBase + 'awd/omva.config.js')
importScripts(__ste$config.sw || swBase + 'awd/omva.sw.js')

importScripts(swBase + 'ajyo/wnrw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aypjajqo', 1)
			...
			// `openDB('$aypjajqo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kybh/')) {
