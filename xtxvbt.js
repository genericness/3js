const swBase = self.location.pathname.replace(/xtxvbt\.js$/, '')

importScripts(swBase + 'nmk/zzch.bundle.js')
importScripts(swBase + 'nmk/zzch.config.js')
importScripts(__ydk$config.sw || swBase + 'nmk/zzch.sw.js')

importScripts(swBase + 'wyuu/rgcc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$snyqaaiv', 1)
			...
			// `openDB('$snyqaaiv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wunn/')) {
