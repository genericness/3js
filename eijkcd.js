const swBase = self.location.pathname.replace(/eijkcd\.js$/, '')

importScripts(swBase + 'xse/svtj.bundle.js')
importScripts(swBase + 'xse/svtj.config.js')
importScripts(__ugi$config.sw || swBase + 'xse/svtj.sw.js')

importScripts(swBase + 'ivcf/kkmr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$uotucfbw', 1)
			...
			// `openDB('$uotucfbw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wruq/')) {
