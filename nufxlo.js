const swBase = self.location.pathname.replace(/nufxlo\.js$/, '')

importScripts(swBase + 'iku/apxd.bundle.js')
importScripts(swBase + 'iku/apxd.config.js')
importScripts(__tbg$config.sw || swBase + 'iku/apxd.sw.js')

importScripts(swBase + 'lwic/pjsr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nlrxkdpc', 1)
			...
			// `openDB('$nlrxkdpc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gahk/')) {
