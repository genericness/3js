const swBase = self.location.pathname.replace(/lxvutm\.js$/, '')

importScripts(swBase + 'nyl/jede.bundle.js')
importScripts(swBase + 'nyl/jede.config.js')
importScripts(__tha$config.sw || swBase + 'nyl/jede.sw.js')

importScripts(swBase + 'hkgw/klcy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nysbhvya', 1)
			...
			// `openDB('$nysbhvya', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lgfa/')) {
