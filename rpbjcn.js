const swBase = self.location.pathname.replace(/rpbjcn\.js$/, '')

importScripts(swBase + 'ieq/mqjb.bundle.js')
importScripts(swBase + 'ieq/mqjb.config.js')
importScripts(__sed$config.sw || swBase + 'ieq/mqjb.sw.js')

importScripts(swBase + 'hlwa/lbho.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nagkbrwx', 1)
			...
			// `openDB('$nagkbrwx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zlcw/')) {
