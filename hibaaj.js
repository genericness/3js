const swBase = self.location.pathname.replace(/hibaaj\.js$/, '')

importScripts(swBase + 'agm/hfze.bundle.js')
importScripts(swBase + 'agm/hfze.config.js')
importScripts(__ook$config.sw || swBase + 'agm/hfze.sw.js')

importScripts(swBase + 'sdpq/hgdb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kitffsww', 1)
			...
			// `openDB('$kitffsww', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mxam/')) {
