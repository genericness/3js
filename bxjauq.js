const swBase = self.location.pathname.replace(/bxjauq\.js$/, '')

importScripts(swBase + 'obs/wyhj.bundle.js')
importScripts(swBase + 'obs/wyhj.config.js')
importScripts(__hxq$config.sw || swBase + 'obs/wyhj.sw.js')

importScripts(swBase + 'bdrm/lqhj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$eyuomnxd', 1)
			...
			// `openDB('$eyuomnxd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dfng/')) {
