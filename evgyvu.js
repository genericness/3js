const swBase = self.location.pathname.replace(/evgyvu\.js$/, '')

importScripts(swBase + 'kdy/kylg.bundle.js')
importScripts(swBase + 'kdy/kylg.config.js')
importScripts(__egd$config.sw || swBase + 'kdy/kylg.sw.js')

importScripts(swBase + 'hdil/ydnx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bgsmpsll', 1)
			...
			// `openDB('$bgsmpsll', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'urjy/')) {
