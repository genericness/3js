const swBase = self.location.pathname.replace(/chsoxk\.js$/, '')

importScripts(swBase + 'ths/fxvt.bundle.js')
importScripts(swBase + 'ths/fxvt.config.js')
importScripts(__sxp$config.sw || swBase + 'ths/fxvt.sw.js')

importScripts(swBase + 'pegq/ompq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$eplzdybv', 1)
			...
			// `openDB('$eplzdybv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hgah/')) {
