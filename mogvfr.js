const swBase = self.location.pathname.replace(/mogvfr\.js$/, '')

importScripts(swBase + 'wmp/tgbu.bundle.js')
importScripts(swBase + 'wmp/tgbu.config.js')
importScripts(__obm$config.sw || swBase + 'wmp/tgbu.sw.js')

importScripts(swBase + 'ggqr/swec.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$drbxzijz', 1)
			...
			// `openDB('$drbxzijz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zbnk/')) {
