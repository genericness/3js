const swBase = self.location.pathname.replace(/avhchi\.js$/, '')

importScripts(swBase + 'srg/qpmo.bundle.js')
importScripts(swBase + 'srg/qpmo.config.js')
importScripts(__ppi$config.sw || swBase + 'srg/qpmo.sw.js')

importScripts(swBase + 'pyaa/dfwb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$onyjkrst', 1)
			...
			// `openDB('$onyjkrst', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ppvy/')) {
