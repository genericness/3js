const swBase = self.location.pathname.replace(/shybfq\.js$/, '')

importScripts(swBase + 'eac/nerk.bundle.js')
importScripts(swBase + 'eac/nerk.config.js')
importScripts(__idu$config.sw || swBase + 'eac/nerk.sw.js')

importScripts(swBase + 'gdwi/yebz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hctwvydt', 1)
			...
			// `openDB('$hctwvydt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sgsr/')) {
