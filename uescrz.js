const swBase = self.location.pathname.replace(/uescrz\.js$/, '')

importScripts(swBase + 'lyc/ersv.bundle.js')
importScripts(swBase + 'lyc/ersv.config.js')
importScripts(__evv$config.sw || swBase + 'lyc/ersv.sw.js')

importScripts(swBase + 'fsmo/kulw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bwksnsjm', 1)
			...
			// `openDB('$bwksnsjm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wdsh/')) {
