const swBase = self.location.pathname.replace(/ycldaz\.js$/, '')

importScripts(swBase + 'zer/fhfq.bundle.js')
importScripts(swBase + 'zer/fhfq.config.js')
importScripts(__ewg$config.sw || swBase + 'zer/fhfq.sw.js')

importScripts(swBase + 'tgbw/cpen.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ntcvtsph', 1)
			...
			// `openDB('$ntcvtsph', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ygsz/')) {
