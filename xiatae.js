const swBase = self.location.pathname.replace(/xiatae\.js$/, '')

importScripts(swBase + 'ljn/hxth.bundle.js')
importScripts(swBase + 'ljn/hxth.config.js')
importScripts(__ggy$config.sw || swBase + 'ljn/hxth.sw.js')

importScripts(swBase + 'wmqf/lupr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$njivlogt', 1)
			...
			// `openDB('$njivlogt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cbea/')) {
