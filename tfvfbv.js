const swBase = self.location.pathname.replace(/tfvfbv\.js$/, '')

importScripts(swBase + 'dky/huqd.bundle.js')
importScripts(swBase + 'dky/huqd.config.js')
importScripts(__hhf$config.sw || swBase + 'dky/huqd.sw.js')

importScripts(swBase + 'ktif/eujg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jowbcyds', 1)
			...
			// `openDB('$jowbcyds', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hdxk/')) {
