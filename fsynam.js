const swBase = self.location.pathname.replace(/fsynam\.js$/, '')

importScripts(swBase + 'sgl/dtac.bundle.js')
importScripts(swBase + 'sgl/dtac.config.js')
importScripts(__jao$config.sw || swBase + 'sgl/dtac.sw.js')

importScripts(swBase + 'ffkk/thyu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kiryufdp', 1)
			...
			// `openDB('$kiryufdp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wqzl/')) {
