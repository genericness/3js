const swBase = self.location.pathname.replace(/wsfugf\.js$/, '')

importScripts(swBase + 'hco/syet.bundle.js')
importScripts(swBase + 'hco/syet.config.js')
importScripts(__cam$config.sw || swBase + 'hco/syet.sw.js')

importScripts(swBase + 'ybqt/hazu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zvvzcoxg', 1)
			...
			// `openDB('$zvvzcoxg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'beef/')) {
