const swBase = self.location.pathname.replace(/bttptd\.js$/, '')

importScripts(swBase + 'vyi/bwgn.bundle.js')
importScripts(swBase + 'vyi/bwgn.config.js')
importScripts(__ufg$config.sw || swBase + 'vyi/bwgn.sw.js')

importScripts(swBase + 'gmae/ajzu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rxcunxtc', 1)
			...
			// `openDB('$rxcunxtc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wzve/')) {
