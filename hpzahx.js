const swBase = self.location.pathname.replace(/hpzahx\.js$/, '')

importScripts(swBase + 'ytk/sjhw.bundle.js')
importScripts(swBase + 'ytk/sjhw.config.js')
importScripts(__qfu$config.sw || swBase + 'ytk/sjhw.sw.js')

importScripts(swBase + 'bovb/yxhx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aizyogbv', 1)
			...
			// `openDB('$aizyogbv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mgrb/')) {
