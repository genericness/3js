const swBase = self.location.pathname.replace(/mmygnz\.js$/, '')

importScripts(swBase + 'rwm/wanh.bundle.js')
importScripts(swBase + 'rwm/wanh.config.js')
importScripts(__jhi$config.sw || swBase + 'rwm/wanh.sw.js')

importScripts(swBase + 'trdk/kpal.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yrxydwum', 1)
			...
			// `openDB('$yrxydwum', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'giip/')) {
