const swBase = self.location.pathname.replace(/amhace\.js$/, '')

importScripts(swBase + 'bjm/jrzu.bundle.js')
importScripts(swBase + 'bjm/jrzu.config.js')
importScripts(__iao$config.sw || swBase + 'bjm/jrzu.sw.js')

importScripts(swBase + 'aqys/shdn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jgavnusq', 1)
			...
			// `openDB('$jgavnusq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ldpe/')) {
