const swBase = self.location.pathname.replace(/bdlqms\.js$/, '')

importScripts(swBase + 'nfa/eyyj.bundle.js')
importScripts(swBase + 'nfa/eyyj.config.js')
importScripts(__cse$config.sw || swBase + 'nfa/eyyj.sw.js')

importScripts(swBase + 'hzqv/xaxh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ljihzkxp', 1)
			...
			// `openDB('$ljihzkxp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rutj/')) {
