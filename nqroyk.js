const swBase = self.location.pathname.replace(/nqroyk\.js$/, '')

importScripts(swBase + 'mpv/tghh.bundle.js')
importScripts(swBase + 'mpv/tghh.config.js')
importScripts(__oxc$config.sw || swBase + 'mpv/tghh.sw.js')

importScripts(swBase + 'jwgu/bous.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$flzwiipd', 1)
			...
			// `openDB('$flzwiipd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'eljp/')) {
