const swBase = self.location.pathname.replace(/hzysjr\.js$/, '')

importScripts(swBase + 'zwl/akji.bundle.js')
importScripts(swBase + 'zwl/akji.config.js')
importScripts(__nxk$config.sw || swBase + 'zwl/akji.sw.js')

importScripts(swBase + 'xyzb/uyab.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$exdckwzp', 1)
			...
			// `openDB('$exdckwzp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qgcl/')) {
