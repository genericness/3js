const swBase = self.location.pathname.replace(/yrbshm\.js$/, '')

importScripts(swBase + 'zcd/qvho.bundle.js')
importScripts(swBase + 'zcd/qvho.config.js')
importScripts(__ore$config.sw || swBase + 'zcd/qvho.sw.js')

importScripts(swBase + 'cpnl/haxg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$foirmrky', 1)
			...
			// `openDB('$foirmrky', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'djwc/')) {
