const swBase = self.location.pathname.replace(/xqlvbq\.js$/, '')

importScripts(swBase + 'oen/rixy.bundle.js')
importScripts(swBase + 'oen/rixy.config.js')
importScripts(__mjf$config.sw || swBase + 'oen/rixy.sw.js')

importScripts(swBase + 'aodp/nodf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$basocajr', 1)
			...
			// `openDB('$basocajr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ekxa/')) {
