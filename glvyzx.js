const swBase = self.location.pathname.replace(/glvyzx\.js$/, '')

importScripts(swBase + 'ijd/ieyq.bundle.js')
importScripts(swBase + 'ijd/ieyq.config.js')
importScripts(__kub$config.sw || swBase + 'ijd/ieyq.sw.js')

importScripts(swBase + 'ksuc/anym.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xxyeyjau', 1)
			...
			// `openDB('$xxyeyjau', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'whgz/')) {
