const swBase = self.location.pathname.replace(/aqoymn\.js$/, '')

importScripts(swBase + 'nqx/bvff.bundle.js')
importScripts(swBase + 'nqx/bvff.config.js')
importScripts(__ifm$config.sw || swBase + 'nqx/bvff.sw.js')

importScripts(swBase + 'oboa/aamc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yubzmlys', 1)
			...
			// `openDB('$yubzmlys', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dxvl/')) {
