const swBase = self.location.pathname.replace(/qiinwu\.js$/, '')

importScripts(swBase + 'fye/stvw.bundle.js')
importScripts(swBase + 'fye/stvw.config.js')
importScripts(__xiv$config.sw || swBase + 'fye/stvw.sw.js')

importScripts(swBase + 'orux/cpag.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vumdpogm', 1)
			...
			// `openDB('$vumdpogm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pkti/')) {
