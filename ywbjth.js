const swBase = self.location.pathname.replace(/ywbjth\.js$/, '')

importScripts(swBase + 'vlh/apjc.bundle.js')
importScripts(swBase + 'vlh/apjc.config.js')
importScripts(__vdt$config.sw || swBase + 'vlh/apjc.sw.js')

importScripts(swBase + 'raht/yfkw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$poarcmha', 1)
			...
			// `openDB('$poarcmha', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gnan/')) {
