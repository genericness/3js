const swBase = self.location.pathname.replace(/cvmzke\.js$/, '')

importScripts(swBase + 'fhc/thdj.bundle.js')
importScripts(swBase + 'fhc/thdj.config.js')
importScripts(__itq$config.sw || swBase + 'fhc/thdj.sw.js')

importScripts(swBase + 'qprp/nprn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fvacyvdy', 1)
			...
			// `openDB('$fvacyvdy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yulv/')) {
