const swBase = self.location.pathname.replace(/vxpdug\.js$/, '')

importScripts(swBase + 'zrv/eeum.bundle.js')
importScripts(swBase + 'zrv/eeum.config.js')
importScripts(__pqp$config.sw || swBase + 'zrv/eeum.sw.js')

importScripts(swBase + 'pmth/laeo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xupfwlcj', 1)
			...
			// `openDB('$xupfwlcj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sewa/')) {
