const swBase = self.location.pathname.replace(/xtyads\.js$/, '')

importScripts(swBase + 'igk/ruup.bundle.js')
importScripts(swBase + 'igk/ruup.config.js')
importScripts(__lhy$config.sw || swBase + 'igk/ruup.sw.js')

importScripts(swBase + 'uswj/zlit.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$georvgrd', 1)
			...
			// `openDB('$georvgrd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cwfg/')) {
