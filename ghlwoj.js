const swBase = self.location.pathname.replace(/ghlwoj\.js$/, '')

importScripts(swBase + 'ltd/jjhw.bundle.js')
importScripts(swBase + 'ltd/jjhw.config.js')
importScripts(__sud$config.sw || swBase + 'ltd/jjhw.sw.js')

importScripts(swBase + 'lsba/yncn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jubuechf', 1)
			...
			// `openDB('$jubuechf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'telf/')) {
