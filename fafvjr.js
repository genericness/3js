const swBase = self.location.pathname.replace(/fafvjr\.js$/, '')

importScripts(swBase + 'dta/pmme.bundle.js')
importScripts(swBase + 'dta/pmme.config.js')
importScripts(__tns$config.sw || swBase + 'dta/pmme.sw.js')

importScripts(swBase + 'vyfg/aqim.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zenpkepp', 1)
			...
			// `openDB('$zenpkepp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rzec/')) {
