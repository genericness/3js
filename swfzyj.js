const swBase = self.location.pathname.replace(/swfzyj\.js$/, '')

importScripts(swBase + 'nin/lxri.bundle.js')
importScripts(swBase + 'nin/lxri.config.js')
importScripts(__mau$config.sw || swBase + 'nin/lxri.sw.js')

importScripts(swBase + 'kwzf/uihg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xznwpxqo', 1)
			...
			// `openDB('$xznwpxqo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ongz/')) {
