const swBase = self.location.pathname.replace(/lsqkiw\.js$/, '')

importScripts(swBase + 'lxm/uplb.bundle.js')
importScripts(swBase + 'lxm/uplb.config.js')
importScripts(__cmt$config.sw || swBase + 'lxm/uplb.sw.js')

importScripts(swBase + 'lnmc/khiu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xuhiizsp', 1)
			...
			// `openDB('$xuhiizsp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'usxr/')) {
