const swBase = self.location.pathname.replace(/tzppbj\.js$/, '')

importScripts(swBase + 'lls/ozxc.bundle.js')
importScripts(swBase + 'lls/ozxc.config.js')
importScripts(__pra$config.sw || swBase + 'lls/ozxc.sw.js')

importScripts(swBase + 'xhsi/tmzm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mqgalrbm', 1)
			...
			// `openDB('$mqgalrbm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wauq/')) {
