const swBase = self.location.pathname.replace(/xlltnw\.js$/, '')

importScripts(swBase + 'kuj/adxw.bundle.js')
importScripts(swBase + 'kuj/adxw.config.js')
importScripts(__xlu$config.sw || swBase + 'kuj/adxw.sw.js')

importScripts(swBase + 'juvm/onks.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hzsnigpw', 1)
			...
			// `openDB('$hzsnigpw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ccsp/')) {
