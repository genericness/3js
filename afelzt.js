const swBase = self.location.pathname.replace(/afelzt\.js$/, '')

importScripts(swBase + 'lhc/fsmb.bundle.js')
importScripts(swBase + 'lhc/fsmb.config.js')
importScripts(__vyu$config.sw || swBase + 'lhc/fsmb.sw.js')

importScripts(swBase + 'gfql/qskd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$irogkrbb', 1)
			...
			// `openDB('$irogkrbb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vtqe/')) {
