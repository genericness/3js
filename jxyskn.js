const swBase = self.location.pathname.replace(/jxyskn\.js$/, '')

importScripts(swBase + 'wwc/navi.bundle.js')
importScripts(swBase + 'wwc/navi.config.js')
importScripts(__kkd$config.sw || swBase + 'wwc/navi.sw.js')

importScripts(swBase + 'yhof/cyqv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tstndzmz', 1)
			...
			// `openDB('$tstndzmz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uwdf/')) {
