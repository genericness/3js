const swBase = self.location.pathname.replace(/gkrpfa\.js$/, '')

importScripts(swBase + 'akf/dwxq.bundle.js')
importScripts(swBase + 'akf/dwxq.config.js')
importScripts(__bvc$config.sw || swBase + 'akf/dwxq.sw.js')

importScripts(swBase + 'qren/slwo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$guxqqkbl', 1)
			...
			// `openDB('$guxqqkbl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tonk/')) {
