const swBase = self.location.pathname.replace(/qrmmpz\.js$/, '')

importScripts(swBase + 'vxf/pxtt.bundle.js')
importScripts(swBase + 'vxf/pxtt.config.js')
importScripts(__apu$config.sw || swBase + 'vxf/pxtt.sw.js')

importScripts(swBase + 'qcge/gfhv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kmezwdkf', 1)
			...
			// `openDB('$kmezwdkf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sooe/')) {
