const swBase = self.location.pathname.replace(/wwrpzj\.js$/, '')

importScripts(swBase + 'gqj/ifld.bundle.js')
importScripts(swBase + 'gqj/ifld.config.js')
importScripts(__juu$config.sw || swBase + 'gqj/ifld.sw.js')

importScripts(swBase + 'nqkc/lloe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cbmgqjps', 1)
			...
			// `openDB('$cbmgqjps', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cwkw/')) {
