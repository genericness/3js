const swBase = self.location.pathname.replace(/zzreyk\.js$/, '')

importScripts(swBase + 'yeg/oppm.bundle.js')
importScripts(swBase + 'yeg/oppm.config.js')
importScripts(__jyu$config.sw || swBase + 'yeg/oppm.sw.js')

importScripts(swBase + 'kawn/phpc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hfggxmwi', 1)
			...
			// `openDB('$hfggxmwi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yikz/')) {
