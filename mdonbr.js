const swBase = self.location.pathname.replace(/mdonbr\.js$/, '')

importScripts(swBase + 'veq/vnog.bundle.js')
importScripts(swBase + 'veq/vnog.config.js')
importScripts(__mfd$config.sw || swBase + 'veq/vnog.sw.js')

importScripts(swBase + 'fxvy/xlcy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rudknzsv', 1)
			...
			// `openDB('$rudknzsv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uwnn/')) {
