const swBase = self.location.pathname.replace(/dgumcc\.js$/, '')

importScripts(swBase + 'tep/vval.bundle.js')
importScripts(swBase + 'tep/vval.config.js')
importScripts(__irs$config.sw || swBase + 'tep/vval.sw.js')

importScripts(swBase + 'isou/phoe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yzzhutko', 1)
			...
			// `openDB('$yzzhutko', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sndr/')) {
