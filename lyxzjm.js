const swBase = self.location.pathname.replace(/lyxzjm\.js$/, '')

importScripts(swBase + 'lvp/vcin.bundle.js')
importScripts(swBase + 'lvp/vcin.config.js')
importScripts(__glz$config.sw || swBase + 'lvp/vcin.sw.js')

importScripts(swBase + 'xhjl/jbys.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lzdrplxx', 1)
			...
			// `openDB('$lzdrplxx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jizf/')) {
