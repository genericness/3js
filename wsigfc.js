const swBase = self.location.pathname.replace(/wsigfc\.js$/, '')

importScripts(swBase + 'lzj/qpiu.bundle.js')
importScripts(swBase + 'lzj/qpiu.config.js')
importScripts(__nbl$config.sw || swBase + 'lzj/qpiu.sw.js')

importScripts(swBase + 'ykrh/nnea.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$daaigwbh', 1)
			...
			// `openDB('$daaigwbh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sjuf/')) {
