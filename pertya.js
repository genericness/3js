const swBase = self.location.pathname.replace(/pertya\.js$/, '')

importScripts(swBase + 'zag/wdqu.bundle.js')
importScripts(swBase + 'zag/wdqu.config.js')
importScripts(__tuu$config.sw || swBase + 'zag/wdqu.sw.js')

importScripts(swBase + 'phtc/qnwq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vjdfkree', 1)
			...
			// `openDB('$vjdfkree', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mlzj/')) {
