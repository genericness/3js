const swBase = self.location.pathname.replace(/nolvee\.js$/, '')

importScripts(swBase + 'arf/kqdc.bundle.js')
importScripts(swBase + 'arf/kqdc.config.js')
importScripts(__sqq$config.sw || swBase + 'arf/kqdc.sw.js')

importScripts(swBase + 'evwo/bxlb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vshvzkdu', 1)
			...
			// `openDB('$vshvzkdu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nhnp/')) {
