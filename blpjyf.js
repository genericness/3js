const swBase = self.location.pathname.replace(/blpjyf\.js$/, '')

importScripts(swBase + 'tbf/vkxp.bundle.js')
importScripts(swBase + 'tbf/vkxp.config.js')
importScripts(__oid$config.sw || swBase + 'tbf/vkxp.sw.js')

importScripts(swBase + 'wfsx/ppof.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wwcdwbsd', 1)
			...
			// `openDB('$wwcdwbsd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hcam/')) {
