const swBase = self.location.pathname.replace(/gawlku\.js$/, '')

importScripts(swBase + 'wwv/qpwe.bundle.js')
importScripts(swBase + 'wwv/qpwe.config.js')
importScripts(__rbj$config.sw || swBase + 'wwv/qpwe.sw.js')

importScripts(swBase + 'mrvr/skap.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tbguxotq', 1)
			...
			// `openDB('$tbguxotq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ypfb/')) {
