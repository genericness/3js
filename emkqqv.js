const swBase = self.location.pathname.replace(/emkqqv\.js$/, '')

importScripts(swBase + 'gnz/rxdq.bundle.js')
importScripts(swBase + 'gnz/rxdq.config.js')
importScripts(__kuk$config.sw || swBase + 'gnz/rxdq.sw.js')

importScripts(swBase + 'mani/rehf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ftvkhcpb', 1)
			...
			// `openDB('$ftvkhcpb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dwpk/')) {
