const swBase = self.location.pathname.replace(/emzkrs\.js$/, '')

importScripts(swBase + 'ofa/ocjo.bundle.js')
importScripts(swBase + 'ofa/ocjo.config.js')
importScripts(__ghv$config.sw || swBase + 'ofa/ocjo.sw.js')

importScripts(swBase + 'aejz/nlaz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dvhkbmlo', 1)
			...
			// `openDB('$dvhkbmlo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ajhe/')) {
