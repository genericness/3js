const swBase = self.location.pathname.replace(/qvhnii\.js$/, '')

importScripts(swBase + 'hzy/winc.bundle.js')
importScripts(swBase + 'hzy/winc.config.js')
importScripts(__aqa$config.sw || swBase + 'hzy/winc.sw.js')

importScripts(swBase + 'rulb/vaju.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vfxxraqy', 1)
			...
			// `openDB('$vfxxraqy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'aqgn/')) {
