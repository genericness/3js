const swBase = self.location.pathname.replace(/yfhrrh\.js$/, '')

importScripts(swBase + 'mil/mjky.bundle.js')
importScripts(swBase + 'mil/mjky.config.js')
importScripts(__oxw$config.sw || swBase + 'mil/mjky.sw.js')

importScripts(swBase + 'kufu/srjr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ppluhkos', 1)
			...
			// `openDB('$ppluhkos', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lkzp/')) {
