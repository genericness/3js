const swBase = self.location.pathname.replace(/bnzukm\.js$/, '')

importScripts(swBase + 'ibv/sexh.bundle.js')
importScripts(swBase + 'ibv/sexh.config.js')
importScripts(__eie$config.sw || swBase + 'ibv/sexh.sw.js')

importScripts(swBase + 'xvzi/xvav.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sciklrel', 1)
			...
			// `openDB('$sciklrel', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'eeak/')) {
