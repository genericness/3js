const swBase = self.location.pathname.replace(/xakryd\.js$/, '')

importScripts(swBase + 'pfk/qtwz.bundle.js')
importScripts(swBase + 'pfk/qtwz.config.js')
importScripts(__kke$config.sw || swBase + 'pfk/qtwz.sw.js')

importScripts(swBase + 'wuti/guar.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$boumwjcn', 1)
			...
			// `openDB('$boumwjcn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bmsi/')) {
