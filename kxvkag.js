const swBase = self.location.pathname.replace(/kxvkag\.js$/, '')

importScripts(swBase + 'yjg/smde.bundle.js')
importScripts(swBase + 'yjg/smde.config.js')
importScripts(__oul$config.sw || swBase + 'yjg/smde.sw.js')

importScripts(swBase + 'ccsh/dfyk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cswnpqjp', 1)
			...
			// `openDB('$cswnpqjp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'itzd/')) {
