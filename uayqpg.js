const swBase = self.location.pathname.replace(/uayqpg\.js$/, '')

importScripts(swBase + 'rix/lklw.bundle.js')
importScripts(swBase + 'rix/lklw.config.js')
importScripts(__gzu$config.sw || swBase + 'rix/lklw.sw.js')

importScripts(swBase + 'dxzc/jslk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$enzgamdl', 1)
			...
			// `openDB('$enzgamdl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'iqdt/')) {
