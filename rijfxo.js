const swBase = self.location.pathname.replace(/rijfxo\.js$/, '')

importScripts(swBase + 'zmp/xbjp.bundle.js')
importScripts(swBase + 'zmp/xbjp.config.js')
importScripts(__rsq$config.sw || swBase + 'zmp/xbjp.sw.js')

importScripts(swBase + 'eyix/zvwq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xzkkdlrl', 1)
			...
			// `openDB('$xzkkdlrl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ydnl/')) {
