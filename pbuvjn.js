const swBase = self.location.pathname.replace(/pbuvjn\.js$/, '')

importScripts(swBase + 'iko/xbya.bundle.js')
importScripts(swBase + 'iko/xbya.config.js')
importScripts(__ktj$config.sw || swBase + 'iko/xbya.sw.js')

importScripts(swBase + 'hotk/nate.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mkmrjakx', 1)
			...
			// `openDB('$mkmrjakx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'utbv/')) {
