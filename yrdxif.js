const swBase = self.location.pathname.replace(/yrdxif\.js$/, '')

importScripts(swBase + 'sip/acqu.bundle.js')
importScripts(swBase + 'sip/acqu.config.js')
importScripts(__bar$config.sw || swBase + 'sip/acqu.sw.js')

importScripts(swBase + 'blcv/tbch.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bwvcvlxf', 1)
			...
			// `openDB('$bwvcvlxf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'npup/')) {
