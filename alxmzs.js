const swBase = self.location.pathname.replace(/alxmzs\.js$/, '')

importScripts(swBase + 'eta/smbs.bundle.js')
importScripts(swBase + 'eta/smbs.config.js')
importScripts(__lyj$config.sw || swBase + 'eta/smbs.sw.js')

importScripts(swBase + 'jljt/wdiv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$czbsfjcx', 1)
			...
			// `openDB('$czbsfjcx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dxdm/')) {
