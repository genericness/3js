const swBase = self.location.pathname.replace(/ggslah\.js$/, '')

importScripts(swBase + 'rgw/bfuq.bundle.js')
importScripts(swBase + 'rgw/bfuq.config.js')
importScripts(__ien$config.sw || swBase + 'rgw/bfuq.sw.js')

importScripts(swBase + 'gjbk/whgm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jzediltm', 1)
			...
			// `openDB('$jzediltm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qpts/')) {
