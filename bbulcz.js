const swBase = self.location.pathname.replace(/bbulcz\.js$/, '')

importScripts(swBase + 'wkt/nvpo.bundle.js')
importScripts(swBase + 'wkt/nvpo.config.js')
importScripts(__eei$config.sw || swBase + 'wkt/nvpo.sw.js')

importScripts(swBase + 'mnnq/qcml.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vrxqknsi', 1)
			...
			// `openDB('$vrxqknsi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ylzo/')) {
