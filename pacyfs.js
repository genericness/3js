const swBase = self.location.pathname.replace(/pacyfs\.js$/, '')

importScripts(swBase + 'oav/lffb.bundle.js')
importScripts(swBase + 'oav/lffb.config.js')
importScripts(__tbl$config.sw || swBase + 'oav/lffb.sw.js')

importScripts(swBase + 'orgx/oswn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pcklhvve', 1)
			...
			// `openDB('$pcklhvve', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xfcc/')) {
