const swBase = self.location.pathname.replace(/uqbezs\.js$/, '')

importScripts(swBase + 'bzc/cidt.bundle.js')
importScripts(swBase + 'bzc/cidt.config.js')
importScripts(__tid$config.sw || swBase + 'bzc/cidt.sw.js')

importScripts(swBase + 'sebr/cipd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zdcvrmho', 1)
			...
			// `openDB('$zdcvrmho', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lnqa/')) {
