const swBase = self.location.pathname.replace(/ohtxtb\.js$/, '')

importScripts(swBase + 'cnu/tevy.bundle.js')
importScripts(swBase + 'cnu/tevy.config.js')
importScripts(__vma$config.sw || swBase + 'cnu/tevy.sw.js')

importScripts(swBase + 'uzge/ymmf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$quoiqmlb', 1)
			...
			// `openDB('$quoiqmlb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hkdb/')) {
