const swBase = self.location.pathname.replace(/hhvbfd\.js$/, '')

importScripts(swBase + 'ddd/spem.bundle.js')
importScripts(swBase + 'ddd/spem.config.js')
importScripts(__hoi$config.sw || swBase + 'ddd/spem.sw.js')

importScripts(swBase + 'pakc/cgnd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yifoklsq', 1)
			...
			// `openDB('$yifoklsq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cxxm/')) {
