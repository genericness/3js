const swBase = self.location.pathname.replace(/kidkdd\.js$/, '')

importScripts(swBase + 'wji/rbbo.bundle.js')
importScripts(swBase + 'wji/rbbo.config.js')
importScripts(__mui$config.sw || swBase + 'wji/rbbo.sw.js')

importScripts(swBase + 'agbg/sgqh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$uhpulepn', 1)
			...
			// `openDB('$uhpulepn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ievo/')) {
