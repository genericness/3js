const swBase = self.location.pathname.replace(/byslyp\.js$/, '')

importScripts(swBase + 'skd/tika.bundle.js')
importScripts(swBase + 'skd/tika.config.js')
importScripts(__czo$config.sw || swBase + 'skd/tika.sw.js')

importScripts(swBase + 'ppiu/tita.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mjntcsqh', 1)
			...
			// `openDB('$mjntcsqh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tegy/')) {
