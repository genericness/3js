const swBase = self.location.pathname.replace(/yliizs\.js$/, '')

importScripts(swBase + 'wco/kjdh.bundle.js')
importScripts(swBase + 'wco/kjdh.config.js')
importScripts(__znw$config.sw || swBase + 'wco/kjdh.sw.js')

importScripts(swBase + 'hjsg/dspv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jflkeazu', 1)
			...
			// `openDB('$jflkeazu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zbhi/')) {
