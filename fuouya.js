const swBase = self.location.pathname.replace(/fuouya\.js$/, '')

importScripts(swBase + 'poj/nsxo.bundle.js')
importScripts(swBase + 'poj/nsxo.config.js')
importScripts(__ukk$config.sw || swBase + 'poj/nsxo.sw.js')

importScripts(swBase + 'gewi/qdef.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$airptput', 1)
			...
			// `openDB('$airptput', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'efkf/')) {
