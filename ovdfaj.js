const swBase = self.location.pathname.replace(/ovdfaj\.js$/, '')

importScripts(swBase + 'gha/ekxx.bundle.js')
importScripts(swBase + 'gha/ekxx.config.js')
importScripts(__vtf$config.sw || swBase + 'gha/ekxx.sw.js')

importScripts(swBase + 'vpym/slgv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gvvyzvrb', 1)
			...
			// `openDB('$gvvyzvrb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bqai/')) {
