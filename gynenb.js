const swBase = self.location.pathname.replace(/gynenb\.js$/, '')

importScripts(swBase + 'efd/raud.bundle.js')
importScripts(swBase + 'efd/raud.config.js')
importScripts(__qqo$config.sw || swBase + 'efd/raud.sw.js')

importScripts(swBase + 'ymth/lrvf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ysgvhfsr', 1)
			...
			// `openDB('$ysgvhfsr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pfgs/')) {
