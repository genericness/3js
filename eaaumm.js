const swBase = self.location.pathname.replace(/eaaumm\.js$/, '')

importScripts(swBase + 'roo/gyaa.bundle.js')
importScripts(swBase + 'roo/gyaa.config.js')
importScripts(__qpj$config.sw || swBase + 'roo/gyaa.sw.js')

importScripts(swBase + 'ruqd/eycf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rejbrnhf', 1)
			...
			// `openDB('$rejbrnhf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zhux/')) {
