const swBase = self.location.pathname.replace(/tluscw\.js$/, '')

importScripts(swBase + 'agi/ldoq.bundle.js')
importScripts(swBase + 'agi/ldoq.config.js')
importScripts(__viq$config.sw || swBase + 'agi/ldoq.sw.js')

importScripts(swBase + 'wucm/yixe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tefgnmvk', 1)
			...
			// `openDB('$tefgnmvk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'aabq/')) {
