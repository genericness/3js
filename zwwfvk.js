const swBase = self.location.pathname.replace(/zwwfvk\.js$/, '')

importScripts(swBase + 'lih/isgb.bundle.js')
importScripts(swBase + 'lih/isgb.config.js')
importScripts(__xkp$config.sw || swBase + 'lih/isgb.sw.js')

importScripts(swBase + 'resp/maux.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ypbmuixw', 1)
			...
			// `openDB('$ypbmuixw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'noav/')) {
