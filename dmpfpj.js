const swBase = self.location.pathname.replace(/dmpfpj\.js$/, '')

importScripts(swBase + 'ang/egay.bundle.js')
importScripts(swBase + 'ang/egay.config.js')
importScripts(__sep$config.sw || swBase + 'ang/egay.sw.js')

importScripts(swBase + 'xbeu/bvfj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ukipdlbc', 1)
			...
			// `openDB('$ukipdlbc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ozxa/')) {
