const swBase = self.location.pathname.replace(/pddtta\.js$/, '')

importScripts(swBase + 'pdh/qirs.bundle.js')
importScripts(swBase + 'pdh/qirs.config.js')
importScripts(__fpc$config.sw || swBase + 'pdh/qirs.sw.js')

importScripts(swBase + 'hsiy/qycy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qqgjjzvb', 1)
			...
			// `openDB('$qqgjjzvb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yquo/')) {
