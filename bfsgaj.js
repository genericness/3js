const swBase = self.location.pathname.replace(/bfsgaj\.js$/, '')

importScripts(swBase + 'hir/dudn.bundle.js')
importScripts(swBase + 'hir/dudn.config.js')
importScripts(__awp$config.sw || swBase + 'hir/dudn.sw.js')

importScripts(swBase + 'eyeb/zoex.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$neryurhs', 1)
			...
			// `openDB('$neryurhs', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qptd/')) {
