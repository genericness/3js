const swBase = self.location.pathname.replace(/kdjhic\.js$/, '')

importScripts(swBase + 'rdu/yszo.bundle.js')
importScripts(swBase + 'rdu/yszo.config.js')
importScripts(__msi$config.sw || swBase + 'rdu/yszo.sw.js')

importScripts(swBase + 'poxs/uwxe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ehslibuk', 1)
			...
			// `openDB('$ehslibuk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pymv/')) {
