const swBase = self.location.pathname.replace(/qkjvhg\.js$/, '')

importScripts(swBase + 'exm/tfmn.bundle.js')
importScripts(swBase + 'exm/tfmn.config.js')
importScripts(__alj$config.sw || swBase + 'exm/tfmn.sw.js')

importScripts(swBase + 'xozm/szrv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bulibyno', 1)
			...
			// `openDB('$bulibyno', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cwly/')) {
