const swBase = self.location.pathname.replace(/vswwln\.js$/, '')

importScripts(swBase + 'srr/hsqy.bundle.js')
importScripts(swBase + 'srr/hsqy.config.js')
importScripts(__dmr$config.sw || swBase + 'srr/hsqy.sw.js')

importScripts(swBase + 'aous/axas.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$empghkyi', 1)
			...
			// `openDB('$empghkyi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'svij/')) {
