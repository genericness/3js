const swBase = self.location.pathname.replace(/mgbqsq\.js$/, '')

importScripts(swBase + 'mco/jrlj.bundle.js')
importScripts(swBase + 'mco/jrlj.config.js')
importScripts(__sma$config.sw || swBase + 'mco/jrlj.sw.js')

importScripts(swBase + 'huoq/ssxb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$azikorts', 1)
			...
			// `openDB('$azikorts', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jxzf/')) {
