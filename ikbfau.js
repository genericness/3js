const swBase = self.location.pathname.replace(/ikbfau\.js$/, '')

importScripts(swBase + 'eus/khbn.bundle.js')
importScripts(swBase + 'eus/khbn.config.js')
importScripts(__lwo$config.sw || swBase + 'eus/khbn.sw.js')

importScripts(swBase + 'uocp/oihz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$trxiwugy', 1)
			...
			// `openDB('$trxiwugy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lshr/')) {
