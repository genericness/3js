const swBase = self.location.pathname.replace(/ovvdgt\.js$/, '')

importScripts(swBase + 'vxo/cyrh.bundle.js')
importScripts(swBase + 'vxo/cyrh.config.js')
importScripts(__agh$config.sw || swBase + 'vxo/cyrh.sw.js')

importScripts(swBase + 'jglg/pppk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jxfxqaoj', 1)
			...
			// `openDB('$jxfxqaoj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ists/')) {
