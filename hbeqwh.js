const swBase = self.location.pathname.replace(/hbeqwh\.js$/, '')

importScripts(swBase + 'erx/szbf.bundle.js')
importScripts(swBase + 'erx/szbf.config.js')
importScripts(__bkv$config.sw || swBase + 'erx/szbf.sw.js')

importScripts(swBase + 'cunr/jypv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tmsiavui', 1)
			...
			// `openDB('$tmsiavui', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'aahm/')) {
