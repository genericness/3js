const swBase = self.location.pathname.replace(/eblyhv\.js$/, '')

importScripts(swBase + 'vxs/qade.bundle.js')
importScripts(swBase + 'vxs/qade.config.js')
importScripts(__cqn$config.sw || swBase + 'vxs/qade.sw.js')

importScripts(swBase + 'gjjr/mgxh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$abjptxzg', 1)
			...
			// `openDB('$abjptxzg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cepc/')) {
