const swBase = self.location.pathname.replace(/czlrhs\.js$/, '')

importScripts(swBase + 'ypz/msgo.bundle.js')
importScripts(swBase + 'ypz/msgo.config.js')
importScripts(__jjj$config.sw || swBase + 'ypz/msgo.sw.js')

importScripts(swBase + 'mmft/oriw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qlhkwnvc', 1)
			...
			// `openDB('$qlhkwnvc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yhht/')) {
