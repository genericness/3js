const swBase = self.location.pathname.replace(/jyfkzl\.js$/, '')

importScripts(swBase + 'aal/gigc.bundle.js')
importScripts(swBase + 'aal/gigc.config.js')
importScripts(__kjj$config.sw || swBase + 'aal/gigc.sw.js')

importScripts(swBase + 'tnez/bapk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$larcmnzt', 1)
			...
			// `openDB('$larcmnzt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wrxz/')) {
