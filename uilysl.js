const swBase = self.location.pathname.replace(/uilysl\.js$/, '')

importScripts(swBase + 'gnt/bpha.bundle.js')
importScripts(swBase + 'gnt/bpha.config.js')
importScripts(__lbt$config.sw || swBase + 'gnt/bpha.sw.js')

importScripts(swBase + 'ljuv/ifmh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ewjnyzbz', 1)
			...
			// `openDB('$ewjnyzbz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mfgk/')) {
