const swBase = self.location.pathname.replace(/nwtmni\.js$/, '')

importScripts(swBase + 'xzn/mgja.bundle.js')
importScripts(swBase + 'xzn/mgja.config.js')
importScripts(__ooy$config.sw || swBase + 'xzn/mgja.sw.js')

importScripts(swBase + 'mghp/mprf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wkovkbij', 1)
			...
			// `openDB('$wkovkbij', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dzch/')) {
