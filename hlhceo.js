const swBase = self.location.pathname.replace(/hlhceo\.js$/, '')

importScripts(swBase + 'ois/uzqk.bundle.js')
importScripts(swBase + 'ois/uzqk.config.js')
importScripts(__nqu$config.sw || swBase + 'ois/uzqk.sw.js')

importScripts(swBase + 'edts/xjem.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$imfwueuj', 1)
			...
			// `openDB('$imfwueuj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jumw/')) {
