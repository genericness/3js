const swBase = self.location.pathname.replace(/jldwsq\.js$/, '')

importScripts(swBase + 'vok/ghsc.bundle.js')
importScripts(swBase + 'vok/ghsc.config.js')
importScripts(__kjb$config.sw || swBase + 'vok/ghsc.sw.js')

importScripts(swBase + 'twvf/vprk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ndqvuyee', 1)
			...
			// `openDB('$ndqvuyee', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'brnc/')) {
