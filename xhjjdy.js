const swBase = self.location.pathname.replace(/xhjjdy\.js$/, '')

importScripts(swBase + 'xao/zubv.bundle.js')
importScripts(swBase + 'xao/zubv.config.js')
importScripts(__kay$config.sw || swBase + 'xao/zubv.sw.js')

importScripts(swBase + 'ovbf/refr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bihhbweq', 1)
			...
			// `openDB('$bihhbweq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bvov/')) {
