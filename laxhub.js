const swBase = self.location.pathname.replace(/laxhub\.js$/, '')

importScripts(swBase + 'jze/nakm.bundle.js')
importScripts(swBase + 'jze/nakm.config.js')
importScripts(__ukb$config.sw || swBase + 'jze/nakm.sw.js')

importScripts(swBase + 'tpdy/itzi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$uujbhjbh', 1)
			...
			// `openDB('$uujbhjbh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kese/')) {
