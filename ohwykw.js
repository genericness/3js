const swBase = self.location.pathname.replace(/ohwykw\.js$/, '')

importScripts(swBase + 'lll/xgra.bundle.js')
importScripts(swBase + 'lll/xgra.config.js')
importScripts(__ylk$config.sw || swBase + 'lll/xgra.sw.js')

importScripts(swBase + 'micu/xvmj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tzxknkmb', 1)
			...
			// `openDB('$tzxknkmb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mmxx/')) {
