const swBase = self.location.pathname.replace(/mgqaog\.js$/, '')

importScripts(swBase + 'rbf/hbvq.bundle.js')
importScripts(swBase + 'rbf/hbvq.config.js')
importScripts(__odi$config.sw || swBase + 'rbf/hbvq.sw.js')

importScripts(swBase + 'gsad/kxnr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vewdncol', 1)
			...
			// `openDB('$vewdncol', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dtwa/')) {
