const swBase = self.location.pathname.replace(/jxuqat\.js$/, '')

importScripts(swBase + 'ygq/fmdw.bundle.js')
importScripts(swBase + 'ygq/fmdw.config.js')
importScripts(__pnd$config.sw || swBase + 'ygq/fmdw.sw.js')

importScripts(swBase + 'tzmu/ekgf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wzxiarul', 1)
			...
			// `openDB('$wzxiarul', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xfbl/')) {
