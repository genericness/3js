const swBase = self.location.pathname.replace(/ujjwei\.js$/, '')

importScripts(swBase + 'xer/bqix.bundle.js')
importScripts(swBase + 'xer/bqix.config.js')
importScripts(__fvg$config.sw || swBase + 'xer/bqix.sw.js')

importScripts(swBase + 'mrgk/uhue.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$iaqbyctx', 1)
			...
			// `openDB('$iaqbyctx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qbtk/')) {
