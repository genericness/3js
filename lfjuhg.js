const swBase = self.location.pathname.replace(/lfjuhg\.js$/, '')

importScripts(swBase + 'wdx/ssgu.bundle.js')
importScripts(swBase + 'wdx/ssgu.config.js')
importScripts(__iek$config.sw || swBase + 'wdx/ssgu.sw.js')

importScripts(swBase + 'kyvu/oylc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$znvimlyj', 1)
			...
			// `openDB('$znvimlyj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dbrz/')) {
