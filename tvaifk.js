const swBase = self.location.pathname.replace(/tvaifk\.js$/, '')

importScripts(swBase + 'ybq/vzor.bundle.js')
importScripts(swBase + 'ybq/vzor.config.js')
importScripts(__ryu$config.sw || swBase + 'ybq/vzor.sw.js')

importScripts(swBase + 'kdqm/lbgq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ofuhggdp', 1)
			...
			// `openDB('$ofuhggdp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hkfq/')) {
