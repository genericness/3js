const swBase = self.location.pathname.replace(/qqtftu\.js$/, '')

importScripts(swBase + 'vky/eskc.bundle.js')
importScripts(swBase + 'vky/eskc.config.js')
importScripts(__sgl$config.sw || swBase + 'vky/eskc.sw.js')

importScripts(swBase + 'vwud/bhyo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$anwnxqhm', 1)
			...
			// `openDB('$anwnxqhm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pdiy/')) {
