const swBase = self.location.pathname.replace(/tlifxg\.js$/, '')

importScripts(swBase + 'rej/curc.bundle.js')
importScripts(swBase + 'rej/curc.config.js')
importScripts(__six$config.sw || swBase + 'rej/curc.sw.js')

importScripts(swBase + 'dasl/rngg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ztnucafk', 1)
			...
			// `openDB('$ztnucafk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'luge/')) {
