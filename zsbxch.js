const swBase = self.location.pathname.replace(/zsbxch\.js$/, '')

importScripts(swBase + 'lin/nmxk.bundle.js')
importScripts(swBase + 'lin/nmxk.config.js')
importScripts(__zey$config.sw || swBase + 'lin/nmxk.sw.js')

importScripts(swBase + 'qcmk/wrfo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jhspzkfz', 1)
			...
			// `openDB('$jhspzkfz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mxrz/')) {
