const swBase = self.location.pathname.replace(/cvpnok\.js$/, '')

importScripts(swBase + 'jez/idfm.bundle.js')
importScripts(swBase + 'jez/idfm.config.js')
importScripts(__bhg$config.sw || swBase + 'jez/idfm.sw.js')

importScripts(swBase + 'snzo/tdts.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jrazbfnn', 1)
			...
			// `openDB('$jrazbfnn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tjzd/')) {
