const swBase = self.location.pathname.replace(/aaeavz\.js$/, '')

importScripts(swBase + 'nit/ohtm.bundle.js')
importScripts(swBase + 'nit/ohtm.config.js')
importScripts(__ers$config.sw || swBase + 'nit/ohtm.sw.js')

importScripts(swBase + 'yqth/mzsn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ibszeiqy', 1)
			...
			// `openDB('$ibszeiqy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fudd/')) {
