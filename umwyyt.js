const swBase = self.location.pathname.replace(/umwyyt\.js$/, '')

importScripts(swBase + 'ota/dshg.bundle.js')
importScripts(swBase + 'ota/dshg.config.js')
importScripts(__ete$config.sw || swBase + 'ota/dshg.sw.js')

importScripts(swBase + 'vvrr/leum.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fshuaufa', 1)
			...
			// `openDB('$fshuaufa', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vgfa/')) {
