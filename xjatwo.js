const swBase = self.location.pathname.replace(/xjatwo\.js$/, '')

importScripts(swBase + 'vxr/srhm.bundle.js')
importScripts(swBase + 'vxr/srhm.config.js')
importScripts(__wzp$config.sw || swBase + 'vxr/srhm.sw.js')

importScripts(swBase + 'gvtm/cglk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vacwttgu', 1)
			...
			// `openDB('$vacwttgu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uscf/')) {
