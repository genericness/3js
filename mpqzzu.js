const swBase = self.location.pathname.replace(/mpqzzu\.js$/, '')

importScripts(swBase + 'jfa/nuam.bundle.js')
importScripts(swBase + 'jfa/nuam.config.js')
importScripts(__hqn$config.sw || swBase + 'jfa/nuam.sw.js')

importScripts(swBase + 'hkow/gihy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vfsakjlt', 1)
			...
			// `openDB('$vfsakjlt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hkgf/')) {
