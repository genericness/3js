const swBase = self.location.pathname.replace(/zjgmio\.js$/, '')

importScripts(swBase + 'tvn/cqgk.bundle.js')
importScripts(swBase + 'tvn/cqgk.config.js')
importScripts(__zcc$config.sw || swBase + 'tvn/cqgk.sw.js')

importScripts(swBase + 'lhvj/scyn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$szkzpmmm', 1)
			...
			// `openDB('$szkzpmmm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gxnb/')) {
