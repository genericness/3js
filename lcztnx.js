const swBase = self.location.pathname.replace(/lcztnx\.js$/, '')

importScripts(swBase + 'mvd/crni.bundle.js')
importScripts(swBase + 'mvd/crni.config.js')
importScripts(__dfe$config.sw || swBase + 'mvd/crni.sw.js')

importScripts(swBase + 'omsi/cnwe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pmntaezr', 1)
			...
			// `openDB('$pmntaezr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rctm/')) {
