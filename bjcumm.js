const swBase = self.location.pathname.replace(/bjcumm\.js$/, '')

importScripts(swBase + 'zgm/xijd.bundle.js')
importScripts(swBase + 'zgm/xijd.config.js')
importScripts(__qts$config.sw || swBase + 'zgm/xijd.sw.js')

importScripts(swBase + 'ynhs/ndmh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wwoghftu', 1)
			...
			// `openDB('$wwoghftu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'unni/')) {
