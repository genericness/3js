const swBase = self.location.pathname.replace(/mhrqbm\.js$/, '')

importScripts(swBase + 'tdw/vslh.bundle.js')
importScripts(swBase + 'tdw/vslh.config.js')
importScripts(__bns$config.sw || swBase + 'tdw/vslh.sw.js')

importScripts(swBase + 'vbky/chxh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bdxtabjb', 1)
			...
			// `openDB('$bdxtabjb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qmem/')) {
