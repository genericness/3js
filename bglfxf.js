const swBase = self.location.pathname.replace(/bglfxf\.js$/, '')

importScripts(swBase + 'ihs/biod.bundle.js')
importScripts(swBase + 'ihs/biod.config.js')
importScripts(__hfs$config.sw || swBase + 'ihs/biod.sw.js')

importScripts(swBase + 'otpy/cmss.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sokokcfc', 1)
			...
			// `openDB('$sokokcfc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dgdf/')) {
