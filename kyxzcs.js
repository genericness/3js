const swBase = self.location.pathname.replace(/kyxzcs\.js$/, '')

importScripts(swBase + 'ynf/crgr.bundle.js')
importScripts(swBase + 'ynf/crgr.config.js')
importScripts(__men$config.sw || swBase + 'ynf/crgr.sw.js')

importScripts(swBase + 'jmhv/gbip.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ulghtsty', 1)
			...
			// `openDB('$ulghtsty', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mfcn/')) {
