const swBase = self.location.pathname.replace(/ytqscu\.js$/, '')

importScripts(swBase + 'wip/yotg.bundle.js')
importScripts(swBase + 'wip/yotg.config.js')
importScripts(__cea$config.sw || swBase + 'wip/yotg.sw.js')

importScripts(swBase + 'rgvx/afbl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ypqounzz', 1)
			...
			// `openDB('$ypqounzz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hprg/')) {
