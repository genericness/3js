const swBase = self.location.pathname.replace(/kajirf\.js$/, '')

importScripts(swBase + 'ynf/lnki.bundle.js')
importScripts(swBase + 'ynf/lnki.config.js')
importScripts(__xuq$config.sw || swBase + 'ynf/lnki.sw.js')

importScripts(swBase + 'oapo/zrbs.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kppgsaut', 1)
			...
			// `openDB('$kppgsaut', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'llkv/')) {
