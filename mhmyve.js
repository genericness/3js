const swBase = self.location.pathname.replace(/mhmyve\.js$/, '')

importScripts(swBase + 'xwr/tiap.bundle.js')
importScripts(swBase + 'xwr/tiap.config.js')
importScripts(__sdc$config.sw || swBase + 'xwr/tiap.sw.js')

importScripts(swBase + 'jyem/nmxs.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tbownmup', 1)
			...
			// `openDB('$tbownmup', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yzgu/')) {
