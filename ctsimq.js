const swBase = self.location.pathname.replace(/ctsimq\.js$/, '')

importScripts(swBase + 'qnc/xcgz.bundle.js')
importScripts(swBase + 'qnc/xcgz.config.js')
importScripts(__trm$config.sw || swBase + 'qnc/xcgz.sw.js')

importScripts(swBase + 'rvzu/wfme.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rkshbcth', 1)
			...
			// `openDB('$rkshbcth', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dvhl/')) {
