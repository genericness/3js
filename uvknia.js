const swBase = self.location.pathname.replace(/uvknia\.js$/, '')

importScripts(swBase + 'ert/tqyf.bundle.js')
importScripts(swBase + 'ert/tqyf.config.js')
importScripts(__qch$config.sw || swBase + 'ert/tqyf.sw.js')

importScripts(swBase + 'auqf/rcpb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ywqsmzqm', 1)
			...
			// `openDB('$ywqsmzqm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uhkb/')) {
