const swBase = self.location.pathname.replace(/dfvdgp\.js$/, '')

importScripts(swBase + 'nlu/hdbn.bundle.js')
importScripts(swBase + 'nlu/hdbn.config.js')
importScripts(__pss$config.sw || swBase + 'nlu/hdbn.sw.js')

importScripts(swBase + 'ckxs/exve.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fvgiwuyq', 1)
			...
			// `openDB('$fvgiwuyq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'onhc/')) {
