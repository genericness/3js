const swBase = self.location.pathname.replace(/akzggq\.js$/, '')

importScripts(swBase + 'fhx/ihmn.bundle.js')
importScripts(swBase + 'fhx/ihmn.config.js')
importScripts(__trv$config.sw || swBase + 'fhx/ihmn.sw.js')

importScripts(swBase + 'jhon/wyrd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nqdhujdt', 1)
			...
			// `openDB('$nqdhujdt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'oilf/')) {
