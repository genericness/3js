const swBase = self.location.pathname.replace(/yrrpxg\.js$/, '')

importScripts(swBase + 'jhr/mzqp.bundle.js')
importScripts(swBase + 'jhr/mzqp.config.js')
importScripts(__gwa$config.sw || swBase + 'jhr/mzqp.sw.js')

importScripts(swBase + 'pgwg/llhb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$funyrloq', 1)
			...
			// `openDB('$funyrloq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'okhb/')) {
