const swBase = self.location.pathname.replace(/jibxua\.js$/, '')

importScripts(swBase + 'syc/kdeb.bundle.js')
importScripts(swBase + 'syc/kdeb.config.js')
importScripts(__hmz$config.sw || swBase + 'syc/kdeb.sw.js')

importScripts(swBase + 'qaua/owxx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nyiidgif', 1)
			...
			// `openDB('$nyiidgif', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hirt/')) {
