const swBase = self.location.pathname.replace(/pttaoj\.js$/, '')

importScripts(swBase + 'dpw/eqea.bundle.js')
importScripts(swBase + 'dpw/eqea.config.js')
importScripts(__qdc$config.sw || swBase + 'dpw/eqea.sw.js')

importScripts(swBase + 'xbpp/wlci.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nyorvasi', 1)
			...
			// `openDB('$nyorvasi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vhld/')) {
