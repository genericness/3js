const swBase = self.location.pathname.replace(/dsnwuk\.js$/, '')

importScripts(swBase + 'xxm/tgfh.bundle.js')
importScripts(swBase + 'xxm/tgfh.config.js')
importScripts(__yuf$config.sw || swBase + 'xxm/tgfh.sw.js')

importScripts(swBase + 'unrk/ydxv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zjsmsaob', 1)
			...
			// `openDB('$zjsmsaob', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rxqy/')) {
