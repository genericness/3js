const swBase = self.location.pathname.replace(/uopnrx\.js$/, '')

importScripts(swBase + 'gtb/yheq.bundle.js')
importScripts(swBase + 'gtb/yheq.config.js')
importScripts(__hlo$config.sw || swBase + 'gtb/yheq.sw.js')

importScripts(swBase + 'hjgl/neck.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rujoeols', 1)
			...
			// `openDB('$rujoeols', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'axjx/')) {
