const swBase = self.location.pathname.replace(/poymbq\.js$/, '')

importScripts(swBase + 'nyw/zmxi.bundle.js')
importScripts(swBase + 'nyw/zmxi.config.js')
importScripts(__sbi$config.sw || swBase + 'nyw/zmxi.sw.js')

importScripts(swBase + 'wqpk/oryr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$faeavdyd', 1)
			...
			// `openDB('$faeavdyd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vegc/')) {
