const swBase = self.location.pathname.replace(/wkihzh\.js$/, '')

importScripts(swBase + 'zqf/vjkw.bundle.js')
importScripts(swBase + 'zqf/vjkw.config.js')
importScripts(__ade$config.sw || swBase + 'zqf/vjkw.sw.js')

importScripts(swBase + 'xkpt/zrtj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nprzxaei', 1)
			...
			// `openDB('$nprzxaei', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dpwh/')) {
