const swBase = self.location.pathname.replace(/rjmcsr\.js$/, '')

importScripts(swBase + 'vsy/ruso.bundle.js')
importScripts(swBase + 'vsy/ruso.config.js')
importScripts(__zjb$config.sw || swBase + 'vsy/ruso.sw.js')

importScripts(swBase + 'jvpy/nlgf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bfmksqyk', 1)
			...
			// `openDB('$bfmksqyk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'elgr/')) {
