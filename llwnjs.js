const swBase = self.location.pathname.replace(/llwnjs\.js$/, '')

importScripts(swBase + 'vny/ikgn.bundle.js')
importScripts(swBase + 'vny/ikgn.config.js')
importScripts(__uzo$config.sw || swBase + 'vny/ikgn.sw.js')

importScripts(swBase + 'egmb/fhvy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kavctvvs', 1)
			...
			// `openDB('$kavctvvs', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vcph/')) {
