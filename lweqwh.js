const swBase = self.location.pathname.replace(/lweqwh\.js$/, '')

importScripts(swBase + 'eui/pfne.bundle.js')
importScripts(swBase + 'eui/pfne.config.js')
importScripts(__tlp$config.sw || swBase + 'eui/pfne.sw.js')

importScripts(swBase + 'fnry/sfmu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hfovecck', 1)
			...
			// `openDB('$hfovecck', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ythf/')) {
