const swBase = self.location.pathname.replace(/sqxwln\.js$/, '')

importScripts(swBase + 'vhr/teah.bundle.js')
importScripts(swBase + 'vhr/teah.config.js')
importScripts(__xvx$config.sw || swBase + 'vhr/teah.sw.js')

importScripts(swBase + 'lesc/aaym.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cdvinrue', 1)
			...
			// `openDB('$cdvinrue', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ueyu/')) {
