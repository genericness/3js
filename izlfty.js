const swBase = self.location.pathname.replace(/izlfty\.js$/, '')

importScripts(swBase + 'gsj/oist.bundle.js')
importScripts(swBase + 'gsj/oist.config.js')
importScripts(__kos$config.sw || swBase + 'gsj/oist.sw.js')

importScripts(swBase + 'ejhu/plzg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$prjlrjdt', 1)
			...
			// `openDB('$prjlrjdt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nrbj/')) {
