const swBase = self.location.pathname.replace(/abhylk\.js$/, '')

importScripts(swBase + 'ovz/bzsg.bundle.js')
importScripts(swBase + 'ovz/bzsg.config.js')
importScripts(__xmq$config.sw || swBase + 'ovz/bzsg.sw.js')

importScripts(swBase + 'djcp/lhkp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tsvkziij', 1)
			...
			// `openDB('$tsvkziij', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gjoq/')) {
