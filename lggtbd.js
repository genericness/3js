const swBase = self.location.pathname.replace(/lggtbd\.js$/, '')

importScripts(swBase + 'teq/drbj.bundle.js')
importScripts(swBase + 'teq/drbj.config.js')
importScripts(__huj$config.sw || swBase + 'teq/drbj.sw.js')

importScripts(swBase + 'ifgm/vlmd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$resympou', 1)
			...
			// `openDB('$resympou', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ccdz/')) {
