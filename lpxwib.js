const swBase = self.location.pathname.replace(/lpxwib\.js$/, '')

importScripts(swBase + 'awx/tlbn.bundle.js')
importScripts(swBase + 'awx/tlbn.config.js')
importScripts(__oqn$config.sw || swBase + 'awx/tlbn.sw.js')

importScripts(swBase + 'upvh/grgc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aghkdmzk', 1)
			...
			// `openDB('$aghkdmzk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'frlr/')) {
