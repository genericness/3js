const swBase = self.location.pathname.replace(/kpbxoc\.js$/, '')

importScripts(swBase + 'ain/xphp.bundle.js')
importScripts(swBase + 'ain/xphp.config.js')
importScripts(__vwx$config.sw || swBase + 'ain/xphp.sw.js')

importScripts(swBase + 'ntdi/oswm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zkdwkiyt', 1)
			...
			// `openDB('$zkdwkiyt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ioef/')) {
