const swBase = self.location.pathname.replace(/rvpnan\.js$/, '')

importScripts(swBase + 'oyb/gfvq.bundle.js')
importScripts(swBase + 'oyb/gfvq.config.js')
importScripts(__aqv$config.sw || swBase + 'oyb/gfvq.sw.js')

importScripts(swBase + 'xqmb/iykd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jevnuhni', 1)
			...
			// `openDB('$jevnuhni', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'plrw/')) {
