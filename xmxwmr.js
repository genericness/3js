const swBase = self.location.pathname.replace(/xmxwmr\.js$/, '')

importScripts(swBase + 'qsa/vqmq.bundle.js')
importScripts(swBase + 'qsa/vqmq.config.js')
importScripts(__axq$config.sw || swBase + 'qsa/vqmq.sw.js')

importScripts(swBase + 'pimx/lplq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cihhdzxt', 1)
			...
			// `openDB('$cihhdzxt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ytud/')) {
