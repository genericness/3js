const swBase = self.location.pathname.replace(/qptztz\.js$/, '')

importScripts(swBase + 'une/sgwc.bundle.js')
importScripts(swBase + 'une/sgwc.config.js')
importScripts(__ezs$config.sw || swBase + 'une/sgwc.sw.js')

importScripts(swBase + 'gesb/twnk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cfodgspg', 1)
			...
			// `openDB('$cfodgspg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pagt/')) {
