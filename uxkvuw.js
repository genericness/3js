const swBase = self.location.pathname.replace(/uxkvuw\.js$/, '')

importScripts(swBase + 'bxm/zcba.bundle.js')
importScripts(swBase + 'bxm/zcba.config.js')
importScripts(__sha$config.sw || swBase + 'bxm/zcba.sw.js')

importScripts(swBase + 'ysje/jtqr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$quegpixv', 1)
			...
			// `openDB('$quegpixv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rugw/')) {
