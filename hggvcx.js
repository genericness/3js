const swBase = self.location.pathname.replace(/hggvcx\.js$/, '')

importScripts(swBase + 'fkk/sbom.bundle.js')
importScripts(swBase + 'fkk/sbom.config.js')
importScripts(__fzo$config.sw || swBase + 'fkk/sbom.sw.js')

importScripts(swBase + 'koij/qqmb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$upkovpwr', 1)
			...
			// `openDB('$upkovpwr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ervl/')) {
