const swBase = self.location.pathname.replace(/ihebpl\.js$/, '')

importScripts(swBase + 'cwt/mqvp.bundle.js')
importScripts(swBase + 'cwt/mqvp.config.js')
importScripts(__exd$config.sw || swBase + 'cwt/mqvp.sw.js')

importScripts(swBase + 'cufq/vpdv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xsjxksqf', 1)
			...
			// `openDB('$xsjxksqf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cptx/')) {
