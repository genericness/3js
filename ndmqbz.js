const swBase = self.location.pathname.replace(/ndmqbz\.js$/, '')

importScripts(swBase + 'uez/jsuj.bundle.js')
importScripts(swBase + 'uez/jsuj.config.js')
importScripts(__cvv$config.sw || swBase + 'uez/jsuj.sw.js')

importScripts(swBase + 'maqk/diiw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kkiqqfit', 1)
			...
			// `openDB('$kkiqqfit', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'blfj/')) {
