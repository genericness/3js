const swBase = self.location.pathname.replace(/nfabli\.js$/, '')

importScripts(swBase + 'rcr/hmie.bundle.js')
importScripts(swBase + 'rcr/hmie.config.js')
importScripts(__cpp$config.sw || swBase + 'rcr/hmie.sw.js')

importScripts(swBase + 'yfzq/dldg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$csqvmkjp', 1)
			...
			// `openDB('$csqvmkjp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hyhs/')) {
