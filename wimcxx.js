const swBase = self.location.pathname.replace(/wimcxx\.js$/, '')

importScripts(swBase + 'yue/jvtj.bundle.js')
importScripts(swBase + 'yue/jvtj.config.js')
importScripts(__hss$config.sw || swBase + 'yue/jvtj.sw.js')

importScripts(swBase + 'kaov/oawz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mtzabphm', 1)
			...
			// `openDB('$mtzabphm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'afxt/')) {
