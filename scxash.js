const swBase = self.location.pathname.replace(/scxash\.js$/, '')

importScripts(swBase + 'nyh/wegf.bundle.js')
importScripts(swBase + 'nyh/wegf.config.js')
importScripts(__hua$config.sw || swBase + 'nyh/wegf.sw.js')

importScripts(swBase + 'repo/zglf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vormcwgf', 1)
			...
			// `openDB('$vormcwgf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dxty/')) {
