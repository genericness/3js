const swBase = self.location.pathname.replace(/wqvmso\.js$/, '')

importScripts(swBase + 'opc/hxqb.bundle.js')
importScripts(swBase + 'opc/hxqb.config.js')
importScripts(__kyx$config.sw || swBase + 'opc/hxqb.sw.js')

importScripts(swBase + 'puwj/mrhu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xibcegzl', 1)
			...
			// `openDB('$xibcegzl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'itqn/')) {
