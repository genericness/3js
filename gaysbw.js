const swBase = self.location.pathname.replace(/gaysbw\.js$/, '')

importScripts(swBase + 'uvr/koys.bundle.js')
importScripts(swBase + 'uvr/koys.config.js')
importScripts(__euk$config.sw || swBase + 'uvr/koys.sw.js')

importScripts(swBase + 'hmty/gozf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hruzfkky', 1)
			...
			// `openDB('$hruzfkky', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mreu/')) {
