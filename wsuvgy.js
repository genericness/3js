const swBase = self.location.pathname.replace(/wsuvgy\.js$/, '')

importScripts(swBase + 'jgg/oogx.bundle.js')
importScripts(swBase + 'jgg/oogx.config.js')
importScripts(__ljr$config.sw || swBase + 'jgg/oogx.sw.js')

importScripts(swBase + 'ohiu/czud.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gomgymbh', 1)
			...
			// `openDB('$gomgymbh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yvhb/')) {
