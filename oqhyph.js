const swBase = self.location.pathname.replace(/oqhyph\.js$/, '')

importScripts(swBase + 'lbm/whtv.bundle.js')
importScripts(swBase + 'lbm/whtv.config.js')
importScripts(__qjw$config.sw || swBase + 'lbm/whtv.sw.js')

importScripts(swBase + 'yjyz/bqfq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$imapdjpk', 1)
			...
			// `openDB('$imapdjpk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'evsc/')) {
