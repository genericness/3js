const swBase = self.location.pathname.replace(/twxmvb\.js$/, '')

importScripts(swBase + 'rnz/tzsh.bundle.js')
importScripts(swBase + 'rnz/tzsh.config.js')
importScripts(__uya$config.sw || swBase + 'rnz/tzsh.sw.js')

importScripts(swBase + 'mpcl/jrpm.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$osjmcnaf', 1)
			...
			// `openDB('$osjmcnaf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rncg/')) {
