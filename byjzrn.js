const swBase = self.location.pathname.replace(/byjzrn\.js$/, '')

importScripts(swBase + 'hij/hmmk.bundle.js')
importScripts(swBase + 'hij/hmmk.config.js')
importScripts(__umx$config.sw || swBase + 'hij/hmmk.sw.js')

importScripts(swBase + 'gpxq/eryz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$iaeagukq', 1)
			...
			// `openDB('$iaeagukq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vtmk/')) {
