const swBase = self.location.pathname.replace(/zpdkaq\.js$/, '')

importScripts(swBase + 'ipt/ctuk.bundle.js')
importScripts(swBase + 'ipt/ctuk.config.js')
importScripts(__ydl$config.sw || swBase + 'ipt/ctuk.sw.js')

importScripts(swBase + 'esnr/gxlo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ybqbochn', 1)
			...
			// `openDB('$ybqbochn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hwja/')) {
