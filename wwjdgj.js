const swBase = self.location.pathname.replace(/wwjdgj\.js$/, '')

importScripts(swBase + 'mid/qacz.bundle.js')
importScripts(swBase + 'mid/qacz.config.js')
importScripts(__jbi$config.sw || swBase + 'mid/qacz.sw.js')

importScripts(swBase + 'kurl/tsdd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nnjlsaqk', 1)
			...
			// `openDB('$nnjlsaqk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rnde/')) {
