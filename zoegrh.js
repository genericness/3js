const swBase = self.location.pathname.replace(/zoegrh\.js$/, '')

importScripts(swBase + 'zas/ofhp.bundle.js')
importScripts(swBase + 'zas/ofhp.config.js')
importScripts(__hkf$config.sw || swBase + 'zas/ofhp.sw.js')

importScripts(swBase + 'zzam/gchq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vzyjvsex', 1)
			...
			// `openDB('$vzyjvsex', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gcmw/')) {
