const swBase = self.location.pathname.replace(/gfwwlu\.js$/, '')

importScripts(swBase + 'veo/svln.bundle.js')
importScripts(swBase + 'veo/svln.config.js')
importScripts(__xwg$config.sw || swBase + 'veo/svln.sw.js')

importScripts(swBase + 'tdid/kknx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$foepopwo', 1)
			...
			// `openDB('$foepopwo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'smho/')) {
