const swBase = self.location.pathname.replace(/nnqmqg\.js$/, '')

importScripts(swBase + 'rev/ktlz.bundle.js')
importScripts(swBase + 'rev/ktlz.config.js')
importScripts(__vnv$config.sw || swBase + 'rev/ktlz.sw.js')

importScripts(swBase + 'tyxb/lyhh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sfhpyjpc', 1)
			...
			// `openDB('$sfhpyjpc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xvmz/')) {
