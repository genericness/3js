const swBase = self.location.pathname.replace(/rklrgx\.js$/, '')

importScripts(swBase + 'ihq/btxv.bundle.js')
importScripts(swBase + 'ihq/btxv.config.js')
importScripts(__txn$config.sw || swBase + 'ihq/btxv.sw.js')

importScripts(swBase + 'grrq/olbl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ybemnyek', 1)
			...
			// `openDB('$ybemnyek', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wrjh/')) {
