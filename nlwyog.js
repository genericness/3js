const swBase = self.location.pathname.replace(/nlwyog\.js$/, '')

importScripts(swBase + 'cfy/wnbs.bundle.js')
importScripts(swBase + 'cfy/wnbs.config.js')
importScripts(__djx$config.sw || swBase + 'cfy/wnbs.sw.js')

importScripts(swBase + 'htmq/jkid.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wlqfkykv', 1)
			...
			// `openDB('$wlqfkykv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'oifs/')) {
