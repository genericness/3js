const swBase = self.location.pathname.replace(/koxscu\.js$/, '')

importScripts(swBase + 'kjb/sjry.bundle.js')
importScripts(swBase + 'kjb/sjry.config.js')
importScripts(__ojt$config.sw || swBase + 'kjb/sjry.sw.js')

importScripts(swBase + 'yjju/nqrv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sxhehqof', 1)
			...
			// `openDB('$sxhehqof', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'epwd/')) {
