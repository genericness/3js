const swBase = self.location.pathname.replace(/cyhyuf\.js$/, '')

importScripts(swBase + 'jiy/djfl.bundle.js')
importScripts(swBase + 'jiy/djfl.config.js')
importScripts(__ccx$config.sw || swBase + 'jiy/djfl.sw.js')

importScripts(swBase + 'umha/ljyj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pssmdvbh', 1)
			...
			// `openDB('$pssmdvbh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tepc/')) {
