const swBase = self.location.pathname.replace(/onzgbr\.js$/, '')

importScripts(swBase + 'ffs/juzz.bundle.js')
importScripts(swBase + 'ffs/juzz.config.js')
importScripts(__lyv$config.sw || swBase + 'ffs/juzz.sw.js')

importScripts(swBase + 'iieq/owdq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qsshbzzr', 1)
			...
			// `openDB('$qsshbzzr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'eobk/')) {
