const swBase = self.location.pathname.replace(/furrdm\.js$/, '')

importScripts(swBase + 'ubl/dgjn.bundle.js')
importScripts(swBase + 'ubl/dgjn.config.js')
importScripts(__dgo$config.sw || swBase + 'ubl/dgjn.sw.js')

importScripts(swBase + 'otau/dpbb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fdzqzvoi', 1)
			...
			// `openDB('$fdzqzvoi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nypv/')) {
