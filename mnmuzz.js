const swBase = self.location.pathname.replace(/mnmuzz\.js$/, '')

importScripts(swBase + 'pmp/qpfr.bundle.js')
importScripts(swBase + 'pmp/qpfr.config.js')
importScripts(__odf$config.sw || swBase + 'pmp/qpfr.sw.js')

importScripts(swBase + 'jcyx/yfka.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mqynyfar', 1)
			...
			// `openDB('$mqynyfar', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'twid/')) {
