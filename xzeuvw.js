const swBase = self.location.pathname.replace(/xzeuvw\.js$/, '')

importScripts(swBase + 'pvw/zuvh.bundle.js')
importScripts(swBase + 'pvw/zuvh.config.js')
importScripts(__lcy$config.sw || swBase + 'pvw/zuvh.sw.js')

importScripts(swBase + 'cwpv/irfl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$thwxiolk', 1)
			...
			// `openDB('$thwxiolk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'glsk/')) {
