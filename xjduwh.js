const swBase = self.location.pathname.replace(/xjduwh\.js$/, '')

importScripts(swBase + 'mhv/ouvy.bundle.js')
importScripts(swBase + 'mhv/ouvy.config.js')
importScripts(__qdh$config.sw || swBase + 'mhv/ouvy.sw.js')

importScripts(swBase + 'egkm/wbgh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$chngnwpd', 1)
			...
			// `openDB('$chngnwpd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ldbt/')) {
