const swBase = self.location.pathname.replace(/ydyral\.js$/, '')

importScripts(swBase + 'qqp/zseb.bundle.js')
importScripts(swBase + 'qqp/zseb.config.js')
importScripts(__cli$config.sw || swBase + 'qqp/zseb.sw.js')

importScripts(swBase + 'tsip/bazf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$koeooynt', 1)
			...
			// `openDB('$koeooynt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bgzg/')) {
