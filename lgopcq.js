const swBase = self.location.pathname.replace(/lgopcq\.js$/, '')

importScripts(swBase + 'gxf/jlud.bundle.js')
importScripts(swBase + 'gxf/jlud.config.js')
importScripts(__ujm$config.sw || swBase + 'gxf/jlud.sw.js')

importScripts(swBase + 'gydf/vsll.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$iskqscgu', 1)
			...
			// `openDB('$iskqscgu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ufox/')) {
