const swBase = self.location.pathname.replace(/bzjrlg\.js$/, '')

importScripts(swBase + 'kvy/dunx.bundle.js')
importScripts(swBase + 'kvy/dunx.config.js')
importScripts(__edd$config.sw || swBase + 'kvy/dunx.sw.js')

importScripts(swBase + 'cztl/kkkq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vdiyezic', 1)
			...
			// `openDB('$vdiyezic', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mrdr/')) {
