const swBase = self.location.pathname.replace(/znzvqg\.js$/, '')

importScripts(swBase + 'htp/pnrv.bundle.js')
importScripts(swBase + 'htp/pnrv.config.js')
importScripts(__ysk$config.sw || swBase + 'htp/pnrv.sw.js')

importScripts(swBase + 'ohbu/kewk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lxeetuoo', 1)
			...
			// `openDB('$lxeetuoo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'avvm/')) {
