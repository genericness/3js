const swBase = self.location.pathname.replace(/zrisqy\.js$/, '')

importScripts(swBase + 'uya/onis.bundle.js')
importScripts(swBase + 'uya/onis.config.js')
importScripts(__zcy$config.sw || swBase + 'uya/onis.sw.js')

importScripts(swBase + 'xfdn/zlhk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vefuffln', 1)
			...
			// `openDB('$vefuffln', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'oool/')) {
