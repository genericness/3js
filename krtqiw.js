const swBase = self.location.pathname.replace(/krtqiw\.js$/, '')

importScripts(swBase + 'zdm/ghim.bundle.js')
importScripts(swBase + 'zdm/ghim.config.js')
importScripts(__ymt$config.sw || swBase + 'zdm/ghim.sw.js')

importScripts(swBase + 'lzjn/uqub.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kzrwqqif', 1)
			...
			// `openDB('$kzrwqqif', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'orae/')) {
