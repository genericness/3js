const swBase = self.location.pathname.replace(/nnlkqo\.js$/, '')

importScripts(swBase + 'tin/bgtv.bundle.js')
importScripts(swBase + 'tin/bgtv.config.js')
importScripts(__qbn$config.sw || swBase + 'tin/bgtv.sw.js')

importScripts(swBase + 'qcvp/ykxq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qrexsifi', 1)
			...
			// `openDB('$qrexsifi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ywez/')) {
