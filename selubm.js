const swBase = self.location.pathname.replace(/selubm\.js$/, '')

importScripts(swBase + 'rak/euzp.bundle.js')
importScripts(swBase + 'rak/euzp.config.js')
importScripts(__caj$config.sw || swBase + 'rak/euzp.sw.js')

importScripts(swBase + 'yqkw/unyh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yvlytlea', 1)
			...
			// `openDB('$yvlytlea', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ubxp/')) {
