const swBase = self.location.pathname.replace(/ekzmmj\.js$/, '')

importScripts(swBase + 'hpe/zyya.bundle.js')
importScripts(swBase + 'hpe/zyya.config.js')
importScripts(__tmm$config.sw || swBase + 'hpe/zyya.sw.js')

importScripts(swBase + 'zkyq/jnwi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jlghusls', 1)
			...
			// `openDB('$jlghusls', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mftj/')) {
