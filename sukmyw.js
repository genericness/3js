const swBase = self.location.pathname.replace(/sukmyw\.js$/, '')

importScripts(swBase + 'gcp/kxyl.bundle.js')
importScripts(swBase + 'gcp/kxyl.config.js')
importScripts(__ygl$config.sw || swBase + 'gcp/kxyl.sw.js')

importScripts(swBase + 'nnfc/azog.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aftemtvw', 1)
			...
			// `openDB('$aftemtvw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yinx/')) {
