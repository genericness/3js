const swBase = self.location.pathname.replace(/kohcub\.js$/, '')

importScripts(swBase + 'bsx/qhbx.bundle.js')
importScripts(swBase + 'bsx/qhbx.config.js')
importScripts(__ufd$config.sw || swBase + 'bsx/qhbx.sw.js')

importScripts(swBase + 'mhti/awym.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ozfmzuoo', 1)
			...
			// `openDB('$ozfmzuoo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qyfm/')) {
