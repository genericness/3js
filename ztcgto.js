const swBase = self.location.pathname.replace(/ztcgto\.js$/, '')

importScripts(swBase + 'enp/oesn.bundle.js')
importScripts(swBase + 'enp/oesn.config.js')
importScripts(__rdy$config.sw || swBase + 'enp/oesn.sw.js')

importScripts(swBase + 'tude/uolt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pmagdjfc', 1)
			...
			// `openDB('$pmagdjfc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'woov/')) {
