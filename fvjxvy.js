const swBase = self.location.pathname.replace(/fvjxvy\.js$/, '')

importScripts(swBase + 'jrc/nkgd.bundle.js')
importScripts(swBase + 'jrc/nkgd.config.js')
importScripts(__yco$config.sw || swBase + 'jrc/nkgd.sw.js')

importScripts(swBase + 'cshk/hafu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xeappmde', 1)
			...
			// `openDB('$xeappmde', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'urpu/')) {
