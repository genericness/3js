const swBase = self.location.pathname.replace(/xbcdue\.js$/, '')

importScripts(swBase + 'dqe/hwpj.bundle.js')
importScripts(swBase + 'dqe/hwpj.config.js')
importScripts(__irc$config.sw || swBase + 'dqe/hwpj.sw.js')

importScripts(swBase + 'wpjj/zage.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wyfkojvz', 1)
			...
			// `openDB('$wyfkojvz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tcez/')) {
