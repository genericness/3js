const swBase = self.location.pathname.replace(/aeukwb\.js$/, '')

importScripts(swBase + 'jjk/zxqu.bundle.js')
importScripts(swBase + 'jjk/zxqu.config.js')
importScripts(__yby$config.sw || swBase + 'jjk/zxqu.sw.js')

importScripts(swBase + 'rvxx/bywx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lynpqiux', 1)
			...
			// `openDB('$lynpqiux', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mavs/')) {
