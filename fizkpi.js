const swBase = self.location.pathname.replace(/fizkpi\.js$/, '')

importScripts(swBase + 'vug/rsrx.bundle.js')
importScripts(swBase + 'vug/rsrx.config.js')
importScripts(__rmy$config.sw || swBase + 'vug/rsrx.sw.js')

importScripts(swBase + 'ldii/yebx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pwmohaux', 1)
			...
			// `openDB('$pwmohaux', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'quio/')) {
