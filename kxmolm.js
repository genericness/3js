const swBase = self.location.pathname.replace(/kxmolm\.js$/, '')

importScripts(swBase + 'slx/aoie.bundle.js')
importScripts(swBase + 'slx/aoie.config.js')
importScripts(__gqy$config.sw || swBase + 'slx/aoie.sw.js')

importScripts(swBase + 'gdez/ascy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kbxgmekm', 1)
			...
			// `openDB('$kbxgmekm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sqwu/')) {
