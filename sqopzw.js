const swBase = self.location.pathname.replace(/sqopzw\.js$/, '')

importScripts(swBase + 'rgy/dumy.bundle.js')
importScripts(swBase + 'rgy/dumy.config.js')
importScripts(__qjg$config.sw || swBase + 'rgy/dumy.sw.js')

importScripts(swBase + 'ssxk/qcrz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lsmavrhs', 1)
			...
			// `openDB('$lsmavrhs', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jdjf/')) {
