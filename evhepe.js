const swBase = self.location.pathname.replace(/evhepe\.js$/, '')

importScripts(swBase + 'sli/dqoy.bundle.js')
importScripts(swBase + 'sli/dqoy.config.js')
importScripts(__mpe$config.sw || swBase + 'sli/dqoy.sw.js')

importScripts(swBase + 'vqna/dftp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zvcemvoj', 1)
			...
			// `openDB('$zvcemvoj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'spsv/')) {
