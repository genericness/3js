const swBase = self.location.pathname.replace(/etxsop\.js$/, '')

importScripts(swBase + 'oen/vrpe.bundle.js')
importScripts(swBase + 'oen/vrpe.config.js')
importScripts(__ikx$config.sw || swBase + 'oen/vrpe.sw.js')

importScripts(swBase + 'atgr/xsst.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dvwldfbo', 1)
			...
			// `openDB('$dvwldfbo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'puqk/')) {
