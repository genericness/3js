const swBase = self.location.pathname.replace(/dpmsdf\.js$/, '')

importScripts(swBase + 'cru/reho.bundle.js')
importScripts(swBase + 'cru/reho.config.js')
importScripts(__buo$config.sw || swBase + 'cru/reho.sw.js')

importScripts(swBase + 'figc/sfst.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kizbghov', 1)
			...
			// `openDB('$kizbghov', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xgut/')) {
