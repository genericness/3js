const swBase = self.location.pathname.replace(/cqtjvy\.js$/, '')

importScripts(swBase + 'cph/kxqb.bundle.js')
importScripts(swBase + 'cph/kxqb.config.js')
importScripts(__ayw$config.sw || swBase + 'cph/kxqb.sw.js')

importScripts(swBase + 'ukak/hceu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$efsyzyqh', 1)
			...
			// `openDB('$efsyzyqh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pltn/')) {
