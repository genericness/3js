const swBase = self.location.pathname.replace(/mukfai\.js$/, '')

importScripts(swBase + 'vph/qeii.bundle.js')
importScripts(swBase + 'vph/qeii.config.js')
importScripts(__llz$config.sw || swBase + 'vph/qeii.sw.js')

importScripts(swBase + 'hnio/tzhc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cunsxjuy', 1)
			...
			// `openDB('$cunsxjuy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qdqs/')) {
