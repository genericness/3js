const swBase = self.location.pathname.replace(/jftoyb\.js$/, '')

importScripts(swBase + 'gvs/edpc.bundle.js')
importScripts(swBase + 'gvs/edpc.config.js')
importScripts(__uhv$config.sw || swBase + 'gvs/edpc.sw.js')

importScripts(swBase + 'danb/vxyb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vefoudet', 1)
			...
			// `openDB('$vefoudet', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'opta/')) {
