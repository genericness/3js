const swBase = self.location.pathname.replace(/pcmwqw\.js$/, '')

importScripts(swBase + 'mtp/rluo.bundle.js')
importScripts(swBase + 'mtp/rluo.config.js')
importScripts(__gkm$config.sw || swBase + 'mtp/rluo.sw.js')

importScripts(swBase + 'vuyn/xbpt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$asrdisxf', 1)
			...
			// `openDB('$asrdisxf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ykfm/')) {
