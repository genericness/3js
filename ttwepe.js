const swBase = self.location.pathname.replace(/ttwepe\.js$/, '')

importScripts(swBase + 'rnq/wdro.bundle.js')
importScripts(swBase + 'rnq/wdro.config.js')
importScripts(__ali$config.sw || swBase + 'rnq/wdro.sw.js')

importScripts(swBase + 'ibuq/mwxe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$myyvulsu', 1)
			...
			// `openDB('$myyvulsu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wcxb/')) {
