const swBase = self.location.pathname.replace(/xzuzzj\.js$/, '')

importScripts(swBase + 'jgm/fwwr.bundle.js')
importScripts(swBase + 'jgm/fwwr.config.js')
importScripts(__dub$config.sw || swBase + 'jgm/fwwr.sw.js')

importScripts(swBase + 'xour/wuar.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vjtuinqg', 1)
			...
			// `openDB('$vjtuinqg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'apsy/')) {
