const swBase = self.location.pathname.replace(/jsxipv\.js$/, '')

importScripts(swBase + 'qdx/calc.bundle.js')
importScripts(swBase + 'qdx/calc.config.js')
importScripts(__sbh$config.sw || swBase + 'qdx/calc.sw.js')

importScripts(swBase + 'jxxt/ljuv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lmzajbje', 1)
			...
			// `openDB('$lmzajbje', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ctxo/')) {
