const swBase = self.location.pathname.replace(/rmgomh\.js$/, '')

importScripts(swBase + 'xgp/rfep.bundle.js')
importScripts(swBase + 'xgp/rfep.config.js')
importScripts(__mzr$config.sw || swBase + 'xgp/rfep.sw.js')

importScripts(swBase + 'divw/ptvz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ykaeyfky', 1)
			...
			// `openDB('$ykaeyfky', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jozf/')) {
