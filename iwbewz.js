const swBase = self.location.pathname.replace(/iwbewz\.js$/, '')

importScripts(swBase + 'hvw/zcfi.bundle.js')
importScripts(swBase + 'hvw/zcfi.config.js')
importScripts(__dek$config.sw || swBase + 'hvw/zcfi.sw.js')

importScripts(swBase + 'tuck/vpij.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yboswlaq', 1)
			...
			// `openDB('$yboswlaq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pmaf/')) {
