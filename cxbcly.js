const swBase = self.location.pathname.replace(/cxbcly\.js$/, '')

importScripts(swBase + 'fag/esjm.bundle.js')
importScripts(swBase + 'fag/esjm.config.js')
importScripts(__faa$config.sw || swBase + 'fag/esjm.sw.js')

importScripts(swBase + 'oyup/kldc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jkqwybau', 1)
			...
			// `openDB('$jkqwybau', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zwzw/')) {
