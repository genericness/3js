const swBase = self.location.pathname.replace(/culies\.js$/, '')

importScripts(swBase + 'axe/fvip.bundle.js')
importScripts(swBase + 'axe/fvip.config.js')
importScripts(__wlc$config.sw || swBase + 'axe/fvip.sw.js')

importScripts(swBase + 'hjpj/xohg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ppmzwgnv', 1)
			...
			// `openDB('$ppmzwgnv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mpsd/')) {
