const swBase = self.location.pathname.replace(/yspmsb\.js$/, '')

importScripts(swBase + 'aff/sqhs.bundle.js')
importScripts(swBase + 'aff/sqhs.config.js')
importScripts(__rdl$config.sw || swBase + 'aff/sqhs.sw.js')

importScripts(swBase + 'mwyh/oogi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hypoacto', 1)
			...
			// `openDB('$hypoacto', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'aott/')) {
