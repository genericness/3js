const swBase = self.location.pathname.replace(/dgbynf\.js$/, '')

importScripts(swBase + 'rra/yusy.bundle.js')
importScripts(swBase + 'rra/yusy.config.js')
importScripts(__pnf$config.sw || swBase + 'rra/yusy.sw.js')

importScripts(swBase + 'zrfs/hfie.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aoplvelc', 1)
			...
			// `openDB('$aoplvelc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qvls/')) {
