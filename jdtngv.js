const swBase = self.location.pathname.replace(/jdtngv\.js$/, '')

importScripts(swBase + 'voz/toda.bundle.js')
importScripts(swBase + 'voz/toda.config.js')
importScripts(__yhk$config.sw || swBase + 'voz/toda.sw.js')

importScripts(swBase + 'dbmj/crle.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$llcmxtou', 1)
			...
			// `openDB('$llcmxtou', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tghq/')) {
