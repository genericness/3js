const swBase = self.location.pathname.replace(/dsomxl\.js$/, '')

importScripts(swBase + 'fki/oxni.bundle.js')
importScripts(swBase + 'fki/oxni.config.js')
importScripts(__oqo$config.sw || swBase + 'fki/oxni.sw.js')

importScripts(swBase + 'ulww/fgyx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yolfwgdl', 1)
			...
			// `openDB('$yolfwgdl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'enju/')) {
