const swBase = self.location.pathname.replace(/kxngxm\.js$/, '')

importScripts(swBase + 'qar/srnp.bundle.js')
importScripts(swBase + 'qar/srnp.config.js')
importScripts(__rao$config.sw || swBase + 'qar/srnp.sw.js')

importScripts(swBase + 'ffxb/mzbc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vqirllts', 1)
			...
			// `openDB('$vqirllts', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'auze/')) {
