const swBase = self.location.pathname.replace(/fdsbzd\.js$/, '')

importScripts(swBase + 'caz/mzin.bundle.js')
importScripts(swBase + 'caz/mzin.config.js')
importScripts(__qsc$config.sw || swBase + 'caz/mzin.sw.js')

importScripts(swBase + 'zklh/hrgy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vjmqygff', 1)
			...
			// `openDB('$vjmqygff', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'urhs/')) {
