const swBase = self.location.pathname.replace(/cgafxu\.js$/, '')

importScripts(swBase + 'wmp/yjok.bundle.js')
importScripts(swBase + 'wmp/yjok.config.js')
importScripts(__lhm$config.sw || swBase + 'wmp/yjok.sw.js')

importScripts(swBase + 'upbw/wred.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qyeqwecf', 1)
			...
			// `openDB('$qyeqwecf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'paqp/')) {
