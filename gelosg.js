const swBase = self.location.pathname.replace(/gelosg\.js$/, '')

importScripts(swBase + 'nyc/ixud.bundle.js')
importScripts(swBase + 'nyc/ixud.config.js')
importScripts(__khv$config.sw || swBase + 'nyc/ixud.sw.js')

importScripts(swBase + 'fsmf/afek.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$saxejkpn', 1)
			...
			// `openDB('$saxejkpn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'awow/')) {
