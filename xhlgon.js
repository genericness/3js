const swBase = self.location.pathname.replace(/xhlgon\.js$/, '')

importScripts(swBase + 'pgw/twih.bundle.js')
importScripts(swBase + 'pgw/twih.config.js')
importScripts(__dih$config.sw || swBase + 'pgw/twih.sw.js')

importScripts(swBase + 'mssa/lpbs.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zneyvpac', 1)
			...
			// `openDB('$zneyvpac', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gids/')) {
