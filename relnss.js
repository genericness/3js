const swBase = self.location.pathname.replace(/relnss\.js$/, '')

importScripts(swBase + 'ifp/dlzp.bundle.js')
importScripts(swBase + 'ifp/dlzp.config.js')
importScripts(__dkn$config.sw || swBase + 'ifp/dlzp.sw.js')

importScripts(swBase + 'vlpa/pxlk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ezrqikyf', 1)
			...
			// `openDB('$ezrqikyf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uvcq/')) {
