const swBase = self.location.pathname.replace(/frjlqs\.js$/, '')

importScripts(swBase + 'yre/vcok.bundle.js')
importScripts(swBase + 'yre/vcok.config.js')
importScripts(__hwp$config.sw || swBase + 'yre/vcok.sw.js')

importScripts(swBase + 'ppsq/cwuu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pqrmmezh', 1)
			...
			// `openDB('$pqrmmezh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bcjd/')) {
