const swBase = self.location.pathname.replace(/vnfivw\.js$/, '')

importScripts(swBase + 'trj/mupj.bundle.js')
importScripts(swBase + 'trj/mupj.config.js')
importScripts(__doi$config.sw || swBase + 'trj/mupj.sw.js')

importScripts(swBase + 'sozv/dqgq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wswkowbz', 1)
			...
			// `openDB('$wswkowbz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'agfg/')) {
