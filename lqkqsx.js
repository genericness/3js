const swBase = self.location.pathname.replace(/lqkqsx\.js$/, '')

importScripts(swBase + 'vmj/ajlt.bundle.js')
importScripts(swBase + 'vmj/ajlt.config.js')
importScripts(__dja$config.sw || swBase + 'vmj/ajlt.sw.js')

importScripts(swBase + 'pcgu/bais.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$byiorbvz', 1)
			...
			// `openDB('$byiorbvz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ghyj/')) {
