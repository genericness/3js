const swBase = self.location.pathname.replace(/jaieur\.js$/, '')

importScripts(swBase + 'pkf/rnsq.bundle.js')
importScripts(swBase + 'pkf/rnsq.config.js')
importScripts(__zeb$config.sw || swBase + 'pkf/rnsq.sw.js')

importScripts(swBase + 'uigw/dgzn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$edmdkpkj', 1)
			...
			// `openDB('$edmdkpkj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bcyg/')) {
