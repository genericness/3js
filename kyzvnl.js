const swBase = self.location.pathname.replace(/kyzvnl\.js$/, '')

importScripts(swBase + 'ugn/ponl.bundle.js')
importScripts(swBase + 'ugn/ponl.config.js')
importScripts(__udj$config.sw || swBase + 'ugn/ponl.sw.js')

importScripts(swBase + 'tahm/nuhr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wcbjdzdd', 1)
			...
			// `openDB('$wcbjdzdd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bynn/')) {
