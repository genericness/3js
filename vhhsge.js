const swBase = self.location.pathname.replace(/vhhsge\.js$/, '')

importScripts(swBase + 'hne/gtdt.bundle.js')
importScripts(swBase + 'hne/gtdt.config.js')
importScripts(__ggr$config.sw || swBase + 'hne/gtdt.sw.js')

importScripts(swBase + 'igrt/ggjp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ewksqvon', 1)
			...
			// `openDB('$ewksqvon', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nbrp/')) {
