const swBase = self.location.pathname.replace(/iquici\.js$/, '')

importScripts(swBase + 'mtl/nudj.bundle.js')
importScripts(swBase + 'mtl/nudj.config.js')
importScripts(__ffd$config.sw || swBase + 'mtl/nudj.sw.js')

importScripts(swBase + 'vili/xjkp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$igoxtbdk', 1)
			...
			// `openDB('$igoxtbdk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gpjj/')) {
