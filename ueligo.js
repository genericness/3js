const swBase = self.location.pathname.replace(/ueligo\.js$/, '')

importScripts(swBase + 'oyg/muho.bundle.js')
importScripts(swBase + 'oyg/muho.config.js')
importScripts(__exg$config.sw || swBase + 'oyg/muho.sw.js')

importScripts(swBase + 'dcyt/hnmv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ixqngfcx', 1)
			...
			// `openDB('$ixqngfcx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'shfr/')) {
