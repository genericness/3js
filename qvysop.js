const swBase = self.location.pathname.replace(/qvysop\.js$/, '')

importScripts(swBase + 'gcj/ifwc.bundle.js')
importScripts(swBase + 'gcj/ifwc.config.js')
importScripts(__zeb$config.sw || swBase + 'gcj/ifwc.sw.js')

importScripts(swBase + 'uupl/uzrr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$oxpewwzt', 1)
			...
			// `openDB('$oxpewwzt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mmqp/')) {
