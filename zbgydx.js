const swBase = self.location.pathname.replace(/zbgydx\.js$/, '')

importScripts(swBase + 'pij/apsg.bundle.js')
importScripts(swBase + 'pij/apsg.config.js')
importScripts(__shj$config.sw || swBase + 'pij/apsg.sw.js')

importScripts(swBase + 'nvqm/svtv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vpzdwdrf', 1)
			...
			// `openDB('$vpzdwdrf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'roah/')) {
