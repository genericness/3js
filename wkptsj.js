const swBase = self.location.pathname.replace(/wkptsj\.js$/, '')

importScripts(swBase + 'bov/aluk.bundle.js')
importScripts(swBase + 'bov/aluk.config.js')
importScripts(__ypm$config.sw || swBase + 'bov/aluk.sw.js')

importScripts(swBase + 'wxtq/qheu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dxvoiogf', 1)
			...
			// `openDB('$dxvoiogf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'phqc/')) {
