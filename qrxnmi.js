const swBase = self.location.pathname.replace(/qrxnmi\.js$/, '')

importScripts(swBase + 'kxh/uvuu.bundle.js')
importScripts(swBase + 'kxh/uvuu.config.js')
importScripts(__nzz$config.sw || swBase + 'kxh/uvuu.sw.js')

importScripts(swBase + 'fhid/ytti.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ufcdnqin', 1)
			...
			// `openDB('$ufcdnqin', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jimv/')) {
