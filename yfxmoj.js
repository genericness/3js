const swBase = self.location.pathname.replace(/yfxmoj\.js$/, '')

importScripts(swBase + 'wxi/spiv.bundle.js')
importScripts(swBase + 'wxi/spiv.config.js')
importScripts(__glp$config.sw || swBase + 'wxi/spiv.sw.js')

importScripts(swBase + 'aqwi/uirq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nbriojrq', 1)
			...
			// `openDB('$nbriojrq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wfks/')) {
