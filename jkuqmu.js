const swBase = self.location.pathname.replace(/jkuqmu\.js$/, '')

importScripts(swBase + 'tvh/hlsi.bundle.js')
importScripts(swBase + 'tvh/hlsi.config.js')
importScripts(__rso$config.sw || swBase + 'tvh/hlsi.sw.js')

importScripts(swBase + 'wgse/rkal.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fglwpepc', 1)
			...
			// `openDB('$fglwpepc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'orfp/')) {
