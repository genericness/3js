const swBase = self.location.pathname.replace(/dshxme\.js$/, '')

importScripts(swBase + 'wvr/pfuv.bundle.js')
importScripts(swBase + 'wvr/pfuv.config.js')
importScripts(__ogr$config.sw || swBase + 'wvr/pfuv.sw.js')

importScripts(swBase + 'zkrx/ldss.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hdjbpcyw', 1)
			...
			// `openDB('$hdjbpcyw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ybew/')) {
