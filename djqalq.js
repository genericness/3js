const swBase = self.location.pathname.replace(/djqalq\.js$/, '')

importScripts(swBase + 'esf/xejp.bundle.js')
importScripts(swBase + 'esf/xejp.config.js')
importScripts(__ceh$config.sw || swBase + 'esf/xejp.sw.js')

importScripts(swBase + 'slhv/szsy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vzlummic', 1)
			...
			// `openDB('$vzlummic', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dtif/')) {
