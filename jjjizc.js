const swBase = self.location.pathname.replace(/jjjizc\.js$/, '')

importScripts(swBase + 'zze/ipqv.bundle.js')
importScripts(swBase + 'zze/ipqv.config.js')
importScripts(__oyb$config.sw || swBase + 'zze/ipqv.sw.js')

importScripts(swBase + 'bbvk/xyzt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fmrxnqpt', 1)
			...
			// `openDB('$fmrxnqpt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'iixy/')) {
