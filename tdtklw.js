const swBase = self.location.pathname.replace(/tdtklw\.js$/, '')

importScripts(swBase + 'qxz/qrsa.bundle.js')
importScripts(swBase + 'qxz/qrsa.config.js')
importScripts(__exi$config.sw || swBase + 'qxz/qrsa.sw.js')

importScripts(swBase + 'ihce/admc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bxjbynso', 1)
			...
			// `openDB('$bxjbynso', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'duoa/')) {
