const swBase = self.location.pathname.replace(/mdvzgv\.js$/, '')

importScripts(swBase + 'qny/vfmm.bundle.js')
importScripts(swBase + 'qny/vfmm.config.js')
importScripts(__hcl$config.sw || swBase + 'qny/vfmm.sw.js')

importScripts(swBase + 'aolr/rhcl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gralhsls', 1)
			...
			// `openDB('$gralhsls', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xnny/')) {
