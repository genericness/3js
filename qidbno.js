const swBase = self.location.pathname.replace(/qidbno\.js$/, '')

importScripts(swBase + 'fsq/mxoh.bundle.js')
importScripts(swBase + 'fsq/mxoh.config.js')
importScripts(__orz$config.sw || swBase + 'fsq/mxoh.sw.js')

importScripts(swBase + 'etaa/uiin.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nascacga', 1)
			...
			// `openDB('$nascacga', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'chwt/')) {
