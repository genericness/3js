const swBase = self.location.pathname.replace(/xpmznu\.js$/, '')

importScripts(swBase + 'nri/ovbx.bundle.js')
importScripts(swBase + 'nri/ovbx.config.js')
importScripts(__prc$config.sw || swBase + 'nri/ovbx.sw.js')

importScripts(swBase + 'fxdd/fdwb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$udplskpr', 1)
			...
			// `openDB('$udplskpr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hoou/')) {
