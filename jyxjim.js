const swBase = self.location.pathname.replace(/jyxjim\.js$/, '')

importScripts(swBase + 'qtz/zqfc.bundle.js')
importScripts(swBase + 'qtz/zqfc.config.js')
importScripts(__odn$config.sw || swBase + 'qtz/zqfc.sw.js')

importScripts(swBase + 'egkc/yxsw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kfvmwurx', 1)
			...
			// `openDB('$kfvmwurx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dkbd/')) {
