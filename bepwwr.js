const swBase = self.location.pathname.replace(/bepwwr\.js$/, '')

importScripts(swBase + 'ujo/opwl.bundle.js')
importScripts(swBase + 'ujo/opwl.config.js')
importScripts(__khn$config.sw || swBase + 'ujo/opwl.sw.js')

importScripts(swBase + 'vpib/qwjq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yakfhxoy', 1)
			...
			// `openDB('$yakfhxoy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nqjn/')) {
