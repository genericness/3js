const swBase = self.location.pathname.replace(/bdlcaf\.js$/, '')

importScripts(swBase + 'ehp/zamd.bundle.js')
importScripts(swBase + 'ehp/zamd.config.js')
importScripts(__bev$config.sw || swBase + 'ehp/zamd.sw.js')

importScripts(swBase + 'rxtr/mbej.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jgidpdnm', 1)
			...
			// `openDB('$jgidpdnm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'clfg/')) {
