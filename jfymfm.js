const swBase = self.location.pathname.replace(/jfymfm\.js$/, '')

importScripts(swBase + 'gvy/jicu.bundle.js')
importScripts(swBase + 'gvy/jicu.config.js')
importScripts(__dha$config.sw || swBase + 'gvy/jicu.sw.js')

importScripts(swBase + 'mopg/kbos.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$urgzofgb', 1)
			...
			// `openDB('$urgzofgb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sgij/')) {
