const swBase = self.location.pathname.replace(/aohslf\.js$/, '')

importScripts(swBase + 'xsd/txjb.bundle.js')
importScripts(swBase + 'xsd/txjb.config.js')
importScripts(__qqi$config.sw || swBase + 'xsd/txjb.sw.js')

importScripts(swBase + 'pkyg/dubb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xablhzzm', 1)
			...
			// `openDB('$xablhzzm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mioa/')) {
