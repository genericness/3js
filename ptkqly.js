const swBase = self.location.pathname.replace(/ptkqly\.js$/, '')

importScripts(swBase + 'doi/uvze.bundle.js')
importScripts(swBase + 'doi/uvze.config.js')
importScripts(__seb$config.sw || swBase + 'doi/uvze.sw.js')

importScripts(swBase + 'jolg/hefe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jzwxdcjt', 1)
			...
			// `openDB('$jzwxdcjt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gihj/')) {
