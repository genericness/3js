const swBase = self.location.pathname.replace(/euxarq\.js$/, '')

importScripts(swBase + 'ebf/xxqq.bundle.js')
importScripts(swBase + 'ebf/xxqq.config.js')
importScripts(__mpe$config.sw || swBase + 'ebf/xxqq.sw.js')

importScripts(swBase + 'zlbe/evmj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dtppyypb', 1)
			...
			// `openDB('$dtppyypb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vlmf/')) {
