const swBase = self.location.pathname.replace(/zyopzl\.js$/, '')

importScripts(swBase + 'qsi/fpxa.bundle.js')
importScripts(swBase + 'qsi/fpxa.config.js')
importScripts(__wxv$config.sw || swBase + 'qsi/fpxa.sw.js')

importScripts(swBase + 'zgsq/fhqz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xktaggrn', 1)
			...
			// `openDB('$xktaggrn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ewji/')) {
