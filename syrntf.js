const swBase = self.location.pathname.replace(/syrntf\.js$/, '')

importScripts(swBase + 'qfh/wyaw.bundle.js')
importScripts(swBase + 'qfh/wyaw.config.js')
importScripts(__tmw$config.sw || swBase + 'qfh/wyaw.sw.js')

importScripts(swBase + 'xigl/dvmb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$oqqoyqxk', 1)
			...
			// `openDB('$oqqoyqxk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ssks/')) {
