const swBase = self.location.pathname.replace(/zcuikq\.js$/, '')

importScripts(swBase + 'nmz/aktk.bundle.js')
importScripts(swBase + 'nmz/aktk.config.js')
importScripts(__fso$config.sw || swBase + 'nmz/aktk.sw.js')

importScripts(swBase + 'asda/fasq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nnvvibcq', 1)
			...
			// `openDB('$nnvvibcq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'exew/')) {
