const swBase = self.location.pathname.replace(/gicqpa\.js$/, '')

importScripts(swBase + 'ucx/wjmc.bundle.js')
importScripts(swBase + 'ucx/wjmc.config.js')
importScripts(__nfi$config.sw || swBase + 'ucx/wjmc.sw.js')

importScripts(swBase + 'xiiu/qkjx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lruaqmxj', 1)
			...
			// `openDB('$lruaqmxj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kkcg/')) {
