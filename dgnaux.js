const swBase = self.location.pathname.replace(/dgnaux\.js$/, '')

importScripts(swBase + 'kxy/tquw.bundle.js')
importScripts(swBase + 'kxy/tquw.config.js')
importScripts(__xov$config.sw || swBase + 'kxy/tquw.sw.js')

importScripts(swBase + 'wyqa/ggsi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jjoncnvn', 1)
			...
			// `openDB('$jjoncnvn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ywrd/')) {
