const swBase = self.location.pathname.replace(/xpfpov\.js$/, '')

importScripts(swBase + 'bxv/mpfr.bundle.js')
importScripts(swBase + 'bxv/mpfr.config.js')
importScripts(__cmm$config.sw || swBase + 'bxv/mpfr.sw.js')

importScripts(swBase + 'djzb/ovyy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wguvodwd', 1)
			...
			// `openDB('$wguvodwd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'letz/')) {
