const swBase = self.location.pathname.replace(/nllyqk\.js$/, '')

importScripts(swBase + 'qby/qlrj.bundle.js')
importScripts(swBase + 'qby/qlrj.config.js')
importScripts(__plu$config.sw || swBase + 'qby/qlrj.sw.js')

importScripts(swBase + 'ihfa/bnyw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xokjixbh', 1)
			...
			// `openDB('$xokjixbh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'goty/')) {
