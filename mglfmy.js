const swBase = self.location.pathname.replace(/mglfmy\.js$/, '')

importScripts(swBase + 'xzs/xjvy.bundle.js')
importScripts(swBase + 'xzs/xjvy.config.js')
importScripts(__iwy$config.sw || swBase + 'xzs/xjvy.sw.js')

importScripts(swBase + 'gikz/utoj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rugwdjoh', 1)
			...
			// `openDB('$rugwdjoh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xamm/')) {
