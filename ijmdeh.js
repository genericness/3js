const swBase = self.location.pathname.replace(/ijmdeh\.js$/, '')

importScripts(swBase + 'gxg/wwyg.bundle.js')
importScripts(swBase + 'gxg/wwyg.config.js')
importScripts(__ntm$config.sw || swBase + 'gxg/wwyg.sw.js')

importScripts(swBase + 'orgc/bcie.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$drnloouz', 1)
			...
			// `openDB('$drnloouz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jjkn/')) {
