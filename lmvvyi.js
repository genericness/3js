const swBase = self.location.pathname.replace(/lmvvyi\.js$/, '')

importScripts(swBase + 'cns/xcmo.bundle.js')
importScripts(swBase + 'cns/xcmo.config.js')
importScripts(__fbz$config.sw || swBase + 'cns/xcmo.sw.js')

importScripts(swBase + 'vyis/iomz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wvihkxza', 1)
			...
			// `openDB('$wvihkxza', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uafy/')) {
