const swBase = self.location.pathname.replace(/rqrcbb\.js$/, '')

importScripts(swBase + 'bhx/rtbm.bundle.js')
importScripts(swBase + 'bhx/rtbm.config.js')
importScripts(__etm$config.sw || swBase + 'bhx/rtbm.sw.js')

importScripts(swBase + 'edvi/odvl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$eqlsgwsa', 1)
			...
			// `openDB('$eqlsgwsa', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sbrz/')) {
