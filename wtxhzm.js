const swBase = self.location.pathname.replace(/wtxhzm\.js$/, '')

importScripts(swBase + 'ebx/vrdr.bundle.js')
importScripts(swBase + 'ebx/vrdr.config.js')
importScripts(__tpi$config.sw || swBase + 'ebx/vrdr.sw.js')

importScripts(swBase + 'wihw/tqyb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ouwcrmnb', 1)
			...
			// `openDB('$ouwcrmnb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uzcg/')) {
