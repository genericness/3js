const swBase = self.location.pathname.replace(/qxlisc\.js$/, '')

importScripts(swBase + 'hnl/xjwq.bundle.js')
importScripts(swBase + 'hnl/xjwq.config.js')
importScripts(__beb$config.sw || swBase + 'hnl/xjwq.sw.js')

importScripts(swBase + 'yaxf/lyaf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lqjuiekf', 1)
			...
			// `openDB('$lqjuiekf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'elsd/')) {
