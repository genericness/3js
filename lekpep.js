const swBase = self.location.pathname.replace(/lekpep\.js$/, '')

importScripts(swBase + 'seh/rixa.bundle.js')
importScripts(swBase + 'seh/rixa.config.js')
importScripts(__vlt$config.sw || swBase + 'seh/rixa.sw.js')

importScripts(swBase + 'tumf/wsus.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yujbhnyq', 1)
			...
			// `openDB('$yujbhnyq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wxeo/')) {
