const swBase = self.location.pathname.replace(/zkjvzl\.js$/, '')

importScripts(swBase + 'hrp/kweu.bundle.js')
importScripts(swBase + 'hrp/kweu.config.js')
importScripts(__mfc$config.sw || swBase + 'hrp/kweu.sw.js')

importScripts(swBase + 'sfbs/jxwp.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sbnwspqz', 1)
			...
			// `openDB('$sbnwspqz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yeuh/')) {
