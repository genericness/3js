const swBase = self.location.pathname.replace(/ukrqgm\.js$/, '')

importScripts(swBase + 'khg/rmgd.bundle.js')
importScripts(swBase + 'khg/rmgd.config.js')
importScripts(__spk$config.sw || swBase + 'khg/rmgd.sw.js')

importScripts(swBase + 'ijur/zlzw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ygooypuu', 1)
			...
			// `openDB('$ygooypuu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wmom/')) {
