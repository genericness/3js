const swBase = self.location.pathname.replace(/isrwmc\.js$/, '')

importScripts(swBase + 'mzx/hmvs.bundle.js')
importScripts(swBase + 'mzx/hmvs.config.js')
importScripts(__xou$config.sw || swBase + 'mzx/hmvs.sw.js')

importScripts(swBase + 'xhfi/ldoe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$saxzgwkb', 1)
			...
			// `openDB('$saxzgwkb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vlpc/')) {
