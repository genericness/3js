const swBase = self.location.pathname.replace(/psffxz\.js$/, '')

importScripts(swBase + 'lvr/dpcm.bundle.js')
importScripts(swBase + 'lvr/dpcm.config.js')
importScripts(__zhs$config.sw || swBase + 'lvr/dpcm.sw.js')

importScripts(swBase + 'rmvu/jont.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jyzclexm', 1)
			...
			// `openDB('$jyzclexm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hijo/')) {
