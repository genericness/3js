const swBase = self.location.pathname.replace(/nkllxv\.js$/, '')

importScripts(swBase + 'imy/jwss.bundle.js')
importScripts(swBase + 'imy/jwss.config.js')
importScripts(__luy$config.sw || swBase + 'imy/jwss.sw.js')

importScripts(swBase + 'obma/dshn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$orvtmblv', 1)
			...
			// `openDB('$orvtmblv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wvbm/')) {
