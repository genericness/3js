const swBase = self.location.pathname.replace(/gqzamq\.js$/, '')

importScripts(swBase + 'sve/uuzx.bundle.js')
importScripts(swBase + 'sve/uuzx.config.js')
importScripts(__noy$config.sw || swBase + 'sve/uuzx.sw.js')

importScripts(swBase + 'jgim/fjcf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cggxbkbm', 1)
			...
			// `openDB('$cggxbkbm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vyzj/')) {
