const swBase = self.location.pathname.replace(/paeaid\.js$/, '')

importScripts(swBase + 'qiw/qkes.bundle.js')
importScripts(swBase + 'qiw/qkes.config.js')
importScripts(__bgt$config.sw || swBase + 'qiw/qkes.sw.js')

importScripts(swBase + 'pqoq/juyz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aooaltrf', 1)
			...
			// `openDB('$aooaltrf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tatz/')) {
