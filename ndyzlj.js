const swBase = self.location.pathname.replace(/ndyzlj\.js$/, '')

importScripts(swBase + 'ong/lpgv.bundle.js')
importScripts(swBase + 'ong/lpgv.config.js')
importScripts(__crz$config.sw || swBase + 'ong/lpgv.sw.js')

importScripts(swBase + 'cvji/ziuu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jvkxwvsw', 1)
			...
			// `openDB('$jvkxwvsw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jxuy/')) {
