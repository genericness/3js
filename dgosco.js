const swBase = self.location.pathname.replace(/dgosco\.js$/, '')

importScripts(swBase + 'ctd/quqs.bundle.js')
importScripts(swBase + 'ctd/quqs.config.js')
importScripts(__wgy$config.sw || swBase + 'ctd/quqs.sw.js')

importScripts(swBase + 'nqid/ozjt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ijhobsed', 1)
			...
			// `openDB('$ijhobsed', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pjkw/')) {
