const swBase = self.location.pathname.replace(/uefymw\.js$/, '')

importScripts(swBase + 'sgh/bexx.bundle.js')
importScripts(swBase + 'sgh/bexx.config.js')
importScripts(__tkr$config.sw || swBase + 'sgh/bexx.sw.js')

importScripts(swBase + 'krcf/wpca.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lrpwbcjq', 1)
			...
			// `openDB('$lrpwbcjq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'azet/')) {
