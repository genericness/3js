const swBase = self.location.pathname.replace(/kvzhoz\.js$/, '')

importScripts(swBase + 'skg/fvij.bundle.js')
importScripts(swBase + 'skg/fvij.config.js')
importScripts(__nhy$config.sw || swBase + 'skg/fvij.sw.js')

importScripts(swBase + 'vuni/pvkt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$znbyebnn', 1)
			...
			// `openDB('$znbyebnn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'omvd/')) {
