const swBase = self.location.pathname.replace(/accicp\.js$/, '')

importScripts(swBase + 'ver/xzoo.bundle.js')
importScripts(swBase + 'ver/xzoo.config.js')
importScripts(__frn$config.sw || swBase + 'ver/xzoo.sw.js')

importScripts(swBase + 'njlx/otzr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zwfptesr', 1)
			...
			// `openDB('$zwfptesr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gixq/')) {
