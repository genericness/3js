const swBase = self.location.pathname.replace(/yieuqe\.js$/, '')

importScripts(swBase + 'pyz/jaww.bundle.js')
importScripts(swBase + 'pyz/jaww.config.js')
importScripts(__aqm$config.sw || swBase + 'pyz/jaww.sw.js')

importScripts(swBase + 'xuei/ohvx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qctjllco', 1)
			...
			// `openDB('$qctjllco', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wjsh/')) {
