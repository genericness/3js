const swBase = self.location.pathname.replace(/tltzve\.js$/, '')

importScripts(swBase + 'oxu/iwhh.bundle.js')
importScripts(swBase + 'oxu/iwhh.config.js')
importScripts(__jlf$config.sw || swBase + 'oxu/iwhh.sw.js')

importScripts(swBase + 'qjuw/pqpe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mrnfmvxy', 1)
			...
			// `openDB('$mrnfmvxy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zudt/')) {
