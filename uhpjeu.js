const swBase = self.location.pathname.replace(/uhpjeu\.js$/, '')

importScripts(swBase + 'xtw/lowr.bundle.js')
importScripts(swBase + 'xtw/lowr.config.js')
importScripts(__iqa$config.sw || swBase + 'xtw/lowr.sw.js')

importScripts(swBase + 'ixxz/pmnr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ukmmkyik', 1)
			...
			// `openDB('$ukmmkyik', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'czwu/')) {
