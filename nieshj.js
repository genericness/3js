const swBase = self.location.pathname.replace(/nieshj\.js$/, '')

importScripts(swBase + 'ypx/xkht.bundle.js')
importScripts(swBase + 'ypx/xkht.config.js')
importScripts(__ixk$config.sw || swBase + 'ypx/xkht.sw.js')

importScripts(swBase + 'fsdb/zndn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fsfntjgf', 1)
			...
			// `openDB('$fsfntjgf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ngsw/')) {
