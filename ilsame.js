const swBase = self.location.pathname.replace(/ilsame\.js$/, '')

importScripts(swBase + 'ebe/xngl.bundle.js')
importScripts(swBase + 'ebe/xngl.config.js')
importScripts(__lzn$config.sw || swBase + 'ebe/xngl.sw.js')

importScripts(swBase + 'ypav/whfo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$giitkldl', 1)
			...
			// `openDB('$giitkldl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'sebe/')) {
