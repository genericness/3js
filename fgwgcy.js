const swBase = self.location.pathname.replace(/fgwgcy\.js$/, '')

importScripts(swBase + 'sie/jcrq.bundle.js')
importScripts(swBase + 'sie/jcrq.config.js')
importScripts(__jjq$config.sw || swBase + 'sie/jcrq.sw.js')

importScripts(swBase + 'pnhy/egzv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kfhftepf', 1)
			...
			// `openDB('$kfhftepf', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ccmj/')) {
