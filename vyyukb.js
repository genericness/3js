const swBase = self.location.pathname.replace(/vyyukb\.js$/, '')

importScripts(swBase + 'tvz/uole.bundle.js')
importScripts(swBase + 'tvz/uole.config.js')
importScripts(__oqw$config.sw || swBase + 'tvz/uole.sw.js')

importScripts(swBase + 'olxp/wxgr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ycasljmj', 1)
			...
			// `openDB('$ycasljmj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ikaw/')) {
