const swBase = self.location.pathname.replace(/qhelvm\.js$/, '')

importScripts(swBase + 'mzq/txhi.bundle.js')
importScripts(swBase + 'mzq/txhi.config.js')
importScripts(__ktg$config.sw || swBase + 'mzq/txhi.sw.js')

importScripts(swBase + 'wcds/sjpd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ukuzjozn', 1)
			...
			// `openDB('$ukuzjozn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fabm/')) {
