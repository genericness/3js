const swBase = self.location.pathname.replace(/plrcoy\.js$/, '')

importScripts(swBase + 'tjt/qktp.bundle.js')
importScripts(swBase + 'tjt/qktp.config.js')
importScripts(__xtn$config.sw || swBase + 'tjt/qktp.sw.js')

importScripts(swBase + 'gxnk/iclc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$azaezvnw', 1)
			...
			// `openDB('$azaezvnw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fxgi/')) {
