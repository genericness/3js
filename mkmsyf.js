const swBase = self.location.pathname.replace(/mkmsyf\.js$/, '')

importScripts(swBase + 'tcd/tfsf.bundle.js')
importScripts(swBase + 'tcd/tfsf.config.js')
importScripts(__ukd$config.sw || swBase + 'tcd/tfsf.sw.js')

importScripts(swBase + 'zkia/pejn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fkibkptj', 1)
			...
			// `openDB('$fkibkptj', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zjuj/')) {
