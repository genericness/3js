const swBase = self.location.pathname.replace(/chwfsr\.js$/, '')

importScripts(swBase + 'oxo/qilo.bundle.js')
importScripts(swBase + 'oxo/qilo.config.js')
importScripts(__lvz$config.sw || swBase + 'oxo/qilo.sw.js')

importScripts(swBase + 'dwjw/cbsx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hugavkyo', 1)
			...
			// `openDB('$hugavkyo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xevt/')) {
