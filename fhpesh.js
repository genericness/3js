const swBase = self.location.pathname.replace(/fhpesh\.js$/, '')

importScripts(swBase + 'wob/qsgr.bundle.js')
importScripts(swBase + 'wob/qsgr.config.js')
importScripts(__ujz$config.sw || swBase + 'wob/qsgr.sw.js')

importScripts(swBase + 'jphw/yivo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ghanuizd', 1)
			...
			// `openDB('$ghanuizd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lrsg/')) {
