const swBase = self.location.pathname.replace(/wnzlnc\.js$/, '')

importScripts(swBase + 'xkj/ltyj.bundle.js')
importScripts(swBase + 'xkj/ltyj.config.js')
importScripts(__ftv$config.sw || swBase + 'xkj/ltyj.sw.js')

importScripts(swBase + 'awjd/rybe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sdbtveua', 1)
			...
			// `openDB('$sdbtveua', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ldnc/')) {
