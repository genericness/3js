const swBase = self.location.pathname.replace(/xvhfbc\.js$/, '')

importScripts(swBase + 'ezn/ibek.bundle.js')
importScripts(swBase + 'ezn/ibek.config.js')
importScripts(__wwe$config.sw || swBase + 'ezn/ibek.sw.js')

importScripts(swBase + 'zfqd/yhfw.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sjiyuupc', 1)
			...
			// `openDB('$sjiyuupc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'oaao/')) {
