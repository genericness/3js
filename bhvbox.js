const swBase = self.location.pathname.replace(/bhvbox\.js$/, '')

importScripts(swBase + 'oor/fxdh.bundle.js')
importScripts(swBase + 'oor/fxdh.config.js')
importScripts(__ovc$config.sw || swBase + 'oor/fxdh.sw.js')

importScripts(swBase + 'uatb/ncxf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fqtsxhcz', 1)
			...
			// `openDB('$fqtsxhcz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ivsx/')) {
