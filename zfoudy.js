const swBase = self.location.pathname.replace(/zfoudy\.js$/, '')

importScripts(swBase + 'hip/ihhh.bundle.js')
importScripts(swBase + 'hip/ihhh.config.js')
importScripts(__rhx$config.sw || swBase + 'hip/ihhh.sw.js')

importScripts(swBase + 'smhq/wurl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yftmknws', 1)
			...
			// `openDB('$yftmknws', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'huqc/')) {
