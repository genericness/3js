const swBase = self.location.pathname.replace(/klgdyk\.js$/, '')

importScripts(swBase + 'kix/bswe.bundle.js')
importScripts(swBase + 'kix/bswe.config.js')
importScripts(__xhk$config.sw || swBase + 'kix/bswe.sw.js')

importScripts(swBase + 'wbyf/fnjo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vsqdprml', 1)
			...
			// `openDB('$vsqdprml', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xayp/')) {
