const swBase = self.location.pathname.replace(/blmhbp\.js$/, '')

importScripts(swBase + 'dxa/llld.bundle.js')
importScripts(swBase + 'dxa/llld.config.js')
importScripts(__wnc$config.sw || swBase + 'dxa/llld.sw.js')

importScripts(swBase + 'jrcw/mvuv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$qpeacpoi', 1)
			...
			// `openDB('$qpeacpoi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xkcb/')) {
