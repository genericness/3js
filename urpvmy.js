const swBase = self.location.pathname.replace(/urpvmy\.js$/, '')

importScripts(swBase + 'mlf/senv.bundle.js')
importScripts(swBase + 'mlf/senv.config.js')
importScripts(__fqa$config.sw || swBase + 'mlf/senv.sw.js')

importScripts(swBase + 'ppww/iept.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fdoxxipz', 1)
			...
			// `openDB('$fdoxxipz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gwxe/')) {
