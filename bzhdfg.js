const swBase = self.location.pathname.replace(/bzhdfg\.js$/, '')

importScripts(swBase + 'pbp/rbnp.bundle.js')
importScripts(swBase + 'pbp/rbnp.config.js')
importScripts(__ctv$config.sw || swBase + 'pbp/rbnp.sw.js')

importScripts(swBase + 'jugo/oxrt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$azyyfryk', 1)
			...
			// `openDB('$azyyfryk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ljuu/')) {
