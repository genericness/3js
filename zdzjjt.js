const swBase = self.location.pathname.replace(/zdzjjt\.js$/, '')

importScripts(swBase + 'mjh/mhlk.bundle.js')
importScripts(swBase + 'mjh/mhlk.config.js')
importScripts(__nqg$config.sw || swBase + 'mjh/mhlk.sw.js')

importScripts(swBase + 'yggo/jhnn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ckddiepz', 1)
			...
			// `openDB('$ckddiepz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bfvi/')) {
