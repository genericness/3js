const swBase = self.location.pathname.replace(/hquxbh\.js$/, '')

importScripts(swBase + 'omt/ykne.bundle.js')
importScripts(swBase + 'omt/ykne.config.js')
importScripts(__cqa$config.sw || swBase + 'omt/ykne.sw.js')

importScripts(swBase + 'qchs/srou.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$oblktneb', 1)
			...
			// `openDB('$oblktneb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wpqc/')) {
