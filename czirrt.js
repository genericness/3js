const swBase = self.location.pathname.replace(/czirrt\.js$/, '')

importScripts(swBase + 'yjr/kzht.bundle.js')
importScripts(swBase + 'yjr/kzht.config.js')
importScripts(__mdp$config.sw || swBase + 'yjr/kzht.sw.js')

importScripts(swBase + 'nxsq/eixj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bbcmnjwt', 1)
			...
			// `openDB('$bbcmnjwt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mwxv/')) {
