const swBase = self.location.pathname.replace(/bhebyt\.js$/, '')

importScripts(swBase + 'zvp/ipbj.bundle.js')
importScripts(swBase + 'zvp/ipbj.config.js')
importScripts(__erp$config.sw || swBase + 'zvp/ipbj.sw.js')

importScripts(swBase + 'wggi/jibj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rwwsckrb', 1)
			...
			// `openDB('$rwwsckrb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'bmgi/')) {
