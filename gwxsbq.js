const swBase = self.location.pathname.replace(/gwxsbq\.js$/, '')

importScripts(swBase + 'nax/cdsi.bundle.js')
importScripts(swBase + 'nax/cdsi.config.js')
importScripts(__tfz$config.sw || swBase + 'nax/cdsi.sw.js')

importScripts(swBase + 'twia/byxh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vqwakmtv', 1)
			...
			// `openDB('$vqwakmtv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ihyt/')) {
