const swBase = self.location.pathname.replace(/mdizls\.js$/, '')

importScripts(swBase + 'ajk/pcyu.bundle.js')
importScripts(swBase + 'ajk/pcyu.config.js')
importScripts(__irt$config.sw || swBase + 'ajk/pcyu.sw.js')

importScripts(swBase + 'gyat/qqwu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$irkrgfiv', 1)
			...
			// `openDB('$irkrgfiv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rgrx/')) {
