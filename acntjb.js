const swBase = self.location.pathname.replace(/acntjb\.js$/, '')

importScripts(swBase + 'rtt/xlcp.bundle.js')
importScripts(swBase + 'rtt/xlcp.config.js')
importScripts(__jwp$config.sw || swBase + 'rtt/xlcp.sw.js')

importScripts(swBase + 'dkyc/qphn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$oknfdrqs', 1)
			...
			// `openDB('$oknfdrqs', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'javf/')) {
