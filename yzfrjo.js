const swBase = self.location.pathname.replace(/yzfrjo\.js$/, '')

importScripts(swBase + 'tnd/umhw.bundle.js')
importScripts(swBase + 'tnd/umhw.config.js')
importScripts(__evm$config.sw || swBase + 'tnd/umhw.sw.js')

importScripts(swBase + 'clbs/dwdj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$thswtxqn', 1)
			...
			// `openDB('$thswtxqn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ktxu/')) {
