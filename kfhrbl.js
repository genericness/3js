const swBase = self.location.pathname.replace(/kfhrbl\.js$/, '')

importScripts(swBase + 'nqk/lrck.bundle.js')
importScripts(swBase + 'nqk/lrck.config.js')
importScripts(__fhe$config.sw || swBase + 'nqk/lrck.sw.js')

importScripts(swBase + 'mwuq/jjfl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fekanmqy', 1)
			...
			// `openDB('$fekanmqy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nest/')) {
