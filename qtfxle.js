const swBase = self.location.pathname.replace(/qtfxle\.js$/, '')

importScripts(swBase + 'rkn/lhbk.bundle.js')
importScripts(swBase + 'rkn/lhbk.config.js')
importScripts(__umt$config.sw || swBase + 'rkn/lhbk.sw.js')

importScripts(swBase + 'zlpx/pbhk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xabkejqr', 1)
			...
			// `openDB('$xabkejqr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rbtq/')) {
