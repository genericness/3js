const swBase = self.location.pathname.replace(/nevqkc\.js$/, '')

importScripts(swBase + 'fmy/qgmi.bundle.js')
importScripts(swBase + 'fmy/qgmi.config.js')
importScripts(__jvd$config.sw || swBase + 'fmy/qgmi.sw.js')

importScripts(swBase + 'pkho/mozj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pkjsfxbl', 1)
			...
			// `openDB('$pkjsfxbl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yqka/')) {
