const swBase = self.location.pathname.replace(/klcqba\.js$/, '')

importScripts(swBase + 'vco/ramm.bundle.js')
importScripts(swBase + 'vco/ramm.config.js')
importScripts(__nwx$config.sw || swBase + 'vco/ramm.sw.js')

importScripts(swBase + 'vhdt/mqax.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$stczczur', 1)
			...
			// `openDB('$stczczur', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rqqr/')) {
