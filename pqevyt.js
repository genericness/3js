const swBase = self.location.pathname.replace(/pqevyt\.js$/, '')

importScripts(swBase + 'rug/lzam.bundle.js')
importScripts(swBase + 'rug/lzam.config.js')
importScripts(__zey$config.sw || swBase + 'rug/lzam.sw.js')

importScripts(swBase + 'qywb/udbn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ugtpzaxp', 1)
			...
			// `openDB('$ugtpzaxp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xmgh/')) {
