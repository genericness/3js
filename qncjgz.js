const swBase = self.location.pathname.replace(/qncjgz\.js$/, '')

importScripts(swBase + 'pdt/qiel.bundle.js')
importScripts(swBase + 'pdt/qiel.config.js')
importScripts(__vcn$config.sw || swBase + 'pdt/qiel.sw.js')

importScripts(swBase + 'whsa/utuq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wohrpgvx', 1)
			...
			// `openDB('$wohrpgvx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vpiw/')) {
