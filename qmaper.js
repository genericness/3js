const swBase = self.location.pathname.replace(/qmaper\.js$/, '')

importScripts(swBase + 'kid/ippr.bundle.js')
importScripts(swBase + 'kid/ippr.config.js')
importScripts(__gos$config.sw || swBase + 'kid/ippr.sw.js')

importScripts(swBase + 'fmcb/qmje.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mblncjiv', 1)
			...
			// `openDB('$mblncjiv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pwip/')) {
