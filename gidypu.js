const swBase = self.location.pathname.replace(/gidypu\.js$/, '')

importScripts(swBase + 'hce/sdzp.bundle.js')
importScripts(swBase + 'hce/sdzp.config.js')
importScripts(__ace$config.sw || swBase + 'hce/sdzp.sw.js')

importScripts(swBase + 'gofn/rhfy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$eqadmzxi', 1)
			...
			// `openDB('$eqadmzxi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zdws/')) {
