const swBase = self.location.pathname.replace(/hehrif\.js$/, '')

importScripts(swBase + 'sad/ngsi.bundle.js')
importScripts(swBase + 'sad/ngsi.config.js')
importScripts(__uis$config.sw || swBase + 'sad/ngsi.sw.js')

importScripts(swBase + 'xcje/iixr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rzwrkdgv', 1)
			...
			// `openDB('$rzwrkdgv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ckaz/')) {
