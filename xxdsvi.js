const swBase = self.location.pathname.replace(/xxdsvi\.js$/, '')

importScripts(swBase + 'djg/hnrl.bundle.js')
importScripts(swBase + 'djg/hnrl.config.js')
importScripts(__jfz$config.sw || swBase + 'djg/hnrl.sw.js')

importScripts(swBase + 'upil/pthi.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zdvojtwm', 1)
			...
			// `openDB('$zdvojtwm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nkyx/')) {
