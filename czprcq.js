const swBase = self.location.pathname.replace(/czprcq\.js$/, '')

importScripts(swBase + 'mxm/qycr.bundle.js')
importScripts(swBase + 'mxm/qycr.config.js')
importScripts(__lik$config.sw || swBase + 'mxm/qycr.sw.js')

importScripts(swBase + 'dkch/hdie.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pxqgpcyk', 1)
			...
			// `openDB('$pxqgpcyk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yqfm/')) {
