const swBase = self.location.pathname.replace(/xttefd\.js$/, '')

importScripts(swBase + 'cxd/hkck.bundle.js')
importScripts(swBase + 'cxd/hkck.config.js')
importScripts(__agd$config.sw || swBase + 'cxd/hkck.sw.js')

importScripts(swBase + 'psii/sioc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ghwmgqec', 1)
			...
			// `openDB('$ghwmgqec', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xued/')) {
