const swBase = self.location.pathname.replace(/liewpr\.js$/, '')

importScripts(swBase + 'nno/ydbe.bundle.js')
importScripts(swBase + 'nno/ydbe.config.js')
importScripts(__xyq$config.sw || swBase + 'nno/ydbe.sw.js')

importScripts(swBase + 'phqd/genk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ahkqcgqn', 1)
			...
			// `openDB('$ahkqcgqn', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pzin/')) {
