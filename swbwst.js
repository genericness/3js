const swBase = self.location.pathname.replace(/swbwst\.js$/, '')

importScripts(swBase + 'zey/ginx.bundle.js')
importScripts(swBase + 'zey/ginx.config.js')
importScripts(__tes$config.sw || swBase + 'zey/ginx.sw.js')

importScripts(swBase + 'damb/yrxz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$upvsecib', 1)
			...
			// `openDB('$upvsecib', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yjky/')) {
