const swBase = self.location.pathname.replace(/fpkokz\.js$/, '')

importScripts(swBase + 'pxp/zidh.bundle.js')
importScripts(swBase + 'pxp/zidh.config.js')
importScripts(__gek$config.sw || swBase + 'pxp/zidh.sw.js')

importScripts(swBase + 'jdpt/jvhb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$stykknta', 1)
			...
			// `openDB('$stykknta', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fxns/')) {
