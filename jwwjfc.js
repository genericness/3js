const swBase = self.location.pathname.replace(/jwwjfc\.js$/, '')

importScripts(swBase + 'krp/lvxm.bundle.js')
importScripts(swBase + 'krp/lvxm.config.js')
importScripts(__wfo$config.sw || swBase + 'krp/lvxm.sw.js')

importScripts(swBase + 'reza/ymte.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$iobscdmh', 1)
			...
			// `openDB('$iobscdmh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kysd/')) {
