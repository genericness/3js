const swBase = self.location.pathname.replace(/jgfdcc\.js$/, '')

importScripts(swBase + 'ura/vbli.bundle.js')
importScripts(swBase + 'ura/vbli.config.js')
importScripts(__dut$config.sw || swBase + 'ura/vbli.sw.js')

importScripts(swBase + 'lkzt/mpzd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jlligemh', 1)
			...
			// `openDB('$jlligemh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zxhg/')) {
