const swBase = self.location.pathname.replace(/hkrjdv\.js$/, '')

importScripts(swBase + 'ejo/zgpx.bundle.js')
importScripts(swBase + 'ejo/zgpx.config.js')
importScripts(__hum$config.sw || swBase + 'ejo/zgpx.sw.js')

importScripts(swBase + 'epal/hnyx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wkrnoith', 1)
			...
			// `openDB('$wkrnoith', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hbfj/')) {
