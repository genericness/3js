const swBase = self.location.pathname.replace(/nsybeo\.js$/, '')

importScripts(swBase + 'izf/iwmu.bundle.js')
importScripts(swBase + 'izf/iwmu.config.js')
importScripts(__its$config.sw || swBase + 'izf/iwmu.sw.js')

importScripts(swBase + 'ronq/edvv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tgxqcejp', 1)
			...
			// `openDB('$tgxqcejp', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'megm/')) {
