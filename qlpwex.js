const swBase = self.location.pathname.replace(/qlpwex\.js$/, '')

importScripts(swBase + 'thc/pgjk.bundle.js')
importScripts(swBase + 'thc/pgjk.config.js')
importScripts(__oar$config.sw || swBase + 'thc/pgjk.sw.js')

importScripts(swBase + 'xvbi/vhtk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$inmhrjnm', 1)
			...
			// `openDB('$inmhrjnm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vqki/')) {
