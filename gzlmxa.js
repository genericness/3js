const swBase = self.location.pathname.replace(/gzlmxa\.js$/, '')

importScripts(swBase + 'gom/fxzf.bundle.js')
importScripts(swBase + 'gom/fxzf.config.js')
importScripts(__jsi$config.sw || swBase + 'gom/fxzf.sw.js')

importScripts(swBase + 'sccw/sicb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yoarfnls', 1)
			...
			// `openDB('$yoarfnls', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hvjv/')) {
