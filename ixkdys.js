const swBase = self.location.pathname.replace(/ixkdys\.js$/, '')

importScripts(swBase + 'lmf/qnhc.bundle.js')
importScripts(swBase + 'lmf/qnhc.config.js')
importScripts(__bmj$config.sw || swBase + 'lmf/qnhc.sw.js')

importScripts(swBase + 'punt/vwvk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pnzmwhfz', 1)
			...
			// `openDB('$pnzmwhfz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ezrg/')) {
