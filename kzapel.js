const swBase = self.location.pathname.replace(/kzapel\.js$/, '')

importScripts(swBase + 'kdr/exnz.bundle.js')
importScripts(swBase + 'kdr/exnz.config.js')
importScripts(__flq$config.sw || swBase + 'kdr/exnz.sw.js')

importScripts(swBase + 'ctxa/wiev.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fyujyita', 1)
			...
			// `openDB('$fyujyita', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nxcz/')) {
