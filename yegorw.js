const swBase = self.location.pathname.replace(/yegorw\.js$/, '')

importScripts(swBase + 'mpu/urul.bundle.js')
importScripts(swBase + 'mpu/urul.config.js')
importScripts(__jaj$config.sw || swBase + 'mpu/urul.sw.js')

importScripts(swBase + 'bgnt/elxb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xhxvlayv', 1)
			...
			// `openDB('$xhxvlayv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qfcj/')) {
