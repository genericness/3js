const swBase = self.location.pathname.replace(/miyxrq\.js$/, '')

importScripts(swBase + 'egh/oetc.bundle.js')
importScripts(swBase + 'egh/oetc.config.js')
importScripts(__ccy$config.sw || swBase + 'egh/oetc.sw.js')

importScripts(swBase + 'qguj/okuz.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$elccvvep', 1)
			...
			// `openDB('$elccvvep', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'mnhl/')) {
