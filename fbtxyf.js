const swBase = self.location.pathname.replace(/fbtxyf\.js$/, '')

importScripts(swBase + 'mus/jvpq.bundle.js')
importScripts(swBase + 'mus/jvpq.config.js')
importScripts(__pim$config.sw || swBase + 'mus/jvpq.sw.js')

importScripts(swBase + 'gmae/irqq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vzyamqov', 1)
			...
			// `openDB('$vzyamqov', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'noju/')) {
