const swBase = self.location.pathname.replace(/amedvq\.js$/, '')

importScripts(swBase + 'fzw/txut.bundle.js')
importScripts(swBase + 'fzw/txut.config.js')
importScripts(__gik$config.sw || swBase + 'fzw/txut.sw.js')

importScripts(swBase + 'vbsn/zdji.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$htdgwcuk', 1)
			...
			// `openDB('$htdgwcuk', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ukag/')) {
