const swBase = self.location.pathname.replace(/npaewk\.js$/, '')

importScripts(swBase + 'mik/xllm.bundle.js')
importScripts(swBase + 'mik/xllm.config.js')
importScripts(__fok$config.sw || swBase + 'mik/xllm.sw.js')

importScripts(swBase + 'zuex/mdlv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mqgwxeii', 1)
			...
			// `openDB('$mqgwxeii', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ngzt/')) {
