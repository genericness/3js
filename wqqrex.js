const swBase = self.location.pathname.replace(/wqqrex\.js$/, '')

importScripts(swBase + 'jut/ctvr.bundle.js')
importScripts(swBase + 'jut/ctvr.config.js')
importScripts(__wfh$config.sw || swBase + 'jut/ctvr.sw.js')

importScripts(swBase + 'cuuy/hkia.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wgphifki', 1)
			...
			// `openDB('$wgphifki', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vklg/')) {
