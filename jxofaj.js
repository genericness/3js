const swBase = self.location.pathname.replace(/jxofaj\.js$/, '')

importScripts(swBase + 'bhs/lyae.bundle.js')
importScripts(swBase + 'bhs/lyae.config.js')
importScripts(__wdk$config.sw || swBase + 'bhs/lyae.sw.js')

importScripts(swBase + 'xlxb/nxcb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vibfgiwi', 1)
			...
			// `openDB('$vibfgiwi', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tfyd/')) {
