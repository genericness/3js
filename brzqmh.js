const swBase = self.location.pathname.replace(/brzqmh\.js$/, '')

importScripts(swBase + 'yjd/ajue.bundle.js')
importScripts(swBase + 'yjd/ajue.config.js')
importScripts(__sdc$config.sw || swBase + 'yjd/ajue.sw.js')

importScripts(swBase + 'jcul/xvhr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ezmuleaz', 1)
			...
			// `openDB('$ezmuleaz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'nshn/')) {
