const swBase = self.location.pathname.replace(/rtiiaw\.js$/, '')

importScripts(swBase + 'awi/wdcd.bundle.js')
importScripts(swBase + 'awi/wdcd.config.js')
importScripts(__rje$config.sw || swBase + 'awi/wdcd.sw.js')

importScripts(swBase + 'tkmg/xnol.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xzepczum', 1)
			...
			// `openDB('$xzepczum', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'foni/')) {
