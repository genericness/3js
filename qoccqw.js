const swBase = self.location.pathname.replace(/qoccqw\.js$/, '')

importScripts(swBase + 'wxc/ough.bundle.js')
importScripts(swBase + 'wxc/ough.config.js')
importScripts(__yax$config.sw || swBase + 'wxc/ough.sw.js')

importScripts(swBase + 'itbe/zxsn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nwannwhr', 1)
			...
			// `openDB('$nwannwhr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'envb/')) {
