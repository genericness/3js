const swBase = self.location.pathname.replace(/yyiehq\.js$/, '')

importScripts(swBase + 'zzj/uurb.bundle.js')
importScripts(swBase + 'zzj/uurb.config.js')
importScripts(__hnj$config.sw || swBase + 'zzj/uurb.sw.js')

importScripts(swBase + 'njnj/xyon.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$uznhxsir', 1)
			...
			// `openDB('$uznhxsir', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xrsr/')) {
