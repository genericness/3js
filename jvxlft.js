const swBase = self.location.pathname.replace(/jvxlft\.js$/, '')

importScripts(swBase + 'qhc/nhry.bundle.js')
importScripts(swBase + 'qhc/nhry.config.js')
importScripts(__urc$config.sw || swBase + 'qhc/nhry.sw.js')

importScripts(swBase + 'uapm/htfe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ldmyihdt', 1)
			...
			// `openDB('$ldmyihdt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hdsu/')) {
