const swBase = self.location.pathname.replace(/jrvevn\.js$/, '')

importScripts(swBase + 'dqa/ymeu.bundle.js')
importScripts(swBase + 'dqa/ymeu.config.js')
importScripts(__wiu$config.sw || swBase + 'dqa/ymeu.sw.js')

importScripts(swBase + 'exps/cvyk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$hpdvkbwl', 1)
			...
			// `openDB('$hpdvkbwl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vtqx/')) {
