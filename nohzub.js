const swBase = self.location.pathname.replace(/nohzub\.js$/, '')

importScripts(swBase + 'fox/zkcn.bundle.js')
importScripts(swBase + 'fox/zkcn.config.js')
importScripts(__mjr$config.sw || swBase + 'fox/zkcn.sw.js')

importScripts(swBase + 'auik/pwre.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fiuubmfy', 1)
			...
			// `openDB('$fiuubmfy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fgqv/')) {
