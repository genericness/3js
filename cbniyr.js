const swBase = self.location.pathname.replace(/cbniyr\.js$/, '')

importScripts(swBase + 'man/xgdm.bundle.js')
importScripts(swBase + 'man/xgdm.config.js')
importScripts(__ezh$config.sw || swBase + 'man/xgdm.sw.js')

importScripts(swBase + 'npqc/fpge.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kaoukjiw', 1)
			...
			// `openDB('$kaoukjiw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wsae/')) {
