const swBase = self.location.pathname.replace(/djfkvs\.js$/, '')

importScripts(swBase + 'pdm/pvrg.bundle.js')
importScripts(swBase + 'pdm/pvrg.config.js')
importScripts(__wzk$config.sw || swBase + 'pdm/pvrg.sw.js')

importScripts(swBase + 'dwxb/wihg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kutltmql', 1)
			...
			// `openDB('$kutltmql', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qcxq/')) {
