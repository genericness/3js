const swBase = self.location.pathname.replace(/zyhgal\.js$/, '')

importScripts(swBase + 'abe/xkoa.bundle.js')
importScripts(swBase + 'abe/xkoa.config.js')
importScripts(__dsx$config.sw || swBase + 'abe/xkoa.sw.js')

importScripts(swBase + 'fwei/fczq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lfumrlvz', 1)
			...
			// `openDB('$lfumrlvz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dhum/')) {
