const swBase = self.location.pathname.replace(/izoyyo\.js$/, '')

importScripts(swBase + 'rhb/nywb.bundle.js')
importScripts(swBase + 'rhb/nywb.config.js')
importScripts(__kjl$config.sw || swBase + 'rhb/nywb.sw.js')

importScripts(swBase + 'ingp/nhty.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cggsxsfm', 1)
			...
			// `openDB('$cggsxsfm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'olqs/')) {
