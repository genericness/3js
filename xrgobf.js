const swBase = self.location.pathname.replace(/xrgobf\.js$/, '')

importScripts(swBase + 'nxt/vptb.bundle.js')
importScripts(swBase + 'nxt/vptb.config.js')
importScripts(__rms$config.sw || swBase + 'nxt/vptb.sw.js')

importScripts(swBase + 'rhhb/yuze.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$chpgsqnz', 1)
			...
			// `openDB('$chpgsqnz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pntu/')) {
