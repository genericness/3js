const swBase = self.location.pathname.replace(/xceyvh\.js$/, '')

importScripts(swBase + 'mkv/bzvy.bundle.js')
importScripts(swBase + 'mkv/bzvy.config.js')
importScripts(__hcm$config.sw || swBase + 'mkv/bzvy.sw.js')

importScripts(swBase + 'ewrs/dmfe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$nmgtiajy', 1)
			...
			// `openDB('$nmgtiajy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jjcb/')) {
