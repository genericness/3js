const swBase = self.location.pathname.replace(/zmdcjr\.js$/, '')

importScripts(swBase + 'zjb/yezh.bundle.js')
importScripts(swBase + 'zjb/yezh.config.js')
importScripts(__lxm$config.sw || swBase + 'zjb/yezh.sw.js')

importScripts(swBase + 'tfko/yotv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sgjyzmpw', 1)
			...
			// `openDB('$sgjyzmpw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qcoz/')) {
