const swBase = self.location.pathname.replace(/klxutg\.js$/, '')

importScripts(swBase + 'cme/urlr.bundle.js')
importScripts(swBase + 'cme/urlr.config.js')
importScripts(__ejr$config.sw || swBase + 'cme/urlr.sw.js')

importScripts(swBase + 'fqxc/hefk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$eileiark', 1)
			...
			// `openDB('$eileiark', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ttqw/')) {
