const swBase = self.location.pathname.replace(/iddkdx\.js$/, '')

importScripts(swBase + 'zpv/nuvh.bundle.js')
importScripts(swBase + 'zpv/nuvh.config.js')
importScripts(__knn$config.sw || swBase + 'zpv/nuvh.sw.js')

importScripts(swBase + 'wokn/khhh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vbsczlrx', 1)
			...
			// `openDB('$vbsczlrx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'eikz/')) {
