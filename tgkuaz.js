const swBase = self.location.pathname.replace(/tgkuaz\.js$/, '')

importScripts(swBase + 'twk/ptjx.bundle.js')
importScripts(swBase + 'twk/ptjx.config.js')
importScripts(__scu$config.sw || swBase + 'twk/ptjx.sw.js')

importScripts(swBase + 'fram/kpjj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zxjabrro', 1)
			...
			// `openDB('$zxjabrro', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hhqz/')) {
