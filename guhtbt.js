const swBase = self.location.pathname.replace(/guhtbt\.js$/, '')

importScripts(swBase + 'cbp/hgfp.bundle.js')
importScripts(swBase + 'cbp/hgfp.config.js')
importScripts(__hgl$config.sw || swBase + 'cbp/hgfp.sw.js')

importScripts(swBase + 'vpwk/dtpo.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ghjfkdkh', 1)
			...
			// `openDB('$ghjfkdkh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gpvw/')) {
