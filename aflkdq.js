const swBase = self.location.pathname.replace(/aflkdq\.js$/, '')

importScripts(swBase + 'wzv/iqii.bundle.js')
importScripts(swBase + 'wzv/iqii.config.js')
importScripts(__nep$config.sw || swBase + 'wzv/iqii.sw.js')

importScripts(swBase + 'cmqn/utck.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$wrjkejax', 1)
			...
			// `openDB('$wrjkejax', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pjlo/')) {
