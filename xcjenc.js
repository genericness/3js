const swBase = self.location.pathname.replace(/xcjenc\.js$/, '')

importScripts(swBase + 'gwq/rzie.bundle.js')
importScripts(swBase + 'gwq/rzie.config.js')
importScripts(__ypc$config.sw || swBase + 'gwq/rzie.sw.js')

importScripts(swBase + 'aalp/qpxj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$edkoehta', 1)
			...
			// `openDB('$edkoehta', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pbbg/')) {
