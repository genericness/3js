const swBase = self.location.pathname.replace(/gfjgpy\.js$/, '')

importScripts(swBase + 'bla/tlvt.bundle.js')
importScripts(swBase + 'bla/tlvt.config.js')
importScripts(__nlv$config.sw || swBase + 'bla/tlvt.sw.js')

importScripts(swBase + 'osqw/vwlc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$jtnsmdfc', 1)
			...
			// `openDB('$jtnsmdfc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'jlnz/')) {
