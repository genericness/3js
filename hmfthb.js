const swBase = self.location.pathname.replace(/hmfthb\.js$/, '')

importScripts(swBase + 'arb/xqht.bundle.js')
importScripts(swBase + 'arb/xqht.config.js')
importScripts(__qoq$config.sw || swBase + 'arb/xqht.sw.js')

importScripts(swBase + 'jtpj/awpv.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$iqhkaskm', 1)
			...
			// `openDB('$iqhkaskm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hpmb/')) {
