const swBase = self.location.pathname.replace(/fycnjw\.js$/, '')

importScripts(swBase + 'qhe/uxay.bundle.js')
importScripts(swBase + 'qhe/uxay.config.js')
importScripts(__hjj$config.sw || swBase + 'qhe/uxay.sw.js')

importScripts(swBase + 'xsji/iaid.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$pdhfpnia', 1)
			...
			// `openDB('$pdhfpnia', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wtiz/')) {
