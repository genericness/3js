const swBase = self.location.pathname.replace(/zuydnn\.js$/, '')

importScripts(swBase + 'yzr/wzzi.bundle.js')
importScripts(swBase + 'yzr/wzzi.config.js')
importScripts(__iyv$config.sw || swBase + 'yzr/wzzi.sw.js')

importScripts(swBase + 'zjfe/rjxk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$samgjvkw', 1)
			...
			// `openDB('$samgjvkw', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gxki/')) {
