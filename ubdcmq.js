const swBase = self.location.pathname.replace(/ubdcmq\.js$/, '')

importScripts(swBase + 'wac/cjlr.bundle.js')
importScripts(swBase + 'wac/cjlr.config.js')
importScripts(__has$config.sw || swBase + 'wac/cjlr.sw.js')

importScripts(swBase + 'yxbs/dgbn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yacqokve', 1)
			...
			// `openDB('$yacqokve', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qerj/')) {
