const swBase = self.location.pathname.replace(/igobyf\.js$/, '')

importScripts(swBase + 'are/wgdb.bundle.js')
importScripts(swBase + 'are/wgdb.config.js')
importScripts(__tnv$config.sw || swBase + 'are/wgdb.sw.js')

importScripts(swBase + 'vtrl/kriy.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gevzyrkc', 1)
			...
			// `openDB('$gevzyrkc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gjoy/')) {
