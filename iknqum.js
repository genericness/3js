const swBase = self.location.pathname.replace(/iknqum\.js$/, '')

importScripts(swBase + 'qsc/rwax.bundle.js')
importScripts(swBase + 'qsc/rwax.config.js')
importScripts(__gzb$config.sw || swBase + 'qsc/rwax.sw.js')

importScripts(swBase + 'anvl/ovmr.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$oijjfhwo', 1)
			...
			// `openDB('$oijjfhwo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zfcb/')) {
