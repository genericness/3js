const swBase = self.location.pathname.replace(/sszwxl\.js$/, '')

importScripts(swBase + 'xum/gztr.bundle.js')
importScripts(swBase + 'xum/gztr.config.js')
importScripts(__yvm$config.sw || swBase + 'xum/gztr.sw.js')

importScripts(swBase + 'dzsb/uxmc.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dwfpztnu', 1)
			...
			// `openDB('$dwfpztnu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'oolp/')) {
