const swBase = self.location.pathname.replace(/afpxnn\.js$/, '')

importScripts(swBase + 'fwy/ehyz.bundle.js')
importScripts(swBase + 'fwy/ehyz.config.js')
importScripts(__mcf$config.sw || swBase + 'fwy/ehyz.sw.js')

importScripts(swBase + 'bbpq/mqku.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ygtgwdme', 1)
			...
			// `openDB('$ygtgwdme', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ryzg/')) {
