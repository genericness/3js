const swBase = self.location.pathname.replace(/alljib\.js$/, '')

importScripts(swBase + 'olj/lpwp.bundle.js')
importScripts(swBase + 'olj/lpwp.config.js')
importScripts(__evc$config.sw || swBase + 'olj/lpwp.sw.js')

importScripts(swBase + 'wevr/oaqg.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kgtqnewc', 1)
			...
			// `openDB('$kgtqnewc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vnxb/')) {
