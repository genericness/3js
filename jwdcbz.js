const swBase = self.location.pathname.replace(/jwdcbz\.js$/, '')

importScripts(swBase + 'hbi/kutp.bundle.js')
importScripts(swBase + 'hbi/kutp.config.js')
importScripts(__cjl$config.sw || swBase + 'hbi/kutp.sw.js')

importScripts(swBase + 'rcqx/qpct.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$plpbgxbb', 1)
			...
			// `openDB('$plpbgxbb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wcth/')) {
