const swBase = self.location.pathname.replace(/xgzgqm\.js$/, '')

importScripts(swBase + 'oss/yrva.bundle.js')
importScripts(swBase + 'oss/yrva.config.js')
importScripts(__zac$config.sw || swBase + 'oss/yrva.sw.js')

importScripts(swBase + 'hnni/jicl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rltyiqgs', 1)
			...
			// `openDB('$rltyiqgs', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rdfb/')) {
