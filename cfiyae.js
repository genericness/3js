const swBase = self.location.pathname.replace(/cfiyae\.js$/, '')

importScripts(swBase + 'gab/sodo.bundle.js')
importScripts(swBase + 'gab/sodo.config.js')
importScripts(__lrk$config.sw || swBase + 'gab/sodo.sw.js')

importScripts(swBase + 'ymnm/apfq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$udzbsmnc', 1)
			...
			// `openDB('$udzbsmnc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ezge/')) {
