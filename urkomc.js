const swBase = self.location.pathname.replace(/urkomc\.js$/, '')

importScripts(swBase + 'ksw/rcfi.bundle.js')
importScripts(swBase + 'ksw/rcfi.config.js')
importScripts(__aes$config.sw || swBase + 'ksw/rcfi.sw.js')

importScripts(swBase + 'losv/mxyd.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$krysnity', 1)
			...
			// `openDB('$krysnity', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lpfa/')) {
