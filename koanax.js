const swBase = self.location.pathname.replace(/koanax\.js$/, '')

importScripts(swBase + 'ock/dqpq.bundle.js')
importScripts(swBase + 'ock/dqpq.config.js')
importScripts(__phm$config.sw || swBase + 'ock/dqpq.sw.js')

importScripts(swBase + 'ualw/upfe.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fybfguop', 1)
			...
			// `openDB('$fybfguop', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'dvds/')) {
