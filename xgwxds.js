const swBase = self.location.pathname.replace(/xgwxds\.js$/, '')

importScripts(swBase + 'gps/wgsx.bundle.js')
importScripts(swBase + 'gps/wgsx.config.js')
importScripts(__rwf$config.sw || swBase + 'gps/wgsx.sw.js')

importScripts(swBase + 'vhkl/uzca.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zurzioxa', 1)
			...
			// `openDB('$zurzioxa', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fngj/')) {
