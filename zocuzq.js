const swBase = self.location.pathname.replace(/zocuzq\.js$/, '')

importScripts(swBase + 'uis/kwvs.bundle.js')
importScripts(swBase + 'uis/kwvs.config.js')
importScripts(__jtd$config.sw || swBase + 'uis/kwvs.sw.js')

importScripts(swBase + 'okkn/swcb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dbnpyrjx', 1)
			...
			// `openDB('$dbnpyrjx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'pcrj/')) {
