const swBase = self.location.pathname.replace(/cgprlm\.js$/, '')

importScripts(swBase + 'key/blnr.bundle.js')
importScripts(swBase + 'key/blnr.config.js')
importScripts(__sbo$config.sw || swBase + 'key/blnr.sw.js')

importScripts(swBase + 'zlii/stmb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$kbdpgizq', 1)
			...
			// `openDB('$kbdpgizq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ajzo/')) {
