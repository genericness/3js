const swBase = self.location.pathname.replace(/sbqriq\.js$/, '')

importScripts(swBase + 'dkx/mwzn.bundle.js')
importScripts(swBase + 'dkx/mwzn.config.js')
importScripts(__vvs$config.sw || swBase + 'dkx/mwzn.sw.js')

importScripts(swBase + 'qmmd/ozwt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$yzbzyyqb', 1)
			...
			// `openDB('$yzbzyyqb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'goeq/')) {
