const swBase = self.location.pathname.replace(/alisoi\.js$/, '')

importScripts(swBase + 'tjg/tgff.bundle.js')
importScripts(swBase + 'tjg/tgff.config.js')
importScripts(__qkd$config.sw || swBase + 'tjg/tgff.sw.js')

importScripts(swBase + 'ritr/rhpb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$rmkzqgaz', 1)
			...
			// `openDB('$rmkzqgaz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'igyq/')) {
