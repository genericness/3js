const swBase = self.location.pathname.replace(/xwigfi\.js$/, '')

importScripts(swBase + 'tge/vpga.bundle.js')
importScripts(swBase + 'tge/vpga.config.js')
importScripts(__iyv$config.sw || swBase + 'tge/vpga.sw.js')

importScripts(swBase + 'fhhx/bsab.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dlljjemr', 1)
			...
			// `openDB('$dlljjemr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wnfh/')) {
