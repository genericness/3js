const swBase = self.location.pathname.replace(/osmxjp\.js$/, '')

importScripts(swBase + 'rkg/pycd.bundle.js')
importScripts(swBase + 'rkg/pycd.config.js')
importScripts(__ugh$config.sw || swBase + 'rkg/pycd.sw.js')

importScripts(swBase + 'caqh/qvrq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bmssthkb', 1)
			...
			// `openDB('$bmssthkb', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fazf/')) {
