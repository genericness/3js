const swBase = self.location.pathname.replace(/sqrqzh\.js$/, '')

importScripts(swBase + 'vtf/czoy.bundle.js')
importScripts(swBase + 'vtf/czoy.config.js')
importScripts(__cbx$config.sw || swBase + 'vtf/czoy.sw.js')

importScripts(swBase + 'rpxg/myyu.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$viydvhlz', 1)
			...
			// `openDB('$viydvhlz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'esbk/')) {
