const swBase = self.location.pathname.replace(/waipyf\.js$/, '')

importScripts(swBase + 'szy/oshx.bundle.js')
importScripts(swBase + 'szy/oshx.config.js')
importScripts(__ood$config.sw || swBase + 'szy/oshx.sw.js')

importScripts(swBase + 'ztkz/zjuq.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sccciugt', 1)
			...
			// `openDB('$sccciugt', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'xoit/')) {
