const swBase = self.location.pathname.replace(/likoam\.js$/, '')

importScripts(swBase + 'wfs/bziv.bundle.js')
importScripts(swBase + 'wfs/bziv.config.js')
importScripts(__cul$config.sw || swBase + 'wfs/bziv.sw.js')

importScripts(swBase + 'legi/eeom.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$dczlwmvd', 1)
			...
			// `openDB('$dczlwmvd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wmwx/')) {
