const swBase = self.location.pathname.replace(/nepodb\.js$/, '')

importScripts(swBase + 'uob/jdhu.bundle.js')
importScripts(swBase + 'uob/jdhu.config.js')
importScripts(__tfg$config.sw || swBase + 'uob/jdhu.sw.js')

importScripts(swBase + 'sjfb/gmer.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xsjdecem', 1)
			...
			// `openDB('$xsjdecem', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'iooy/')) {
