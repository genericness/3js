const swBase = self.location.pathname.replace(/fnggse\.js$/, '')

importScripts(swBase + 'mua/kkjr.bundle.js')
importScripts(swBase + 'mua/kkjr.config.js')
importScripts(__idg$config.sw || swBase + 'mua/kkjr.sw.js')

importScripts(swBase + 'milz/ysed.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$gvpbkaxo', 1)
			...
			// `openDB('$gvpbkaxo', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ivjf/')) {
