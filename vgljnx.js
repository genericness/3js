const swBase = self.location.pathname.replace(/vgljnx\.js$/, '')

importScripts(swBase + 'bog/pnaw.bundle.js')
importScripts(swBase + 'bog/pnaw.config.js')
importScripts(__csy$config.sw || swBase + 'bog/pnaw.sw.js')

importScripts(swBase + 'looo/rnly.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ktcnoqcg', 1)
			...
			// `openDB('$ktcnoqcg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'gpoe/')) {
