const swBase = self.location.pathname.replace(/viylcy\.js$/, '')

importScripts(swBase + 'wzz/fvbj.bundle.js')
importScripts(swBase + 'wzz/fvbj.config.js')
importScripts(__pni$config.sw || swBase + 'wzz/fvbj.sw.js')

importScripts(swBase + 'bidl/qhzf.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sxxpdayu', 1)
			...
			// `openDB('$sxxpdayu', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tpkd/')) {
