const swBase = self.location.pathname.replace(/xvlaco\.js$/, '')

importScripts(swBase + 'kll/kkge.bundle.js')
importScripts(swBase + 'kll/kkge.config.js')
importScripts(__fhc$config.sw || swBase + 'kll/kkge.sw.js')

importScripts(swBase + 'wplr/rsvj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$sjgaihbd', 1)
			...
			// `openDB('$sjgaihbd', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'lttg/')) {
