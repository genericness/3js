const swBase = self.location.pathname.replace(/zaobbq\.js$/, '')

importScripts(swBase + 'ngm/pslf.bundle.js')
importScripts(swBase + 'ngm/pslf.config.js')
importScripts(__lvu$config.sw || swBase + 'ngm/pslf.sw.js')

importScripts(swBase + 'xtma/nkku.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$cyfczohz', 1)
			...
			// `openDB('$cyfczohz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'vpoj/')) {
