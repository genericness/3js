const swBase = self.location.pathname.replace(/kqdiwd\.js$/, '')

importScripts(swBase + 'tcv/zhsg.bundle.js')
importScripts(swBase + 'tcv/zhsg.config.js')
importScripts(__cmr$config.sw || swBase + 'tcv/zhsg.sw.js')

importScripts(swBase + 'vlic/vcwt.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xvnrpmkm', 1)
			...
			// `openDB('$xvnrpmkm', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'denb/')) {
