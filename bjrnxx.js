const swBase = self.location.pathname.replace(/bjrnxx\.js$/, '')

importScripts(swBase + 'dql/iffc.bundle.js')
importScripts(swBase + 'dql/iffc.config.js')
importScripts(__wmy$config.sw || swBase + 'dql/iffc.sw.js')

importScripts(swBase + 'ehal/flnb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$fkiwuxoh', 1)
			...
			// `openDB('$fkiwuxoh', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'yaty/')) {
