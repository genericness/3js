const swBase = self.location.pathname.replace(/kaxbky\.js$/, '')

importScripts(swBase + 'hqj/budj.bundle.js')
importScripts(swBase + 'hqj/budj.config.js')
importScripts(__sol$config.sw || swBase + 'hqj/budj.sw.js')

importScripts(swBase + 'uuzw/edqh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lrjurkoa', 1)
			...
			// `openDB('$lrjurkoa', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'qpry/')) {
