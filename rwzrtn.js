const swBase = self.location.pathname.replace(/rwzrtn\.js$/, '')

importScripts(swBase + 'wgt/wtlb.bundle.js')
importScripts(swBase + 'wgt/wtlb.config.js')
importScripts(__pql$config.sw || swBase + 'wgt/wtlb.sw.js')

importScripts(swBase + 'sjgs/aqnj.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$anoynstq', 1)
			...
			// `openDB('$anoynstq', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ocoa/')) {
