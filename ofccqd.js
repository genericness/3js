const swBase = self.location.pathname.replace(/ofccqd\.js$/, '')

importScripts(swBase + 'tll/uyvu.bundle.js')
importScripts(swBase + 'tll/uyvu.config.js')
importScripts(__ood$config.sw || swBase + 'tll/uyvu.sw.js')

importScripts(swBase + 'ktgn/vooh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vgczowtl', 1)
			...
			// `openDB('$vgczowtl', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'cfnq/')) {
