const swBase = self.location.pathname.replace(/oqtlde\.js$/, '')

importScripts(swBase + 'kqq/helj.bundle.js')
importScripts(swBase + 'kqq/helj.config.js')
importScripts(__yyz$config.sw || swBase + 'kqq/helj.sw.js')

importScripts(swBase + 'dwpe/nfey.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$areojxux', 1)
			...
			// `openDB('$areojxux', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'orhs/')) {
