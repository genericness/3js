const swBase = self.location.pathname.replace(/gbmgzn\.js$/, '')

importScripts(swBase + 'xbi/oryp.bundle.js')
importScripts(swBase + 'xbi/oryp.config.js')
importScripts(__cit$config.sw || swBase + 'xbi/oryp.sw.js')

importScripts(swBase + 'rdlc/uqzn.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$mhvqwxun', 1)
			...
			// `openDB('$mhvqwxun', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'kbrp/')) {
