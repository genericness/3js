const swBase = self.location.pathname.replace(/ubdtaz\.js$/, '')

importScripts(swBase + 'sgr/fndw.bundle.js')
importScripts(swBase + 'sgr/fndw.config.js')
importScripts(__jag$config.sw || swBase + 'sgr/fndw.sw.js')

importScripts(swBase + 'ednv/ueal.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$zgatxovy', 1)
			...
			// `openDB('$zgatxovy', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'difx/')) {
