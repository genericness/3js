const swBase = self.location.pathname.replace(/zukfiw\.js$/, '')

importScripts(swBase + 'ydt/ogfg.bundle.js')
importScripts(swBase + 'ydt/ogfg.config.js')
importScripts(__cgf$config.sw || swBase + 'ydt/ogfg.sw.js')

importScripts(swBase + 'gbdn/vzey.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ivvgurts', 1)
			...
			// `openDB('$ivvgurts', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rscs/')) {
