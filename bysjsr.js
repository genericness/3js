const swBase = self.location.pathname.replace(/bysjsr\.js$/, '')

importScripts(swBase + 'nfz/ozuv.bundle.js')
importScripts(swBase + 'nfz/ozuv.config.js')
importScripts(__hcn$config.sw || swBase + 'nfz/ozuv.sw.js')

importScripts(swBase + 'imyq/kpde.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$emukrjql', 1)
			...
			// `openDB('$emukrjql', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'wone/')) {
