const swBase = self.location.pathname.replace(/utjtzd\.js$/, '')

importScripts(swBase + 'lxd/fzwe.bundle.js')
importScripts(swBase + 'lxd/fzwe.config.js')
importScripts(__jbo$config.sw || swBase + 'lxd/fzwe.sw.js')

importScripts(swBase + 'gilz/yypk.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$bevkxhpc', 1)
			...
			// `openDB('$bevkxhpc', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'uurd/')) {
