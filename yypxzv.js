const swBase = self.location.pathname.replace(/yypxzv\.js$/, '')

importScripts(swBase + 'cua/actb.bundle.js')
importScripts(swBase + 'cua/actb.config.js')
importScripts(__qiw$config.sw || swBase + 'cua/actb.sw.js')

importScripts(swBase + 'ojpq/qdcb.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$vvfhsssz', 1)
			...
			// `openDB('$vvfhsssz', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'fhyk/')) {
