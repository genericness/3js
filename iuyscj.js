const swBase = self.location.pathname.replace(/iuyscj\.js$/, '')

importScripts(swBase + 'ngv/gyao.bundle.js')
importScripts(swBase + 'ngv/gyao.config.js')
importScripts(__lar$config.sw || swBase + 'ngv/gyao.sw.js')

importScripts(swBase + 'kndq/txhl.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$aicjzftv', 1)
			...
			// `openDB('$aicjzftv', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'tyxo/')) {
