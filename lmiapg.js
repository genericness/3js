const swBase = self.location.pathname.replace(/lmiapg\.js$/, '')

importScripts(swBase + 'bml/ggyo.bundle.js')
importScripts(swBase + 'bml/ggyo.config.js')
importScripts(__jcx$config.sw || swBase + 'bml/ggyo.sw.js')

importScripts(swBase + 'xpou/mrrx.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$xjrlykec', 1)
			...
			// `openDB('$xjrlykec', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'zuan/')) {
