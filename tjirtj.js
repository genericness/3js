const swBase = self.location.pathname.replace(/tjirtj\.js$/, '')

importScripts(swBase + 'twi/tbtc.bundle.js')
importScripts(swBase + 'twi/tbtc.config.js')
importScripts(__sso$config.sw || swBase + 'twi/tbtc.sw.js')

importScripts(swBase + 'nzqa/xvli.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ionedcua', 1)
			...
			// `openDB('$ionedcua', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ddpz/')) {
