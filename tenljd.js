const swBase = self.location.pathname.replace(/tenljd\.js$/, '')

importScripts(swBase + 'rjy/asxh.bundle.js')
importScripts(swBase + 'rjy/asxh.config.js')
importScripts(__lgf$config.sw || swBase + 'rjy/asxh.sw.js')

importScripts(swBase + 'ofno/awbh.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$ptwblrwr', 1)
			...
			// `openDB('$ptwblrwr', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'hnqy/')) {
