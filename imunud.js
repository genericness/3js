const swBase = self.location.pathname.replace(/imunud\.js$/, '')

importScripts(swBase + 'jes/iyck.bundle.js')
importScripts(swBase + 'jes/iyck.config.js')
importScripts(__ekl$config.sw || swBase + 'jes/iyck.sw.js')

importScripts(swBase + 'eqgj/puos.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$tlyififx', 1)
			...
			// `openDB('$tlyififx', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'rpcu/')) {
