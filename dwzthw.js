const swBase = self.location.pathname.replace(/dwzthw\.js$/, '')

importScripts(swBase + 'hzf/vhfo.bundle.js')
importScripts(swBase + 'hzf/vhfo.config.js')
importScripts(__wup$config.sw || swBase + 'hzf/vhfo.sw.js')

importScripts(swBase + 'mebc/doir.all.js')
...
function patchSjDestCodec() {
	return new Promise((resolve) => {
		...
		try {
			const req = indexedDB.open('$lnckkuvg', 1)
			...
			// `openDB('$lnckkuvg', 1, { upgrade })` skip creating its stores.
			req.onupgradeneeded = () => {
				try {
...
self.addEventListener('fetch', (event) => {
	...
	if (url.pathname.startsWith(swBase + 'ataq/')) {
